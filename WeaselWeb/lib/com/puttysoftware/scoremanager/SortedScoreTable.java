...
public class SortedScoreTable extends ScoreTable {
    ...
    public SortedScoreTable(final int mv, final int length,
        ...
        for (x = 0; x < this.table.size(); x++) {
            this.table.get(x).setScore(startingScore);
        }
    }
    ...
    public void addScore(final int mv, final long[] newScore,
            final String newName) {
        final Score newEntry = new Score(mv, newScore, newName);
        if (this.sortOrder) {
            // Append the new score to the end
            this.table.add(newEntry);
            // Sort the score table
    ...
    public boolean checkScore(final int mv, final long[] newScore) {
        final Score newEntry = new Score(mv, newScore, "");
    ...
    public static SortedScoreTable readSortedScoreTableXML(final XDataReader xdr)
        ...
        final SortedScoreTable sst = new SortedScoreTable(unitLen, len, order,
                unitArr);
        for (int x = 0; x < len; x++) {
            sst.table.set(x, Score.readScoreXML(xdr));
        }
        return sst;
    }

    public void writeSortedScoreTableXML(final XDataWriter xdw)
            throws IOException {
