...
public class GameManager implements MazeEffectConstants {
    ...
    public GameManager() {
        ...
        this.lastUsedBowIndex = WeaselWeb.getApplication().getObjects()
    ...
    private void saveSavedMazeObject() {
        ...
        final GenericCharacter player = (GenericCharacter) WeaselWeb
    ...
    public void doRemoteAction(final int x, final int y, final int z) {
        ...
        final MazeObject acted = WeaselWeb.getApplication().getMazeManager()
                ...
                for (y = this.vwMgr.getViewingWindowLocationY(); y <= this.vwMgr
                    ...
                    try {
                        if (visible) {
                            ...
                            this.drawGrid[xFix][yFix].setIcon(ImageManager
                                    .getCompositeImage(icon1, icon2));
                        } else {
                            this.drawGrid[xFix][yFix]
                                    .setIcon(ImageManager.getImage("Darkness"));
                        }
                    } catch (final ArrayIndexOutOfBoundsException ae) {
                        if (this.trueSightFlag) {
                            this.drawGrid[xFix][yFix].setIcon(
                                    new EmptyVoid().editorRenderHook(y, x,
                                            this.plMgr.getPlayerLocationZ()));
                        } else {
                            this.drawGrid[xFix][yFix].setIcon(
    ...
    public void decay() {
        if (this.actingRemotely) {
            WeaselWeb.getApplication().getMazeManager().getMaze().setCell(
    ...
    public void decayTo(final MazeObject decay) {
        if (this.actingRemotely) {
            WeaselWeb.getApplication().getMazeManager().getMaze().setCell(decay,
    ...
    public void showSwitchBowDialog() {
        ...
        final String result = CommonDialogs.showInputDialog(
                "Switch to which bow?", "WeaselWeb", userChoices,
    ...
    public void identifyObject(final int x, final int y) {
        ...
        try {
            ...
            WeaselWeb.getApplication()
                    .showMessage(ev.getGameBase().getImageName());
            SoundManager.playSound(SoundConstants.SOUND_CATEGORY_SOLVING_MAZE,
                    SoundConstants.SOUND_IDENTIFY);
        }
    }
