...
public abstract class Creature implements StatConstants {
    ...
    public void doDamagePercentage(final int fP) {
        int percent = fP;
        if (percent > Creature.FULL_HEAL_PERCENTAGE) {
            percent = Creature.FULL_HEAL_PERCENTAGE;
        }
        if (percent < 0) {
            percent = 0;
        }
        final double percentMultiplier = percent
                / (double) Creature.FULL_HEAL_PERCENTAGE;
        int modValue = (int) (this.getEffectedMaximumHP() * percentMultiplier);
        if (modValue <= 0) {
            modValue = 1;
        }
    ...
    public void drainPercentage(final int fP) {
        int percent = fP;
        if (percent > Creature.FULL_HEAL_PERCENTAGE) {
            percent = Creature.FULL_HEAL_PERCENTAGE;
        }
        ...
        final double percentMultiplier = percent
                / (double) Creature.FULL_HEAL_PERCENTAGE;
        int modValue = (int) (this.getMaximumMP() * percentMultiplier);
        if (modValue <= 0) {
            modValue = 1;
        }
    ...
    public String getAllEffectMessages(final int which) {
        ...
        for (x = 0; x < this.effectList.length; x++) {
            try {
                sb.append(this.get(x).getMessage(which));
                sb.append("\n");
            } catch (final NullPointerException np) {
                // Do nothing
            } catch (final ArrayIndexOutOfBoundsException aioob) {
    ...
    public int getAttacksPerRound() {
        return this.stats[StatConstants.STAT_ATTACKS_PER_ROUND];
    }
    ...
    public int getSpellsPerRound() {
        return this.stats[StatConstants.STAT_SPELLS_PER_ROUND];
    }
    ...
    public void regeneratePercentage(final int fP) {
        int percent = fP;
        if (percent > Creature.FULL_HEAL_PERCENTAGE) {
            percent = Creature.FULL_HEAL_PERCENTAGE;
        }
    ...
    public void setAttacksPerRound(final int value) {
        this.stats[StatConstants.STAT_ATTACKS_PER_ROUND] = value;
    }
    ...
    public void setSpellsPerRound(final int value) {
        this.stats[StatConstants.STAT_SPELLS_PER_ROUND] = value;
    }
