...
import net.worldwizard.worldz.objects.HorizontalBarrier;
import net.worldwizard.worldz.objects.VerticalBarrier;
...
public abstract class GenericGenerator extends GenericWall {
    ...
    public boolean preMoveAction(final boolean ie, final int dirX,
        ...
        final WorldObject mo2 = app.getWorldManager().getWorldObject(dirX - 1,
                dirY, pz, WorldConstants.LAYER_OBJECT);
        try {
            mo2Name = mo2.getName();
        ...
        final WorldObject mo4 = app.getWorldManager().getWorldObject(dirX,
                dirY - 1, pz, WorldConstants.LAYER_OBJECT);
        try {
            mo4Name = mo4.getName();
        } catch (final NullPointerException np) {
            mo4Name = invalidName;
        ...
        final WorldObject mo6 = app.getWorldManager().getWorldObject(dirX,
                dirY + 1, pz, WorldConstants.LAYER_OBJECT);
        try {
            mo6Name = mo6.getName();
        } catch (final NullPointerException np) {
        ...
        final WorldObject mo8 = app.getWorldManager().getWorldObject(dirX + 1,
                dirY, pz, WorldConstants.LAYER_OBJECT);
        try {
            mo8Name = mo8.getName();
        } catch (final NullPointerException np) {
        ...
        if (flag) {
            ...
            this.activateTimer(this.TIMER_DELAY);
            app.getGameManager().redrawWorldNoRebuild();
        }
    ...
    protected boolean scan(final int dir, final int x, final int y, final int z,
        ...
        if (dir == DirectionConstants.DIRECTION_EAST) {
            for (int l = 1; l < limit; l++) {
                final WorldObject mo = app.getWorldManager().getWorldObject(
                        x + l, y, z, WorldConstants.LAYER_OBJECT);
                String moName;
                try {
                    moName = mo.getName();
                } catch (final NullPointerException np) {
            ...
            for (int l = 1; l < limit; l++) {
                final WorldObject mo = app.getWorldManager().getWorldObject(x,
                        y - l, z, WorldConstants.LAYER_OBJECT);
                String moName;
                try {
                    moName = mo.getName();
                } catch (final NullPointerException np) {
