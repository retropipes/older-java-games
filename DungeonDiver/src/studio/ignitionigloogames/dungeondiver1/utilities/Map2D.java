...
public class Map2D implements NDimensionalMap {
    ...
    public MapObject getCellOffset(final NDimensionalLocation loc,
        ...
        if (this.mapData != null && col >= 0 && col < this.mapData.length
                && this.mapData[col] != null && row >= 0
                && row < this.mapData[col].length
                && this.mapData[col][row] != null) {
            return this.mapData[col][row];
        }
        return defaultValue;
    }
    ...
    public void drawGame(final NDimensionalLocation otherDimensions) {
        ...
        for (x = this.viewingWindowLocation.x; x < maxViewingWindowColumn; x++) {
            for (y = this.viewingWindowLocation.y; y < maxViewingWindowRow; y++) {
                if (this.mapData != null && y >= 0 && y < this.mapData.length
                        && this.mapData[y] != null && x >= 0
