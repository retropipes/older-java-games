...
public class BossAIRoutine extends AIRoutine {
    ...
    public int getNextAction(final Creature c) {
        ...
        final int action = (int) whichAction.generate();
        if (action == 1) {
        ...
        if (cost <= currMP) {
            ...
            if (action == 1) {
                if (this.delevelRounds == 0) {
                    if ((int) chance.generate() <= BossAIRoutine.DELEVEL_CHANCE) {
                ...
                if (this.chargeRounds == 0) {
                    if ((int) chance.generate() <= BossAIRoutine.CHARGE_CHANCE) {
                ...
                if (cost <= currMP && this.poisonRounds == 0) {
                    if ((int) chance.generate() <= BossAIRoutine.POISON_CHANCE) {
                ...
                if (cost <= currMP) {
                    ...
                    if (currHP <= targetHP) {
                        if ((int) chance.generate() <= BossAIRoutine.HEAL_CHANCE) {
                            this.spell = which;
                            return AIRoutine.ACTION_CAST_SPELL;
                        } else {
                            this.spell = null;
