...
import com.puttysoftware.fantastlex.ai.map.AbstractMapAIRoutine;
import com.puttysoftware.fantastlex.ai.map.AutoMapAI;
import com.puttysoftware.fantastlex.ai.map.MapAIContext;
...
public class MapBattleLogic extends AbstractBattle {
    ...
    private void doBattleInternal(final Maze bMaze, final MapBattle b) {
        // Initialize Battle
        FantastleX.getApplication().setInBattle();
        ...
        if (!this.resultDoneAlready) {
            ...
            if (this.result == BattleResults.WON) {
                ...
                FantastleX
                        ...
                        .getGameManager()
                        .addToScore(
                                Math.max(1, (this.vsd.getTotalExp() + gold)
            ...
            // Leave Battle
            this.hideBattle();
            FantastleX.getApplication().setInGame();
            // Return to whence we came
            FantastleX.getApplication().getGameManager().showOutput();
            FantastleX.getApplication().getGameManager().redrawMaze();
            FantastleX.getApplication().getGameManager().updateStats();
            // Check for Game Over
            FantastleX.getApplication().getGameManager().checkGameOver();
        }
    }
    ...
    public void executeNextAIAction() {
        ...
        case AbstractMapAIRoutine.ACTION_MOVE:
            final int x = this.bd.getActiveCharacter().getTemplate().getMapAI()
                    .getMoveX();
            final int y = this.bd.getActiveCharacter().getTemplate().getMapAI()
                    .getMoveY();
    ...
    private boolean updatePositionInternal(final int x, final int y,
        ...
        if (next != null && nextGround != null && currGround != null) {
            if (!next.isSolidInBattle()) {
                if (useAP
                        && this.getActiveActionCounter() >= AbstractMazeObject
            ...
            // Confirm Flee
            if (!active.getTemplate().hasAI() || FantastleX.inDebugMode()) {
    ...
    private void waitForAI() {
        this.battleGUI.turnEventHandlersOff();
        FantastleX.getApplication().getMenuManager().disableBattleMenus();
    }
    ...
    private void stopWaitingForAI() {
        ...
        this.battleGUI.turnEventHandlersOn();
        FantastleX.getApplication().getMenuManager().enableBattleMenus();
    }
