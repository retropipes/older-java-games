...
import com.puttysoftware.fantastlex.maze.objects.BarrierGenerator;
import com.puttysoftware.fantastlex.maze.objects.CrackedWall;
import com.puttysoftware.fantastlex.maze.objects.Crevasse;
...
import com.puttysoftware.fantastlex.maze.objects.EnragedBarrierGenerator;
import com.puttysoftware.fantastlex.maze.objects.ForceField;
import com.puttysoftware.fantastlex.maze.objects.HotRock;
import com.puttysoftware.fantastlex.maze.objects.Ice;
import com.puttysoftware.fantastlex.maze.objects.IcedBarrierGenerator;
...
import com.puttysoftware.fantastlex.maze.objects.PoisonedBarrierGenerator;
import com.puttysoftware.fantastlex.maze.objects.ShockedBarrierGenerator;
import com.puttysoftware.fantastlex.maze.objects.Slime;
...
class LayeredTower implements Cloneable {
            ...
            for (v = y - r; v <= y + r; v++) {
                try {
                    ...
                    if (testName.equals("Monster")) {
                        ...
                        FantastleX.getApplication().getBattle()
                                .doBattleByProxy();
                        // Respawn it
                        this.generateOneMonster();
                    }
    ...
    private void fillFloorRandomlyInBattle(final Maze map, final int z,
        ...
        final int columns = this.getColumns();
        final int rows = this.getRows();
        for (e = 0; e < MazeConstants.LAYER_GROUND; e++) {
            final AbstractMazeObject[] objectsWithoutPrerequisites = objects
        ...
        // Pass 3
        for (int layer = 0; layer < MazeConstants.LAYER_GROUND; layer++) {
            final AbstractMazeObject[] requiredObjects = objects
    ...
    public void tickTimers(final int floor) {
        ...
        if (this.timerActive) {
            ...
            if (this.timerValue == 0) {
                ...
                FantastleX.getApplication().getGameManager().solvedLevel();
            }
        }
    }
    ...
    public boolean rotateRadiusClockwise(final int x, final int y, final int z,
        ...
        final AbstractMazeObject[][][] tempStorage = new AbstractMazeObject[2 * r + 1][2 * r + 1][MazeConstants.LAYER_COUNT];
        try {
            for (u = x - r; u <= x + r; u++) {
                for (v = y - r; v <= y + r; v++) {
    ...
    public void radialScanShuffleObjects(final int x, final int y, final int z,
        ...
        final boolean needsFix = px >= x - r && px <= x + r && py >= y - r
                && py <= y + r;
        final AbstractMazeObject[][][] preShuffle = new AbstractMazeObject[2 * r + 1][2 * r + 1][MazeConstants.LAYER_COUNT];
    ...
    public void warpObject(final AbstractMazeObject mo, final int x,
        ...
        randomRow = row.generate();
        randomColumn = column.generate();
        final AbstractMazeObject currObj = this.getCell(randomRow,
                randomColumn, z, MazeConstants.LAYER_OBJECT);
        if (!currObj.isSolid()) {
                ...
                for (z = 0; z < sizeZ; z++) {
                    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
                        this.savedTowerState.setCell(
                                FantastleX.getApplication().getObjects()
