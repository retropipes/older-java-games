...
class EquipmentFactory {
    ...
    static Equipment createEnhancedEquipment(final Equipment oldE,
            final int bonus) {
    ...
    static Equipment createFaithPoweredEquipment(final Equipment oldE,
    ...
    static Socks createSocks(final int actionType, final int price) {
        final String[] names = EquipmentFactory.createSocksNames();
        return new Socks(names[actionType - 1], price, actionType, 1);
    }

    static String[] createOneHandedWeaponNames(final int weaponType) {
    ...
    static String[] createTwoHandedWeaponNames(final int weaponType) {
        final String[] res = new String[WeaponMaterialConstants.MATERIALS_COUNT];
        for (int x = 0; x < res.length; x++) {
            res[x] = WeaponMaterialConstants.MATERIAL_COMMON_NAMES[x] + " "
                    + WeaponConstants.get2HWeapons()[weaponType];
