package com.puttysoftware.fantastlex.resourcemanagers;

public class SoundConstants {
    // Public Sound Constants
    public static final int SOUND_ACTION_FAILED = 0;
    public static final int SOUND_ARROW = 1;
    public static final int SOUND_ARROW_DIE = 2;
    public static final int SOUND_ATTACK = 3;
    public static final int SOUND_BARRIER = 4;
    public static final int SOUND_BATTLE = 5;
    public static final int SOUND_BIND = 6;
    public static final int SOUND_BOLT = 7;
    public static final int SOUND_BUTTON = 8;
    public static final int SOUND_CHAIN = 9;
    public static final int SOUND_CHANGE = 10;
    public static final int SOUND_COLD = 11;
    public static final int SOUND_CONFUSED = 12;
    public static final int SOUND_COUNTER = 13;
    public static final int SOUND_CRACK = 14;
    public static final int SOUND_CREATE = 15;
    public static final int SOUND_CRITICAL = 16;
    public static final int SOUND_DARKNESS = 17;
    public static final int SOUND_DEFENSE = 18;
    public static final int SOUND_DESTROY = 19;
    public static final int SOUND_DIZZY = 20;
    public static final int SOUND_DOWN = 21;
    public static final int SOUND_DRAIN = 22;
    public static final int SOUND_DRUNK = 23;
    public static final int SOUND_EASIER = 24;
    public static final int SOUND_EQUIP = 25;
    public static final int SOUND_EXPLODE = 26;
    public static final int SOUND_FALLING = 27;
    public static final int SOUND_FINISH = 28;
    public static final int SOUND_FIREBALL = 29;
    public static final int SOUND_FOCUS = 30;
    public static final int SOUND_FORCE_FIELD = 31;
    public static final int SOUND_FUMBLE = 32;
    public static final int SOUND_GAME_OVER = 33;
    public static final int SOUND_GENERATE = 34;
    public static final int SOUND_GRAB = 35;
    public static final int SOUND_HARDER = 36;
    public static final int SOUND_HEAL = 37;
    public static final int SOUND_HIGH_SCORE = 38;
    public static final int SOUND_HIT = 39;
    public static final int SOUND_HURT = 40;
    public static final int SOUND_IDENTIFY = 41;
    public static final int SOUND_INTO_PIT = 42;
    public static final int SOUND_LEVEL_UP = 43;
    public static final int SOUND_LIGHT = 44;
    public static final int SOUND_LOADING = 45;
    public static final int SOUND_LOGO = 46;
    public static final int SOUND_LOW_HEALTH = 47;
    public static final int SOUND_MISSED = 48;
    public static final int SOUND_NEXT_ROUND = 49;
    public static final int SOUND_ON_WHO = 50;
    public static final int SOUND_OW = 51;
    public static final int SOUND_PICK_LOCK = 52;
    public static final int SOUND_PLAYER_UP = 53;
    public static final int SOUND_PUSH_PULL = 54;
    public static final int SOUND_SHATTER = 55;
    public static final int SOUND_SHOP = 56;
    public static final int SOUND_SINK_BLOCK = 57;
    public static final int SOUND_SLIME = 58;
    public static final int SOUND_SPECIAL = 59;
    public static final int SOUND_SPELL = 60;
    public static final int SOUND_SPRING = 61;
    public static final int SOUND_SQUISH = 62;
    public static final int SOUND_SUMMON = 63;
    public static final int SOUND_SUN_STONE = 64;
    public static final int SOUND_TELEPORT = 65;
    public static final int SOUND_TRANSACT = 66;
    public static final int SOUND_UNLOCK = 67;
    public static final int SOUND_UP = 68;
    public static final int SOUND_VICTORY = 69;
    public static final int SOUND_WALK = 70;
    public static final int SOUND_WALK_FAILED = 73;
    public static final int SOUND_WALK_ICE = 74;
    public static final int SOUND_WALK_LAVA = 75;
    public static final int SOUND_WALK_SLIME = 76;
    public static final int SOUND_WALK_WATER = 77;
    public static final int SOUND_WALL_TRAP = 78;
    public static final int SOUND_WATER = 79;
    public static final int SOUND_WEAKNESS = 80;
    public static final int SOUND_WIN_GAME = 81;
    // Package-Protected Constants
    static final String[] SOUND_NAMES = { "action_failed", "arrow",
            "arrow_die", "attack", "barrier", "battle", "bind", "bolt",
            "button", "chain", "change", "cold", "confused", "counter",
            "crack", "create", "critical", "darkness", "defense", "destroy",
            "dizzy", "down", "drain", "drunk", "easier", "equip", "explode",
            "falling", "finish", "fireball", "focus", "force_field", "fumble",
            "game_over", "generate", "grab", "harder", "heal", "high_score",
            "hit", "hurt", "identify", "into_pit", "level_up", "light",
            "loading", "logo", "low_health", "missed", "next_round", "on_who",
            "ow", "pick_lock", "player_up", "push_pull", "shatter", "shop",
            "sink_block", "slime", "special", "spell", "spring", "squish",
            "summon", "sun_stone", "teleport", "transact", "unlock", "up",
            "victory", "walk_1", "walk_2", "walk_3", "walk_failed", "walk_ice",
            "walk_lava", "walk_slime", "walk_water", "wall_trap", "water",
            "weakness", "win_game" };

    // Private constructor
    private SoundConstants() {
        // Do nothing
    }
}