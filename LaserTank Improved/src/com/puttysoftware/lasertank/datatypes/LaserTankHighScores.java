...
public class LaserTankHighScores {
    ...
    public static LaserTankHighScores loadFromFile(final File file)
            throws IOException {
    ...
    public static LaserTankHighScores loadFromResource(final String resource)
            throws IOException {
        try (InputStream fs = LaserTankHighScores.class
    ...
    // Internal stuff
    private static LaserTankHighScores loadFromStream(final InputStream fs)
            throws IOException {
        ...
        while (success) {
            try {
                ...
                final int moves = ByteBuffer.wrap(moveData).asShortBuffer()
                        .get();
                // Load shots
                ...
                final int shots = ByteBuffer.wrap(shotData).asShortBuffer()
                        .get();
        ...
        final Integer[] moveLoadTemp = moveTemp
                .toArray(new Integer[moveTemp.size()]);
        final Integer[] shotLoadTemp = shotTemp
                .toArray(new Integer[shotTemp.size()]);
