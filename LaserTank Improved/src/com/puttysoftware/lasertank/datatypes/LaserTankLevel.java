...
public class LaserTankLevel {
    ...
    private static void decodeLVLObjectData(final byte[] rawData,
        ...
        for (int x = 0; x < 16; x++) {
            for (int y = 0; y < 16; y++) {
                ...
                storage.setCell(objID, x, y, floorIndex,
                        LaserTankLevel.ILVL_OBJECT_META_INDEX_OBJECTS);
                // Populate Attribute ID
                storage.setCell(attrID, x, y, floorIndex,
                        LaserTankLevel.ILVL_OBJECT_META_INDEX_ATTRIBUTES);
                // Populate Direction ID
                storage.setCell(dirID, x, y, floorIndex,
                        LaserTankLevel.ILVL_OBJECT_META_INDEX_DIRECTIONS);
                // Populate Index ID
                storage.setCell(indexID, x, y, floorIndex,
                        LaserTankLevel.ILVL_OBJECT_META_INDEX_INDEXES);
                // Populate Frame ID
                storage.setCell(frameID, x, y, floorIndex,
                        LaserTankLevel.ILVL_OBJECT_META_INDEX_FRAMES);
            }
        }
    }

    public static LaserTankLevel loadLVL(final String filename)
            throws IOException {
    ...
    public static LaserTankLevel load(final String filename)
            throws IOException {
        try (GameIOReader gio = new GameIODataReader(filename)) {
    ...
    // Internal stuff
    private static LaserTankLevel loadLVLFromGameIO(final GameIOReader gio)
            throws IOException {
        ...
        while (!gio.atEOF()) {
            ...
            LaserTankLevel.decodeLVLObjectData(
                    gio.readBytes(LaserTankLevel.LVL_OBJECT_DATA_LEN),
    ...
    private static LaserTankLevel loadFromGameIO(final GameIOReader gio)
            throws IOException {
        // Create a level object
        final LaserTankLevel levelData = new LaserTankLevel();
    ...
    public final void addLevel() {
        this.metaData
                .add(new StringStorage(LaserTankLevel.ILVL_METADATA_INDEXES));
        this.difficulty.add(DifficultyConstants.DIFFICULTY_KIDS);
        this.objectData.add(
                new LaserTankLevelStorage(LaserTankLevel.ILVL_OBJECT_DATA_ROWS,
                        LaserTankLevel.ILVL_OBJECT_DATA_COLS,
                        LaserTankLevel.ILVL_OBJECT_DATA_FLOORS,
                        LaserTankLevel.ILVL_OBJECT_DATA_METAS));
        this.levelCount += 1;
    }
    ...
    public final String getAuthor(final int level) {
        return this.metaData.get(level)
                .getCell(LaserTankLevel.ILVL_METADATA_INDEX_AUTHOR);
    }
    ...
    public final String getHint(final int level) {
        return this.metaData.get(level)
                .getCell(LaserTankLevel.ILVL_METADATA_INDEX_HINT);
    }

    public final String getName(final int level) {
        return this.metaData.get(level)
                .getCell(LaserTankLevel.ILVL_METADATA_INDEX_NAME);
    }

    public final int getObjectID(final int row, final int col, final int floor,
            final int level) {
        return this.objectData.get(level).getCell(row, col, floor,
                LaserTankLevel.ILVL_OBJECT_META_INDEX_OBJECTS);
    }

    public final int getAttributeID(final int row, final int col,
            final int floor, final int level) {
        return this.objectData.get(level).getCell(row, col, floor,
                LaserTankLevel.ILVL_OBJECT_META_INDEX_ATTRIBUTES);
    }

    public final int getDirectionID(final int row, final int col,
            final int floor, final int level) {
        return this.objectData.get(level).getCell(row, col, floor,
                LaserTankLevel.ILVL_OBJECT_META_INDEX_DIRECTIONS);
    }

    public final int getIndexID(final int row, final int col, final int floor,
            final int level) {
        return this.objectData.get(level).getCell(row, col, floor,
                LaserTankLevel.ILVL_OBJECT_META_INDEX_INDEXES);
    }

    public final int getFrameID(final int row, final int col, final int floor,
            final int level) {
        return this.objectData.get(level).getCell(row, col, floor,
                LaserTankLevel.ILVL_OBJECT_META_INDEX_FRAMES);
    }
    ...
    public final void setAuthor(final String newValue, final int level) {
        this.metaData.get(level).setCell(newValue,
                LaserTankLevel.ILVL_METADATA_INDEX_AUTHOR);
    }
    ...
    public final void setHint(final String newValue, final int level) {
        this.metaData.get(level).setCell(newValue,
                LaserTankLevel.ILVL_METADATA_INDEX_HINT);
    }

    public final void setName(final String newValue, final int level) {
        this.metaData.get(level).setCell(newValue,
                LaserTankLevel.ILVL_METADATA_INDEX_NAME);
    }

    public final void setObjectID(final int newID, final int row, final int col,
            final int floor, final int level) {
        this.objectData.get(level).setCell(newID, row, col, floor,
                LaserTankLevel.ILVL_OBJECT_META_INDEX_OBJECTS);
    }

    public final void setAttributeID(final int newID, final int row,
            final int col, final int floor, final int level) {
        this.objectData.get(level).setCell(newID, row, col, floor,
                LaserTankLevel.ILVL_OBJECT_META_INDEX_ATTRIBUTES);
    }

    public final void setDirectionID(final int newID, final int row,
            final int col, final int floor, final int level) {
        this.objectData.get(level).setCell(newID, row, col, floor,
                LaserTankLevel.ILVL_OBJECT_META_INDEX_DIRECTIONS);
    }

    public final void setIndexID(final int newID, final int row, final int col,
            final int floor, final int level) {
        this.objectData.get(level).setCell(newID, row, col, floor,
                LaserTankLevel.ILVL_OBJECT_META_INDEX_INDEXES);
    }

    public final void setFrameID(final int newID, final int row, final int col,
            final int floor, final int level) {
        this.objectData.get(level).setCell(newID, row, col, floor,
                LaserTankLevel.ILVL_OBJECT_META_INDEX_FRAMES);
    }
