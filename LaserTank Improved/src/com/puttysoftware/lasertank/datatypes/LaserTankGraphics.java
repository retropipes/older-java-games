...
public class LaserTankGraphics {
    ...
    public static LaserTankGraphics loadFromFile(final File file)
            throws IOException {
    ...
    public static LaserTankGraphics loadFromResource(final String resource)
            throws IOException {
        try (InputStream fs = LaserTankGraphics.class
    ...
    // Internal stuff
    private static LaserTankGraphics loadFromStream(final InputStream fs)
            throws IOException {
        int bytesRead = 0;
        // Load file ID
        final byte[] fileIdData = new byte[LaserTankGraphics.FILE_ID_LEN];
        fs.read(fileIdData);
        final String loadFileId = GameIOUtilities
                .decodeWindowsStringData(fileIdData);
        ...
        final String loadName = GameIOUtilities
                .decodeWindowsStringData(nameData);
        // Load author
        ...
        final String loadAuthor = GameIOUtilities
                .decodeWindowsStringData(authorData);
        // Load info
        ...
        final String loadInfo = GameIOUtilities
                .decodeWindowsStringData(infoData);
        // Load game image
        final BufferedImageIcon loadGame = ImageBMP.readFromStream(fs)
                .convertToGameImage();
        // Load mask image
        final BufferedImageIcon loadMask = ImageBMP.readFromStream(fs)
                .convertToGameImage();
        // Merge game and mask images
        final BufferedImageIcon mergeGraphics = LaserTankGraphics
                .mergeGameAndMask(loadGame, loadMask);
        // Return final result
        return new LaserTankGraphics(loadName, loadAuthor, loadInfo,
                mergeGraphics);
    }

    private static BufferedImageIcon mergeGameAndMask(
            final BufferedImageIcon game, final BufferedImageIcon mask) {
    ...
    private LaserTankGraphics(final String loadName, final String loadAuthor,
            final String loadInfo, final BufferedImageIcon mergeGraphics) {
        this.name = loadName;
        this.info = loadInfo;
