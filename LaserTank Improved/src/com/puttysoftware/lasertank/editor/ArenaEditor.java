...
public class ArenaEditor {
    ...
    private class SwitcherHandler implements ActionListener {
        ...
        public void actionPerformed(final ActionEvent e) {
            try {
                ...
                if (cmd.equals(StringLoader
                        .loadEditor(EditorString.LOWER_GROUND_LAYER))) {
                    ae.changeLayerImpl(ArenaConstants.LAYER_LOWER_GROUND);
                } else if (cmd.equals(StringLoader
                        .loadEditor(EditorString.UPPER_GROUND_LAYER))) {
                    ae.changeLayerImpl(ArenaConstants.LAYER_UPPER_GROUND);
                } else if (cmd.equals(StringLoader
                        .loadEditor(EditorString.LOWER_OBJECTS_LAYER))) {
                    ae.changeLayerImpl(ArenaConstants.LAYER_LOWER_OBJECTS);
                } else if (cmd.equals(StringLoader
                        .loadEditor(EditorString.UPPER_OBJECTS_LAYER))) {
    ...
    public boolean addLevel() {
        ...
        if (success) {
            CommonDialogs.showDialog(
                    StringLoader.loadEditor(EditorString.LEVEL_ADDED));
        } else {
            CommonDialogs.showDialog(
                    StringLoader.loadEditor(EditorString.LEVEL_ADDING_FAILED));
        }
        return success;
    }
    ...
    public void changeLayer() {
        ...
        final String choice = CommonDialogs.showInputDialog(
                StringLoader.loadEditor(EditorString.CHANGE_LAYER_PROMPT),
                StringLoader.loadEditor(EditorString.EDITOR), list,
                list[this.elMgr.getEditorLocationW()]);
        if (choice != null) {
    ...
    void editObjectProperties(final int x, final int y) {
        ...
        try {
            ...
            if (!mo.defersSetProperties()) {
                ...
                if (mo2 == null) {
                    LaserTank.getApplication().showMessage(StringLoader
                            .loadEditor(EditorString.NO_PROPERTIES));
                } else {
                    this.updateUndoHistory(this.savedArenaObject, gridX, gridY,
                            this.elMgr.getEditorLocationZ(),
                            this.elMgr.getEditorLocationW(),
    ...
    public void editPlayerLocation() {
        ...
        LaserTank.getApplication().showMessage(
                StringLoader.loadEditor(EditorString.SET_START_POINT));
    }
    ...
    public void fillLevel() {
        if (this.confirmNonUndoable()) {
            LaserTank.getApplication().getArenaManager().getArena()
                    .fillDefault();
            LaserTank.getApplication().showMessage(
                    StringLoader.loadEditor(EditorString.LEVEL_FILLED));
    ...
    public void goToLevelHandler() {
        ...
        input = CommonDialogs.showInputDialog(
                StringLoader.loadEditor(EditorString.GO_TO_WHICH_LEVEL), msg,
                choices, choices[0]);
        if (input != null) {
    ...
    public boolean newArena() {
        ...
        if (success) {
            this.arenaChanged = true;
            CommonDialogs.showDialog(
                    StringLoader.loadEditor(EditorString.ARENA_CREATED));
        } else {
            CommonDialogs.showDialog(StringLoader
                    .loadEditor(EditorString.ARENA_CREATION_FAILED));
        }
        return success;
    }
    ...
    public void redo() {
        ...
        if (x != -1 && y != -1 && z != -1 && u != -1) {
            ...
            LaserTank.getApplication().showMessage(
                    StringLoader.loadEditor(EditorString.NOTHING_TO_REDO));
        }
    }
    ...
    public void redrawEditor() {
        ...
        app.pack();
        app.setTitle(StringLoader.loadEditor(EditorString.EDITOR_TITLE_1)
                + (z + 1) + StringLoader.loadEditor(EditorString.EDITOR_TITLE_2)
                + (u + 1)
                + StringLoader.loadCommon(CommonString.SPACE_DASH_SPACE)
                + StringLoader.loadTime(e));
    ...
    public boolean removeLevel() {
        ...
        String input;
        input = CommonDialogs.showInputDialog(
                StringLoader.loadEditor(EditorString.WHICH_LEVEL_TO_REMOVE),
                StringLoader.loadEditor(EditorString.REMOVE_LEVEL), choices,
                choices[0]);
        if (input != null) {
    ...
    public boolean resizeLevel() {
        ...
        input3 = CommonDialogs.showTextInputDialogWithDefault(
                StringLoader.loadEditor(EditorString.NUMBER_OF_FLOORS), msg,
                Integer.toString(app.getArenaManager().getArena().getFloors()));
        if (input3 != null) {
            try {
                levelSizeZ = Integer.parseInt(input3);
                if (levelSizeZ < minF) {
                    throw new NumberFormatException(StringLoader
                            .loadEditor(EditorString.FLOORS_TOO_LOW));
                }
                if (levelSizeZ > maxF) {
                    throw new NumberFormatException(StringLoader
                            .loadEditor(EditorString.FLOORS_TOO_HIGH));
                }
    ...
    void setPlayerLocation(final int x, final int y) {
        ...
        try {
            ...
            app.getArenaManager().getArena().setCell(template, destX, destY,
                    this.elMgr.getEditorLocationZ(), template.getLayer());
            LaserTank.getApplication().showMessage(
                    StringLoader.loadEditor(EditorString.START_POINT_SET));
        } catch (final ArrayIndexOutOfBoundsException aioob) {
            try {
                app.getArenaManager().getArena().setStartRow(this.activePlayer,
                        oldY);
            ...
            LaserTank.getApplication().showMessage(
                    StringLoader.loadEditor(EditorString.AIM_WITHIN_THE_ARENA));
        }
        // Swap event handlers
    ...
    public void undo() {
        ...
        if (x != -1 && y != -1 && z != -1 && u != -1) {
            ...
            LaserTank.getApplication().showMessage(
                    StringLoader.loadEditor(EditorString.NOTHING_TO_UNDO));
        }
    }
