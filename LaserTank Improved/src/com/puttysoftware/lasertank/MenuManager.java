...
public class MenuManager {
            ...
            try {
                ...
                if (cmd.equals(StringLoader.loadMenu(MenuString.ITEM_NEW))) {
                    ...
                    app.getArenaManager().setLoaded(loaded);
                        ...
                        .equals(StringLoader.loadMenu(MenuString.ITEM_OPEN))) {
                    loaded = app.getArenaManager().loadArena();
                    app.getArenaManager().setLoaded(loaded);
                } else if (cmd.equals(
                        StringLoader.loadMenu(MenuString.ITEM_OPEN_DEFAULT))) {
                    loaded = app.getArenaManager().loadArenaDefault();
                    app.getArenaManager().setLoaded(loaded);
                } else if (cmd
                        .equals(StringLoader.loadMenu(MenuString.ITEM_CLOSE))) {
                    // Close the window
                    ...
                    app.getGUIManager().showGUI();
                        ...
                        .equals(StringLoader.loadMenu(MenuString.ITEM_SAVE))) {
                    if (app.getArenaManager().getLoaded()) {
                        ...
                        CommonDialogs.showDialog(StringLoader
                                .loadMenu(MenuString.ERROR_NO_ARENA_OPENED));
                    }
                } else if (cmd.equals(
                        StringLoader.loadMenu(MenuString.ITEM_SAVE_AS))) {
                    if (app.getArenaManager().getLoaded()) {
                ...
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_SAVE_AS_PROTECTED))) {
                    if (app.getArenaManager().getLoaded()) {
                        ...
                        StringLoader.loadMenu(MenuString.ITEM_PREFERENCES))) {
                    // Show preferences dialog
                    PreferencesManager.showPrefs();
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_PRINT_GAMEBOARD))) {
                    BoardPrinter.printBoard(app.getMasterContent());
                } else if (cmd
                        .equals(StringLoader.loadMenu(MenuString.ITEM_EXIT))) {
                    // Exit program
                    if (app.getGUIManager().quitHandler()) {
                        System.exit(0);
                    }
                } else if (cmd
                        .equals(StringLoader.loadMenu(MenuString.ITEM_QUIT))) {
                    // Quit program
                    if (app.getGUIManager().quitHandler()) {
                        System.exit(0);
                    }
                } else if (cmd
                        .equals(StringLoader.loadMenu(MenuString.ITEM_PLAY))) {
                    ...
                    if (proceed) {
                        ...
                        .equals(StringLoader.loadMenu(MenuString.ITEM_EDIT))) {
                    // Edit the current arena
                    app.exitCurrentMode();
                    app.getEditor().editArena();
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_USE_CLASSIC_ACCELERATORS))) {
                    // Toggle accelerators
                    app.getMenuManager().toggleAccelerators();
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_RESET_CURRENT_LEVEL))) {
                    final int result = CommonDialogs.showConfirmDialog(
                            StringLoader.loadMenu(
                                    MenuString.CONFIRM_RESET_CURRENT_LEVEL),
                            GlobalLoader.loadUntranslated(
                                    UntranslatedString.PROGRAM_NAME));
                    if (result == JOptionPane.YES_OPTION) {
                        game.abortAndWaitForMLOLoop();
                        game.resetCurrentLevel();
                    }
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_SHOW_SCORE_TABLE))) {
                    game.showScoreTable();
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_REPLAY_SOLUTION))) {
                    game.abortAndWaitForMLOLoop();
                    game.replaySolution();
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_RECORD_SOLUTION))) {
                    game.toggleRecording();
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_LOAD_PLAYBACK_FILE))) {
                    game.abortAndWaitForMLOLoop();
                    LPBManager.loadLPB();
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_PREVIOUS_LEVEL))) {
                    game.abortAndWaitForMLOLoop();
                    game.previousLevel();
                } else if (cmd.equals(
                        StringLoader.loadMenu(MenuString.ITEM_SKIP_LEVEL))) {
                    game.abortAndWaitForMLOLoop();
                    game.solvedLevel(false);
                } else if (cmd.equals(
                        StringLoader.loadMenu(MenuString.ITEM_LOAD_LEVEL))) {
                    game.abortAndWaitForMLOLoop();
                    game.loadLevel();
                } else if (cmd.equals(
                        StringLoader.loadMenu(MenuString.ITEM_SHOW_HINT))) {
                    CommonDialogs.showDialog(
                            app.getArenaManager().getArena().getHint().trim());
                } else if (cmd.equals(
                        StringLoader.loadMenu(MenuString.ITEM_CHEATS))) {
                    game.enterCheatCode();
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_CHANGE_OTHER_AMMO))) {
                    game.changeOtherAmmoMode();
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_CHANGE_OTHER_TOOL))) {
                    game.changeOtherToolMode();
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_CHANGE_OTHER_RANGE))) {
                    game.changeOtherRangeMode();
                } else if (cmd.equals(StringLoader
                        .loadTime(ArenaConstants.ERA_DISTANT_PAST))) {
                    ...
                    menu.gameEraDistantFuture.setSelected(false);
                        ...
                        .loadTime(ArenaConstants.ERA_DISTANT_FUTURE))) {
                    ...
                    menu.gameEraDistantFuture.setSelected(true);
                        ...
                        .equals(StringLoader.loadMenu(MenuString.ITEM_UNDO))) {
                    ...
                    if (app.isInEditorMode()) {
                        ...
                        .equals(StringLoader.loadMenu(MenuString.ITEM_REDO))) {
                    ...
                    if (app.isInEditorMode()) {
                        ...
                        StringLoader.loadMenu(MenuString.ITEM_CUT_LEVEL))) {
                    // Cut Level
                    ...
                    editor.updateEditorLevelAbsolute(level);
                        ...
                        StringLoader.loadMenu(MenuString.ITEM_COPY_LEVEL))) {
                    // Copy Level
                    app.getArenaManager().getArena().copyLevel();
                } else if (cmd.equals(
                        StringLoader.loadMenu(MenuString.ITEM_PASTE_LEVEL))) {
                    // Paste Level
                    app.getArenaManager().getArena().pasteLevel();
                    editor.fixLimits();
                    editor.redrawEditor();
                } else if (cmd.equals(StringLoader.loadMenu(
                        MenuString.ITEM_INSERT_LEVEL_FROM_CLIPBOARD))) {
                    // Insert Level From Clipboard
                    app.getArenaManager().getArena().insertLevelFromClipboard();
                    editor.fixLimits();
                } else if (cmd.equals(
                        StringLoader.loadMenu(MenuString.ITEM_CLEAR_HISTORY))) {
                    // Clear undo/redo history, confirm first
                    final int res = CommonDialogs.showConfirmDialog(
                            StringLoader
                                    .loadMenu(MenuString.CONFIRM_CLEAR_HISTORY),
                            StringLoader.loadEditor(EditorString.EDITOR));
                    if (res == JOptionPane.YES_OPTION) {
                        editor.clearHistory();
                    }
                } else if (cmd.equals(
                        StringLoader.loadMenu(MenuString.ITEM_GO_TO_LEVEL))) {
                    // Go To Level
                    editor.goToLevelHandler();
                } else if (cmd.equals(
                        StringLoader.loadMenu(MenuString.ITEM_UP_ONE_FLOOR))) {
                    // Go up one floor
                    editor.updateEditorPosition(1, 0);
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_DOWN_ONE_FLOOR))) {
                    // Go down one floor
                    editor.updateEditorPosition(-1, 0);
                } else if (cmd.equals(
                        StringLoader.loadMenu(MenuString.ITEM_UP_ONE_LEVEL))) {
                    // Go up one level
                    editor.updateEditorPosition(0, 1);
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_DOWN_ONE_LEVEL))) {
                    // Go down one level
                    editor.updateEditorPosition(0, -1);
                } else if (cmd.equals(
                        StringLoader.loadMenu(MenuString.ITEM_ADD_A_LEVEL))) {
                    // Add a level
                    editor.addLevel();
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_REMOVE_A_LEVEL))) {
                    // Remove a level
                    editor.removeLevel();
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_FILL_CURRENT_LEVEL))) {
                    // Fill level
                    editor.fillLevel();
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_RESIZE_CURRENT_LEVEL))) {
                    // Resize level
                    editor.resizeLevel();
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_LEVEL_PREFERENCES))) {
                    // Set Level Preferences
                    editor.setLevelPrefs();
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_SET_START_POINT))) {
                    // Set Start Point
                    editor.editPlayerLocation();
                } else if (cmd.equals(
                        StringLoader.loadMenu(MenuString.ITEM_CHANGE_LAYER))) {
                    // Change Layer
                    editor.changeLayer();
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_ENABLE_GLOBAL_MOVE_SHOOT))) {
                    // Enable Global Move-Shoot
                    LaserTank.getApplication().getArenaManager().getArena()
                            .setMoveShootAllowedGlobally(true);
                    menu.editorGlobalMoveShoot.setText(StringLoader.loadMenu(
                            MenuString.ITEM_DISABLE_GLOBAL_MOVE_SHOOT));
                } else if (cmd.equals(StringLoader
                        .loadMenu(MenuString.ITEM_DISABLE_GLOBAL_MOVE_SHOOT))) {
                    // Disable Global Move-Shoot
                    menu.editorGlobalMoveShoot.setText(StringLoader.loadMenu(
                            MenuString.ITEM_ENABLE_GLOBAL_MOVE_SHOOT));
                    app.getArenaManager().getArena()
                    ...
                    menu.editorEraDistantFuture.setSelected(true);
                        ...
                        .loadMenu(MenuString.ITEM_ABOUT_LASERTANK))) {
    ...
    private JMenuItem editorUndo, editorRedo, editorCutLevel, editorCopyLevel,
            ...
            editorChangeLayer, editorGlobalMoveShoot;
    ...
    public final void updateMenuItemState() {
        try {
            ...
            if (app.isInEditorMode()) {
                ...
                try {
                    if (elMgr.getEditorLocationZ() == elMgr
                ...
                try {
                    if (elMgr.getEditorLocationU() == elMgr
    ...
    private JMenu buildFileMenu(final MenuHandler mhandler) {
        final JMenu fileMenu = new JMenu(
                StringLoader.loadMenu(MenuString.MENU_FILE));
        this.fileNew = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_NEW));
        this.fileOpen = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_OPEN));
        this.fileOpenDefault = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_OPEN_DEFAULT));
        this.fileClose = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_CLOSE));
        this.fileSave = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_SAVE));
        this.fileSaveAs = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_SAVE_AS));
        this.fileSaveAsProtected = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_SAVE_AS_PROTECTED));
        this.filePreferences = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_PREFERENCES));
        this.filePrint = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_PRINT_GAMEBOARD));
        if (System
                .getProperty(GlobalLoader
                        .loadUntranslated(UntranslatedString.OS_NAME))
                .contains(GlobalLoader
                        .loadUntranslated(UntranslatedString.WINDOWS))) {
            this.fileExit = new JMenuItem(
                    StringLoader.loadMenu(MenuString.ITEM_EXIT));
        } else {
            this.fileExit = new JMenuItem(
                    StringLoader.loadMenu(MenuString.ITEM_QUIT));
        }
        ...
        if (!System
                .getProperty(GlobalLoader
                        .loadUntranslated(UntranslatedString.OS_NAME))
                .equalsIgnoreCase(GlobalLoader
                        .loadUntranslated(UntranslatedString.MAC_OS_X))) {
    ...
    private JMenu buildPlayMenu(final MenuHandler mhandler) {
        final JMenu playMenu = new JMenu(
                StringLoader.loadMenu(MenuString.MENU_PLAY));
        this.playPlay = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_PLAY));
        this.playEdit = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_EDIT));
        this.playToggleAccelerators = new JCheckBoxMenuItem(StringLoader
                .loadMenu(MenuString.ITEM_USE_CLASSIC_ACCELERATORS));
    ...
    private JMenu buildGameMenu(final MenuHandler mhandler) {
        final JMenu gameMenu = new JMenu(
                StringLoader.loadMenu(MenuString.MENU_GAME));
        this.gameReset = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_RESET_CURRENT_LEVEL));
        this.gameShowTable = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_SHOW_SCORE_TABLE));
        this.gameReplaySolution = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_REPLAY_SOLUTION));
        this.gameRecordSolution = new JCheckBoxMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_RECORD_SOLUTION));
        this.gameLoadLPB = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_LOAD_PLAYBACK_FILE));
        this.gamePreviousLevel = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_PREVIOUS_LEVEL));
        ...
        this.gameChangeOtherAmmoMode = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_CHANGE_OTHER_AMMO));
        this.gameChangeOtherToolMode = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_CHANGE_OTHER_TOOL));
        this.gameChangeOtherRangeMode = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_CHANGE_OTHER_RANGE));
        this.gameEraDistantPast = new JCheckBoxMenuItem(
                StringLoader.loadTime(ArenaConstants.ERA_DISTANT_PAST), false);
        this.gameEraPast = new JCheckBoxMenuItem(
                StringLoader.loadTime(ArenaConstants.ERA_PAST), false);
        this.gameEraPresent = new JCheckBoxMenuItem(
                StringLoader.loadTime(ArenaConstants.ERA_PRESENT), true);
        this.gameEraFuture = new JCheckBoxMenuItem(
                StringLoader.loadTime(ArenaConstants.ERA_FUTURE), false);
        ...
        this.gameTimeTravelSubMenu = new JMenu(
                StringLoader.loadMenu(MenuString.SUB_TIME_TRAVEL));
    ...
    private JMenu buildEditorMenu(final MenuHandler mhandler) {
        final JMenu editorMenu = new JMenu(
                StringLoader.loadMenu(MenuString.MENU_EDITOR));
        this.editorUndo = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_UNDO));
        this.editorRedo = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_REDO));
        ...
        this.editorPasteLevel = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_PASTE_LEVEL));
        this.editorInsertLevelFromClipboard = new JMenuItem(StringLoader
                .loadMenu(MenuString.ITEM_INSERT_LEVEL_FROM_CLIPBOARD));
        ...
        this.editorDownOneFloor = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_DOWN_ONE_FLOOR));
        this.editorUpOneLevel = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_UP_ONE_LEVEL));
        this.editorDownOneLevel = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_DOWN_ONE_LEVEL));
        this.editorAddLevel = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_ADD_A_LEVEL));
        this.editorRemoveLevel = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_REMOVE_A_LEVEL));
        this.editorFillLevel = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_FILL_CURRENT_LEVEL));
        this.editorResizeLevel = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_RESIZE_CURRENT_LEVEL));
        this.editorLevelPreferences = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_LEVEL_PREFERENCES));
        this.editorSetStartPoint = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_SET_START_POINT));
        this.editorChangeLayer = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_CHANGE_LAYER));
        this.editorGlobalMoveShoot = new JMenuItem(StringLoader
                .loadMenu(MenuString.ITEM_ENABLE_GLOBAL_MOVE_SHOOT));
    ...
    private JMenu buildHelpMenu(final MenuHandler mhandler) {
        final JMenu helpMenu = new JMenu(
                StringLoader.loadMenu(MenuString.MENU_HELP));
        this.helpAbout = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_ABOUT_LASERTANK));
        this.helpHelp = new JMenuItem(
                StringLoader.loadMenu(MenuString.ITEM_LASERTANK_HELP));
    ...
    private void attachAccelerators() {
        ...
        this.editorInsertLevelFromClipboard
                .setAccelerator(this.accel.editorInsertLevelFromClipboardAccel);
        this.editorClearHistory
                .setAccelerator(this.accel.editorClearHistoryAccel);
        ...
        this.editorDownOneLevel
                .setAccelerator(this.accel.editorDownOneLevelAccel);
    }
