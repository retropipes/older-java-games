...
class PreferencesGUIManager {
    ...
    private void setUpGUI() {
        ...
        this.sounds = new JCheckBox(
                StringLoader.loadPref(PrefString.ENABLE_SOUNDS), true);
        this.music = new JCheckBox(
                StringLoader.loadPref(PrefString.ENABLE_MUSIC), true);
        this.checkUpdatesStartup = new JCheckBox(
                StringLoader.loadPref(PrefString.STARTUP_UPDATES), true);
        this.moveOneAtATime = new JCheckBox(
                StringLoader.loadPref(PrefString.ONE_MOVE), true);
        this.enableAnimation = new JCheckBox(
                StringLoader.loadPref(PrefString.ENABLE_ANIMATION), true);
        this.actionDelay = new JComboBox<>(
                new String[] { StringLoader.loadPref(PrefString.SPEED_1),
                        StringLoader.loadPref(PrefString.SPEED_2),
                        StringLoader.loadPref(PrefString.SPEED_3),
                        StringLoader.loadPref(PrefString.SPEED_4),
                        StringLoader.loadPref(PrefString.SPEED_5) });
        ...
        this.editorShowAllObjects = new JCheckBox(
                StringLoader.loadPref(PrefString.SHOW_ALL_OBJECTS), true);
        ...
        settingsPane
                .add(new JLabel(StringLoader.loadPref(PrefString.SPEED_LABEL)));
        settingsPane.add(this.actionDelay);
        settingsPane.add(new JLabel(
                StringLoader.loadPref(PrefString.ACTIVE_LANGUAGE_LABEL)));
        settingsPane.add(this.languageList);
        settingsPane.add(new JLabel(
                StringLoader.loadPref(PrefString.EDITOR_LAYOUT_LABEL)));
