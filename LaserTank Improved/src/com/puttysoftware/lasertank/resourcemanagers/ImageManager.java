...
public class ImageManager {
    ...
    static BufferedImageIcon getUncachedImage(final AbstractArenaObject obj,
            final boolean useText) {
        try {
            final String name = obj.getImageName();
            final String normalName = ImageManager.normalizeName(name);
            final URL url = ImageManager.LOAD_CLASS.getResource(GlobalLoader
                    .loadUntranslated(UntranslatedString.OBJECTS_PATH)
                    + normalName + GlobalLoader.loadUntranslated(
                            UntranslatedString.IMAGE_FORMAT_PNG));
                ...
                if (ImageManager.DRAW_FONT == null) {
                    try (InputStream is = ImageManager.class
                            .getResourceAsStream(GlobalLoader.loadUntranslated(
                                    UntranslatedString.FONT_PATH)
                                    + GlobalLoader.loadUntranslated(
                                            UntranslatedString.FONT_FILENAME))) {
