...
public abstract class AbstractTunnel extends AbstractArenaObject {
    ...
    private static void checkTunnelsOfColor(final int color) {
        ...
        final int tx = app.getGameManager().getPlayerManager()
                .getPlayerLocationX();
        final int ty = app.getGameManager().getPlayerManager()
                .getPlayerLocationY();
        final int[] pgrmdest = app.getArenaManager().getArena()
                .circularScanTunnel(0, 0, 0, AbstractTunnel.SCAN_RADIUS, tx, ty,
                        AbstractTunnel.getTunnelOfColor(color), false);
        if (pgrmdest != null) {
    ...
    public void postMoveAction(final int dirX, final int dirY, final int dirZ) {
        ...
        final int ty = app.getGameManager().getPlayerManager()
                .getPlayerLocationY();
        final int[] pgrmdest = app.getArenaManager().getArena()
                .circularScanTunnel(dirX, dirY, dirZ,
                        AbstractTunnel.SCAN_RADIUS, tx, ty,
                        AbstractTunnel.getTunnelOfColor(this.getColor()), true);
        if (pgrmdest != null) {
    ...
    public boolean pushIntoAction(final AbstractMovableObject pushed,
        ...
        final int[] pgrmdest = app.getArenaManager().getArena()
                .circularScanTunnel(x, y, z, AbstractTunnel.SCAN_RADIUS, tx, ty,
                        AbstractTunnel.getTunnelOfColor(this.getColor()),
                        false);
        if (pgrmdest != null) {
            AbstractTunnel.tunnelsFull[color] = false;
