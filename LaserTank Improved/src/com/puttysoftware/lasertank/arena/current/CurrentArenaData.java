...
public final class CurrentArenaData extends AbstractArenaData {
					...
					for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
						try {
							...
							if (obj.isOfType(TypeConstants.TYPE_ANTI)) {
								...
								final Direction invert = DirectionResolver.resolveRelativeInvert(unres[0], unres[1]);
								if (d == invert) {
									this.foundX = xFix;
									this.foundY = u;
									return true;
	...
	public void updateRedoHistory(final HistoryStatus whatWas) {
		this.iue.updateRedoHistory((LowLevelArenaDataStore) this.data, whatWas);
	}

	@Override
	public void updateUndoHistory(final HistoryStatus whatWas) {
		this.iue.updateUndoHistory((LowLevelArenaDataStore) this.data, whatWas);
	}
