...
public final class CurrentArenaData extends AbstractArenaData {
    ...
    private static CurrentArenaData readDataG1(final AbstractArena arena,
            final XMLFileReader reader, final int ver) throws IOException {
    ...
    public CurrentArenaData() {
        ...
        this.fillVirtual();
        this.dirtyData = new FlagStorage(AbstractArenaData.MIN_COLUMNS,
                AbstractArenaData.MIN_ROWS, AbstractArenaData.MIN_FLOORS);
    ...
    public int[] findObject(final AbstractArena arena, final int z,
            final AbstractArenaObject target) {
        // Perform the scan
                        ...
                        try {
                            ...
                            if (obj.isOfType(TypeConstants.TYPE_ANTI)) {
                                final int[] unres = DirectionResolver
                                        .unresolveRelative(obj.getDirection());
                                final Direction invert = DirectionResolver
                                        .resolveRelativeInvert(unres[0],
                                                unres[1]);
                                if (d == invert) {
                                    this.foundX = xFix;
                                    this.foundY = u;
    ...
    public AbstractArenaData readData(final AbstractArena arena,
        ...
        if (FormatConstants.isFormatVersionValidGeneration1(formatVersion)) {
            ...
            throw new IOException(
                    StringLoader.loadError(ErrorString.UNKNOWN_ARENA_FORMAT));
        }
    }

    @Override
    public void readSavedState(final XMLFileReader reader,
    ...
    private void readSavedStateG2(final XMLFileReader reader,
            final int formatVersion) throws IOException {
    ...
    private void readSavedStateG3(final XMLFileReader reader,
            final int formatVersion) throws IOException {
        int y, x, z, saveSizeX, saveSizeY, saveSizeZ;
        saveSizeX = reader.readInt();
        saveSizeY = reader.readInt();
    ...
    private void readSavedStateG4(final XMLFileReader reader,
            final int formatVersion) throws IOException {
        int y, x, z, saveSizeX, saveSizeY, saveSizeZ;
        saveSizeX = reader.readInt();
        saveSizeY = reader.readInt();
    ...
    private void readSavedStateG5(final XMLFileReader reader,
            final int formatVersion) throws IOException {
    ...
    private void readSavedStateG6(final XMLFileReader reader,
            final int formatVersion) throws IOException {
        int y, x, z, w, saveSizeX, saveSizeY, saveSizeZ;
        saveSizeX = reader.readInt();
        saveSizeY = reader.readInt();
    ...
    @Override
    public void writeData(final AbstractArena arena, final XMLFileWriter writer)
            throws IOException {
        int y, x, z, w;
