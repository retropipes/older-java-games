...
public class ArenaManager {
    ...
    private static void loadFile(final String filename,
        ...
        if (!FilenameChecker.isFilenameOK(ArenaManager.getNameWithoutExtension(
            ...
            CommonDialogs.showErrorDialog(
                    StringLoader.loadDialog(DialogString.ILLEGAL_CHARACTERS),
    ...
    private static void saveFile(final String filename,
        ...
        if (isSavedGame) {
            LaserTank.getApplication().showMessage(
                    StringLoader.loadMessage(MessageString.SAVING_GAME));
        } else {
            LaserTank.getApplication().showMessage(
                    StringLoader.loadMessage(MessageString.SAVING_ARENA));
        }
    ...
    private boolean loadArenaImpl(final String initialDirectory) {
        ...
        final FileDialog fd = new FileDialog((JFrame) null,
                StringLoader.loadDialog(DialogString.LOAD), FileDialog.LOAD);
            ...
            if (file != null && dir != null) {
                ...
                if (extension.equals(Extension.getArenaExtension())) {
                    ...
                    CommonDialogs.showDialog(StringLoader
                            .loadDialog(DialogString.NON_ARENA_FILE));
                }
            } else {
                // User cancelled
                if (this.loaded) {
    ...
    public boolean saveArena(final boolean protect) {
        ...
        if (app.isInGameMode()) {
            if (this.lastUsedGameFile != null && !this.lastUsedGameFile
                    .equals(StringLoader.loadCommon(CommonString.EMPTY))) {
    ...
    public boolean saveArenaAs(final boolean protect) {
        ...
        String fileOnly = GlobalLoader
                .loadUntranslated(UntranslatedString.DOUBLE_BACKSLASH);
        String extension, file, dir;
        final String lastSave = PreferencesManager.getLastDirSave();
        final FileDialog fd = new FileDialog((JFrame) null,
                StringLoader.loadDialog(DialogString.SAVE), FileDialog.SAVE);
