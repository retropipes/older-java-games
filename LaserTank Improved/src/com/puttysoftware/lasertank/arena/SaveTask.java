...
public class SaveTask extends Thread {
    ...
    public SaveTask(final String file, final boolean saved,
        ...
        this.setName(GlobalLoader
                .loadUntranslated(UntranslatedString.NEW_AG_SAVER_NAME));
    }
    ...
    public void run() {
        ...
        try {
            ...
            if (this.isSavedGame) {
                CommonDialogs.showDialog(StringLoader
                        .loadDialog(DialogString.GAME_SAVING_FAILED));
            } else {
                CommonDialogs.showDialog(StringLoader
                        .loadDialog(DialogString.ARENA_SAVING_FAILED));
            }
            success = false;
        ...
        if (this.isSavedGame) {
            LaserTank.getApplication().showMessage(
                    StringLoader.loadMessage(MessageString.GAME_SAVED));
        } else {
            LaserTank.getApplication().showMessage(
                    StringLoader.loadMessage(MessageString.ARENA_SAVED));
        }
