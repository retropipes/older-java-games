...
public class SortedScoreTable extends ScoreTable {
    ...
    public boolean check(final Score newEntry) {
        ...
        if (this.sortOrder == SortOrder.DESCENDING) {
            ...
            return !Collections.min(temp, new Score.DescendingSorter())
                    .equals(newEntry);
        } else {
            // Copy the current table to the temporary table
            Collections.copy(temp, this.table);
            ...
            return !Collections.max(temp, new Score.AscendingSorter())
                    .equals(newEntry);
        }
    }
    ...
    public void save(final GameIOWriter gio) throws IOException {
        ...
        for (final Score sc : this.table) {
            sc.save(gio);
        }
    }

    public static SortedScoreTable load(final GameIOReader gio)
            throws IOException {
        final int len = gio.readInt();
        final SortOrder so = SortOrder.valueOf(gio.readString());
        final SortedScoreTable sst = new SortedScoreTable(len, so);
        for (int x = 0; x < len; x++) {
            final Score sc = Score.load(gio);
            sst.add(sc);
        }
        return sst;
    }
