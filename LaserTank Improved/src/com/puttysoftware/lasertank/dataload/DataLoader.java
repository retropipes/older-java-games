...
public class DataLoader {
    ...
    private static void cacheFile(final DataFile file) {
        ...
        try (final InputStream is = DataLoader.LOAD_CLASS
                .getResourceAsStream(DataLoader.LOAD_PATH + filename
                        + Extension.getStringsExtensionWithPeriod())) {
            DataLoader.CACHE.get(fileID).load(is);
        } catch (final IOException ioe) {
            CommonDialogs.showErrorDialog(
                    "Something has gone horribly wrong trying to cache object data!",
                    "FATAL ERROR");
    ...
    private static void cacheSolidFile(final SolidDataFile file) {
        ...
        try (final InputStream is = DataLoader.LOAD_CLASS
            ...
            CommonDialogs.showErrorDialog(
                    "Something has gone horribly wrong trying to cache object solidity data!",
                    "FATAL ERROR");
            LaserTank.logErrorDirectly(ioe);
        }
    }
    ...
    public static boolean loadSolid(final int objID) {
        final SolidDataFile sdf = SolidDataFileNames
                .getFile(DataLoader.getFromCache(DataFile.SOLID)
                        .getProperty(Integer.toString(objID)));
        return Boolean.parseBoolean(DataLoader.getFromSolidCache(sdf)
                .getProperty(Integer.toString(objID)));
    }

    public static boolean loadAttributeSolid(final int attrID) {
        return Boolean
                .parseBoolean(DataLoader.getFromCache(DataFile.SOLID_ATTRIBUTE)
                        .getProperty(Integer.toString(attrID)));
    }

    public static int[] loadDirection(final int objID) {
        final String rawList = DataLoader.getFromCache(DataFile.DIRECTION)
                .getProperty(Integer.toString(objID));
        final String[] rawSplit = rawList.split(DataLoader.LIST_SPLIT);
        final int[] data = new int[rawSplit.length];
    ...
    public static int loadFrame(final int objID) {
        return Integer.parseInt(DataLoader.getFromCache(DataFile.FRAME)
                .getProperty(Integer.toString(objID)));
    }

    public static boolean loadFriction(final int objID) {
        return Boolean.parseBoolean(DataLoader.getFromCache(DataFile.FRICTION)
                .getProperty(Integer.toString(objID)));
    }

    public static int[] loadIndex(final int objID) {
        final String rawList = DataLoader.getFromCache(DataFile.INDEX)
                .getProperty(Integer.toString(objID));
        final String[] rawSplit = rawList.split(DataLoader.LIST_SPLIT);
        final int[] data = new int[rawSplit.length];
        for (int x = 0; x < data.length; x++) {
    ...
    public static boolean loadMovable(final int objID) {
        return Boolean.parseBoolean(DataLoader.getFromCache(DataFile.MOVABLE)
                .getProperty(Integer.toString(objID)));
    }

    public static boolean loadShoot(final int objID) {
        return Boolean.parseBoolean(DataLoader.getFromCache(DataFile.SHOOT)
                .getProperty(Integer.toString(objID)));
    }

    public static int loadWeight(final int objID) {
        return Integer.parseInt(DataLoader.getFromCache(DataFile.WEIGHT)
                .getProperty(Integer.toString(objID)));
    }

    public static int loadTransformFire(final int objID) {
        return Integer.parseInt(DataLoader.getFromCache(DataFile.TRANSFORM_FIRE)
                .getProperty(Integer.toString(objID)));
    }

    public static int loadTransformIce(final int objID) {
        return Integer.parseInt(DataLoader.getFromCache(DataFile.TRANSFORM_ICE)
                .getProperty(Integer.toString(objID)));
    }

    public static int loadTransformStone(final int objID) {
        return Integer
                .parseInt(DataLoader.getFromCache(DataFile.TRANSFORM_STONE)
                        .getProperty(Integer.toString(objID)));
    }

    public static int loadMaterial(final int objID) {
        return Integer.parseInt(DataLoader.getFromCache(DataFile.MATERIAL)
                .getProperty(Integer.toString(objID)));
    }

    public static int loadHeight(final int objID) {
        return Integer.parseInt(DataLoader.getFromCache(DataFile.HEIGHT)
                .getProperty(Integer.toString(objID)));
    }

    public static int loadLayer(final int objID) {
        return Integer.parseInt(DataLoader.getFromCache(DataFile.LAYER)
                .getProperty(Integer.toString(objID)));
    }

    public static boolean loadLethal(final int objID) {
        return Boolean.parseBoolean(DataLoader.getFromCache(DataFile.LETHAL)
                .getProperty(Integer.toString(objID)));
    }

    public static boolean loadReflectEast(final int objID) {
        return Boolean
                .parseBoolean(DataLoader.getFromCache(DataFile.REFLECT_EAST)
                        .getProperty(Integer.toString(objID)));
    }

    public static boolean loadReflectNorth(final int objID) {
        return Boolean
                .parseBoolean(DataLoader.getFromCache(DataFile.REFLECT_NORTH)
                        .getProperty(Integer.toString(objID)));
    }

    public static boolean loadReflectSouth(final int objID) {
        return Boolean
                .parseBoolean(DataLoader.getFromCache(DataFile.REFLECT_SOUTH)
                        .getProperty(Integer.toString(objID)));
    }

    public static boolean loadReflectWest(final int objID) {
        return Boolean
                .parseBoolean(DataLoader.getFromCache(DataFile.REFLECT_WEST)
                        .getProperty(Integer.toString(objID)));
    }
}
