...
public class GlobalLoader {
    ...
    private static void cacheFile(final GlobalFile file) {
        ...
        try (final InputStream is = GlobalLoader.LOAD_CLASS
                .getResourceAsStream(GlobalLoader.LOAD_PATH + filename
    ...
    public static String loadLanguage(final int strID) {
        return GlobalLoader.getFromCache(GlobalFile.LANGUAGES)
                .getProperty(Integer.toString(strID));
    }

    public static String loadUntranslated(final UntranslatedString str) {
        return GlobalLoader.getFromCache(GlobalFile.NOT_TRANSLATED)
                .getProperty(Integer.toString(str.ordinal()));
    }

    public static String loadImage(final int imageID) {
        return GlobalLoader.getFromCache(GlobalFile.IMAGES)
                .getProperty(Integer.toString(imageID));
    }

    public static String loadFrame(final int frameID) {
        return GlobalLoader.getFromCache(GlobalFile.FRAMES)
                .getProperty(Integer.toString(frameID));
    }

    public static String loadDirection(final int dir) {
        return GlobalLoader.getFromCache(GlobalFile.DIRECTIONS)
                .getProperty(Integer.toString(dir));
    }

    public static String loadIndex(final int index) {
        return GlobalLoader.getFromCache(GlobalFile.LANGUAGES)
                .getProperty(Integer.toString(index));
    }
}
