...
public class GameIOUtilities {
    public static class ByteOrder {
        ...
        public static short reverse(final short valueToReverse) {
            final byte[] valueAsBytes = new byte[] {
                    (byte) (valueToReverse & 0xFF),
                    (byte) (valueToReverse >> 8 & 0xFF), };
            return (short) ((valueAsBytes[1] & 0xFF)
    ...
    public static class DataInputStreamLittleEndian implements AutoCloseable {
        ...
        public DataInputStreamLittleEndian(
                final DataInputStream newSystemStream) {
        ...
        public String readString(final int numberOfCharacters)
                throws IOException {
    ...
    public static class ImageBMP {
        public static abstract class DIBHeader {
            ...
            public static DIBHeader buildFromStream(
                    final DataInputStreamLittleEndian reader) {
                DIBHeader returnValue = null;
                try {
                    final int dibHeaderSizeInBytes = reader.readInt();
                    // hack
                    if (dibHeaderSizeInBytes == 40) {
                        returnValue = new DIBHeaderBitmapInfo()
            ...
            public abstract DIBHeader readFromStream(
        ...
        public static class DIBHeaderBitmapInfo extends DIBHeader {
            ...
            public DIBHeaderBitmapInfo(final int newSizeInBytes,
                    final Coords newImageSizeInPixels, final short newPlanes,
                    final short newBitsPerPixel, final int newCompression,
                    final int newImageSizeInBytes,
                    final Coords newPixelsPerMeter,
                    final int newNumberOfColorsInPalette,
                ...
                if (this.imageSizeInBytes == 0) {
                    this.imageSizeInBytes = this.imageSizeInPixels.x
                            * this.imageSizeInPixels.y * this.bitsPerPixel / 8;
                }
            }
            ...
            public String toString() {
                final String returnValue = "<DIBHeader " + "size='"
                        + this.sizeInBytes + "' " + "imageSizeInPixels='"
                        + this.imageSizeInPixels.x + ","
                        + this.imageSizeInPixels.y + "' " + "planes='"
                        + this.planes + "' " + "bitsPerPixel='"
                        + this.bitsPerPixel + "' " + "compression='"
                        + this.compression + "' " + "imageSizeInBytes='"
                        + this.imageSizeInBytes + "' " + "pixelsPerMeter='"
                        + this.pixelsPerMeter.x + "," + this.pixelsPerMeter.y
                        + "' " + "numberOfColorsInPalette='"
                        + this.numberOfColorsInPalette + "' "
        ...
        public static class FileHeader {
            public static FileHeader readFromStream(
                    final DataInputStreamLittleEndian reader) {
            ...
            public FileHeader(final String newSignature, final int newFileSize,
                    final short newReserved1, final short newReserved2,
                    final int newFileOffsetToPixelArray) {
            ...
            @Override
            public String toString() {
                final String returnValue = "<FileHeader " + "signature='"
                        + this.signature + "' " + "fileSize='" + this.fileSize
                        + "' " + "fileOffsetToPixelArray ='"
                        + this.fileOffsetToPixelArray + "' " + "/>";
                return returnValue;
            }
        }

        public static ImageBMP readFromStream(final InputStream is)
                throws IOException {
            ImageBMP returnValue = null;
            try (DataInputStreamLittleEndian reader = new DataInputStreamLittleEndian(
                ...
                final int numberOfBytesInPixelData = dibHeader
                        .imageSizeInBytes();
                final byte[] pixelData = new byte[numberOfBytesInPixelData];
                reader.read(pixelData);
                returnValue = new ImageBMP(fileHeader, dibHeader, colorTable,
                        pixelData);
            }
            return returnValue;
        }
        ...
        public ImageBMP(final FileHeader newFileHeader,
                final DIBHeader newDibHeader, final int[] newColorTable,
        ...
        public BufferedImageIcon convertToGameImage() {
            ...
            returnValue = new java.awt.image.BufferedImage(imageSizeInPixels.x,
                    imageSizeInPixels.y,
            ...
            for (int y = 0; y < imageSizeInPixels.y; y++) {
                for (int x = 0; x < imageSizeInPixels.x; x++) {
                    ...
                    for (int b = 0; b < bytesPerPixel; b++) {
                        pixelColorArgb += (this.pixelData[byteOffsetForPixel
                                + b] & 0xFF) << 8 * b;
                    }
    ...
    public static String decodeWindowsStringData(final byte[] data) {
        return Charset.forName("ISO-8859-1").decode(ByteBuffer.wrap(data))
                .toString();
    }

    public static long readUnsignedInt(final DataInput data)
            throws IOException {
