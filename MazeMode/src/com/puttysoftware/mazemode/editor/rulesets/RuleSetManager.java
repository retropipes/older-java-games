/*  MazeMode: A RuleSet-Solving Game
...
import com.puttysoftware.mazemode.maze.FilenameChecker;
...
public class RuleSetManager {
    ...
    public static boolean exportRuleSet() {
        ...
        String extension;
        final FileDialog fc = new FileDialog(app.getOutputFrame(),
                "Save Rule Set", FileDialog.SAVE);
        while (!FilenameChecker.isFilenameOK(fileOnly)) {
            fc.setVisible(true);
            if (fc.getDirectory() != null && fc.getFile() != null) {
                final File file = new File(fc.getDirectory() + fc.getFile());
                extension = RuleSetManager.getExtension(file);
                filename = file.getAbsolutePath();
