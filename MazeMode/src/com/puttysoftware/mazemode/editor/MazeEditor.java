...
import com.puttysoftware.mazemode.objects.FinishTo;
import com.puttysoftware.mazemode.objects.InvisibleOneShotTeleport;
import com.puttysoftware.mazemode.objects.InvisibleTeleport;
import com.puttysoftware.mazemode.objects.MetalButton;
import com.puttysoftware.mazemode.objects.MovingFinish;
import com.puttysoftware.mazemode.objects.OneShotTeleport;
import com.puttysoftware.mazemode.objects.Player;
import com.puttysoftware.mazemode.objects.RandomInvisibleOneShotTeleport;
import com.puttysoftware.mazemode.objects.RandomInvisibleTeleport;
import com.puttysoftware.mazemode.objects.RandomOneShotTeleport;
import com.puttysoftware.mazemode.objects.RandomTeleport;
import com.puttysoftware.mazemode.objects.StairsDown;
import com.puttysoftware.mazemode.objects.StairsUp;
import com.puttysoftware.mazemode.objects.Teleport;
import com.puttysoftware.mazemode.objects.TreasureChest;
import com.puttysoftware.mazemode.objects.TwoWayTeleport;
...
public class MazeEditor {
    ...
    public void updateEditorLevelAbsolute(final int w) {
        this.elMgr.setEditorLocationW(w);
        // Level Change
        MazeMode.getApplication().getMazeManager().getMaze().switchLevel(w);
        this.fixLimits();
        this.setUpGUI();
        this.checkMenus();
        this.redrawEditor();
    ...
    public MazeObject editTreasureChestContents() {
        MazeMode.getApplication().showMessage("Pick treasure chest contents");
    ...
    public void setTeleportDestination(final int x, final int y) {
        ...
        if (this.TELEPORT_TYPE == MazeEditor.TELEPORT_TYPE_MOVING_FINISH) {
            MazeMode.getApplication().showMessage("Next moving finish set.");
        } else if (this.TELEPORT_TYPE == MazeEditor.TELEPORT_TYPE_FIRST_MOVING_FINISH) {
            MazeMode.getApplication().showMessage("First moving finish set.");
        } else {
            MazeMode.getApplication().showMessage("Destination set.");
        }
        app.getMazeManager().setDirty(true);
        this.redrawEditor();
    }
    ...
    public void setMetalButtonTarget(final int x, final int y) {
        ...
        this.secondaryPane.addMouseListener(this.mhandler);
        this.checkMenus();
        MazeMode.getApplication().showMessage("Target set.");
        app.getMazeManager().setDirty(true);
        this.redrawEditor();
    }
    ...
    public void setTreasureChestContents() {
        ...
        app.getMazeManager().getMaze().setCell(new TreasureChest(contents),
                ...
                MazeConstants.LAYER_OBJECT);
        this.checkMenus();
        MazeMode.getApplication().showMessage("Contents set.");
        app.getMazeManager().setDirty(true);
        this.redrawEditor();
    }
    ...
    public void editPlayerLocation() {
        ...
        MazeMode.getApplication().showMessage("Click to set start point");
    }
    ...
    void setPlayerLocation(final int x, final int y) {
        ...
        try {
            ...
            app.getMazeManager().getMaze().setCell(new Player(), destX, destY,
                    this.elMgr.getEditorLocationZ(),
                    MazeConstants.LAYER_OBJECT);
            MazeMode.getApplication().showMessage("Start point set.");
        } catch (final ArrayIndexOutOfBoundsException aioob) {
    ...
    public void fillLevel() {
        if (this.confirmNonUndoable(
            ...
            MazeMode.getApplication().getMazeManager().getMaze()
                    .fillLevelDefault();
            MazeMode.getApplication().showMessage("Level filled.");
    ...
    public void fillFloor() {
        if (this.confirmNonUndoable(
            ...
            MazeMode.getApplication().showMessage("Floor filled.");
            MazeMode.getApplication().getMazeManager().setDirty(true);
            this.redrawEditor();
        }
    }
    ...
    public void fillLevelRandomly() {
        if (this.confirmNonUndoable(
            ...
            if (MazeMode.getApplication().getMenuManager().useFillRuleSets()) {
                MazeMode.getApplication().getMazeManager().getMaze()
            ...
            MazeMode.getApplication().showMessage("Level randomly filled.");
            MazeMode.getApplication().getMazeManager().setDirty(true);
            this.redrawEditor();
        }
    }
    ...
    public void fillFloorRandomly() {
        if (this.confirmNonUndoable(
            ...
            MazeMode.getApplication().showMessage("Floor randomly filled.");
            MazeMode.getApplication().getMazeManager().setDirty(true);
            this.redrawEditor();
        }
    }
    ...
    void goToDestination(final int x, final int y) {
        if (this.goToDestMode) {
            ...
            final MazeObject there = MazeMode.getApplication().getMazeManager()
    ...
    public void exitEditor() {
        ...
        gm.stateChanged();
        MazeMode.getApplication().getGUIManager().showGUI();
    }
    ...
    public void undo() {
        ...
        if (x != -1 && y != -1 && z != -1 && w != -1) {
            ...
            MazeMode.getApplication().showMessage("Nothing to undo");
        }
    }
    ...
    public void redo() {
        ...
        if (x != -1 && y != -1 && z != -1 && w != -1) {
            ...
            MazeMode.getApplication().showMessage("Nothing to redo");
        }
    }
