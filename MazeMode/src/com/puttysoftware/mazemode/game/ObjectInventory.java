...
import com.puttysoftware.mazemode.objects.NormalAmulet;
import com.puttysoftware.mazemode.objects.RegularBoots;
...
public final class ObjectInventory implements Cloneable {
    ...
    public static ObjectInventory readInventoryX(final XDataReader reader,
            final int formatVersion) throws IOException {
        final MazeObjectList objects = MazeMode.getApplication().getObjects();
        final ObjectInventory i = new ObjectInventory();
        i.boots = (GenericBoots) objects.readMazeObjectX(reader, formatVersion);
        if (i.boots == null) {
            i.boots = ObjectInventory.DEFAULT_BOOTS;
        }
        i.amulet = (GenericAmulet) objects.readMazeObjectX(reader,
                formatVersion);
        if (i.amulet == null) {
    ...
    public void writeInventoryX(final XDataWriter writer) throws IOException {
        this.boots.writeMazeObjectX(writer);
        this.amulet.writeMazeObjectX(writer);
