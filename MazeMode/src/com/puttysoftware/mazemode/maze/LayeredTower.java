...
import com.puttysoftware.mazemode.generic.RandomGenerationRule;
import com.puttysoftware.mazemode.generic.TypeConstants;
import com.puttysoftware.mazemode.objects.BarrierGenerator;
import com.puttysoftware.mazemode.objects.CrackedWall;
import com.puttysoftware.mazemode.objects.Crevasse;
...
import com.puttysoftware.mazemode.objects.EnragedBarrierGenerator;
import com.puttysoftware.mazemode.objects.ForceField;
import com.puttysoftware.mazemode.objects.HotRock;
import com.puttysoftware.mazemode.objects.Ice;
import com.puttysoftware.mazemode.objects.IcedBarrierGenerator;
...
import com.puttysoftware.mazemode.objects.PoisonedBarrierGenerator;
import com.puttysoftware.mazemode.objects.ShockedBarrierGenerator;
import com.puttysoftware.mazemode.objects.Slime;
...
class LayeredTower implements Cloneable {
    ...
    public void tickTimers(final int floor) {
        ...
        if (this.timerActive) {
            ...
            if (this.timerValue == 0) {
                ...
                MazeMode.getApplication().getGameManager().solvedLevel();
            }
        }
    }
    ...
    public boolean isSquareVisible(final int x1, final int y1, final int x2,
        ...
        boolean result = true;
        final int zLoc = MazeMode.getApplication().getGameManager()
    ...
    private static MazeObject[][][] shuffleObjects(final MazeObject[][][] preShuffle,
            final int r) {
    ...
    public void writeXLayeredTower(final XDataWriter writer) throws IOException {
        ...
        for (x = 0; x < this.getColumns(); x++) {
            for (y = 0; y < this.getRows(); y++) {
                for (z = 0; z < this.getFloors(); z++) {
                    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
                        this.getCell(y, x, z, e).writeMazeObjectX(writer);
                    }
                }
            }
        }
        ...
        for (x = 0; x < lt.getColumns(); x++) {
            for (y = 0; y < lt.getRows(); y++) {
                for (z = 0; z < lt.getFloors(); z++) {
                    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
                        lt.setCell(MazeMode.getApplication().getObjects()
                                .readMazeObjectX(reader, ver), y, x, z, e);
