...
final class LayeredDungeon implements Cloneable {
    ...
    public LayeredDungeon(final int rows, final int cols, final int floors) {
        ...
        this.savedDungeonState = new LowLevelAMODataStore(cols, rows, floors,
                MazeConstants.LAYER_COUNT);
        ...
        this.visionRadius = LayeredDungeon.MAX_VISION_RADIUS;
        this.regionSize = 8;
        this.bombs = 0;
        this.hammers = 0;
        this.keys = 0;
    ...
    public static int getMaxFloors() {
        return LayeredDungeon.MAX_FLOORS;
    }

    public static int getMaxColumns() {
        return LayeredDungeon.MAX_COLUMNS;
    }

    public static int getMaxRows() {
        return LayeredDungeon.MAX_ROWS;
    }
    ...
    public LayeredDungeon clone() {
        final LayeredDungeon copy = new LayeredDungeon(this.getRows(),
    ...
    public void updateMonsterPosition(final int move, final int xLoc,
        ...
        try {
            ...
            if (!there.isSolid() && !there.getName().equals("Monster")) {
                if (LayeredDungeon.radialScan(xLoc, yLoc, 0, pLocX, pLocY)) {
    ...
    public void fillFloorRandomly(final Maze maze, final int z, final int w) {
        // Pre-Pass
        final MazeObjectList objects = DDRemix.getApplication().getObjects();
    ...
    public void fullScanExit() {
        int u, v, z;
        final int l = MazeConstants.LAYER_OBJECT;
        z = LayeredDungeon.MAX_FLOORS - 1;
        // Perform the scan
        for (u = 0; u < this.getColumns(); u++) {
            for (v = 0; v < this.getRows(); v++) {
    ...
    public void writeLayeredDungeon(final XDataWriter writer)
            throws IOException {
        int y, x, z, e;
    ...
    public static LayeredDungeon readLayeredDungeonV1(final XDataReader reader)
            throws IOException {
        int y, x, z, e, mazeSizeX, mazeSizeY, mazeSizeZ;
        mazeSizeX = reader.readInt();
        mazeSizeY = reader.readInt();
        mazeSizeZ = reader.readInt();
        final LayeredDungeon lt = new LayeredDungeon(mazeSizeX, mazeSizeY,
                mazeSizeZ);
    ...
    public void writeSavedDungeonState(final XDataWriter writer)
            throws IOException {
        int x, y, z, e;
        writer.writeInt(this.getColumns());
        writer.writeInt(this.getRows());
    ...
    public void readSavedDungeonState(final XDataReader reader,
            final int formatVersion) throws IOException {
        int x, y, z, e, sizeX, sizeY, sizeZ;
        sizeX = reader.readInt();
        sizeY = reader.readInt();
        sizeZ = reader.readInt();
        this.savedDungeonState = new LowLevelAMODataStore(sizeY, sizeX, sizeZ,
                MazeConstants.LAYER_COUNT);
        for (x = 0; x < sizeY; x++) {
            for (y = 0; y < sizeX; y++) {
                for (z = 0; z < sizeZ; z++) {
                    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
                        this.savedDungeonState.setCell(
                                DDRemix.getApplication().getObjects()
