...
public class ImageTransformer {
    ...
    private static final int[] MONSTER_COLORS = new int[] {
	    MONSTER_FIRST.getRGB(), MONSTER_SECOND.getRGB(),
	    MONSTER_OUTLINE.getRGB(), MONSTER_THIRD.getRGB(),
	    MONSTER_FOURTH.getRGB(), MONSTER_FIFTH.getRGB(),
	    MONSTER_EYE.getRGB() };
	...
	if (input != null && e != null) {
	    Color[] tcs = e.getTransformColors();
	    BufferedImageIcon result;
	    if (PreferencesManager.getHighDefEnabled()) {
		result = new BufferedRetinaImageIcon(input);
	    } else {
		...
		for (int y = 0; y < imageSize; y++) {
		    ...
		    if (a != 0) {
			for (int z = 0; z < MONSTER_COLORS.length; z++) {
			    if (pixel == MONSTER_COLORS[z]) {
