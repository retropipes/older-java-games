...
class GameGUIManager {
	...
	if (this.vwMgr == null) {
	    this.vwMgr = DynamicDungeon.getApplication().getGameManager()
		    .getViewManager();
	    this.setUpGUI();
	}
    }
	...
	if (this.outputFrame.isVisible()) {
		...
		for (y = this.vwMgr.getViewingWindowLocationY(); y <= this.vwMgr
		    ...
		    try {
			if (visible) {
			    final AbstractDungeonObject obj1 = m.getCell(y, x,
				    m.getPlayerLocationZ(),
				    DungeonConstants.LAYER_GROUND)
				    .gameRenderHook(y, x,
					    m.getPlayerLocationZ());
			    final AbstractDungeonObject obj2 = m.getCell(y, x,
				    m.getPlayerLocationZ(),
				...
				final AbstractDungeonObject obj3 = NOTE
					.gameRenderHook(y, x,
						m.getPlayerLocationZ());
				final BufferedImageIcon img3 = ObjectImageManager
					.getImage(obj3.getName(), obj3
			...
			final AbstractDungeonObject obj = ev.gameRenderHook(y,
				x, m.getPlayerLocationZ());
			this.drawGrid.setImageCell(ObjectImageManager.getImage(
				obj.getName(), obj.getGameBaseID(),
	...
	public void mouseClicked(final MouseEvent e) {
	    try {
		final GameLogicManager gm = DynamicDungeon.getApplication()
			.getGameManager();
		if (e.isShiftDown()) {
		    final int x = e.getX();
		    final int y = e.getY();
