...
class SavedTowerState implements Cloneable {
	...
	// Copy constructor
	public SavedTowerState(final SavedTowerState other) {
		int cols = other.c;
		int rows = other.r;
		int floors = other.f;
		this.saveData = new MapObject[cols][rows][floors][MapConstants.LAYER_COUNT];
			...
			for (y = 0; y < this.r; y++) {
				for (z = 0; z < this.f; z++) {
					for (e = 0; e < MapConstants.LAYER_COUNT; e++) {
						try {
							this.saveData[x][y][z][e] = other.saveData[x][y][z][e].getClass()
									.getConstructor(MapObject.class).newInstance(other);
						} catch (InstantiationException | IllegalAccessException | IllegalArgumentException
								| InvocationTargetException | NoSuchMethodException | SecurityException e1) {
							Gemma.getErrorLogger().logError(e1);
						}
					}
				}
			}
