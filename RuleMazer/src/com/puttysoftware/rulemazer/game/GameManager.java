...
import com.puttysoftware.rulemazer.maze.effects.MazeEffectManager;
import com.puttysoftware.rulemazer.objects.AnnihilationWand;
import com.puttysoftware.rulemazer.objects.Bow;
import com.puttysoftware.rulemazer.objects.DarkGem;
import com.puttysoftware.rulemazer.objects.DarkWand;
import com.puttysoftware.rulemazer.objects.DisarmTrapWand;
import com.puttysoftware.rulemazer.objects.Empty;
import com.puttysoftware.rulemazer.objects.EmptyVoid;
import com.puttysoftware.rulemazer.objects.FinishMakingWand;
import com.puttysoftware.rulemazer.objects.FireAmulet;
import com.puttysoftware.rulemazer.objects.HotBoots;
import com.puttysoftware.rulemazer.objects.IceAmulet;
import com.puttysoftware.rulemazer.objects.LightGem;
import com.puttysoftware.rulemazer.objects.LightWand;
import com.puttysoftware.rulemazer.objects.PasswallBoots;
import com.puttysoftware.rulemazer.objects.Player;
import com.puttysoftware.rulemazer.objects.SlipperyBoots;
import com.puttysoftware.rulemazer.objects.SunStone;
import com.puttysoftware.rulemazer.objects.TeleportWand;
import com.puttysoftware.rulemazer.objects.Wall;
import com.puttysoftware.rulemazer.objects.WallBreakingWand;
import com.puttysoftware.rulemazer.objects.WallMakingWand;
...
public class GameManager implements MazeEffectConstants {
    ...
    public GameManager() {
        ...
        this.lastUsedBowIndex = Main.getApplication().getObjects()
    ...
    private void saveSavedMazeObject() {
        ...
        final GenericCharacter player = (GenericCharacter) Main.getApplication()
    ...
    public void doRemoteAction(final int x, final int y, final int z) {
        ...
        final MazeObject acted = Main.getApplication().getMazeManager()
    ...
    public void doClockwiseRotate(final int r) {
        ...
        if (b) {
            ...
            Main.getApplication().showMessage("Rotation failed!");
        }
    }
    ...
    public void fireArrow(final int x, final int y) {
        if (this.objectInv.getUses(this.activeBow) == 0) {
            Main.getApplication().showMessage("You're out of arrows!");
        } else {
        ...
        do {
            ...
            if (proceed) {
                ...
                try {
                    ...
                    this.decayEffects();
                    this.fireStepActions();
                    Main.getApplication().showMessage("Can't go that way");
                    o = new Empty();
                    proceed = false;
                }
            } else {
            ...
            if (redrawsSuspended && !this.checkLoopCondition(proceed, x, y,
    ...
    public void updatePositionAbsolute(final int x, final int y, final int z) {
        ...
        try {
            ...
            m.setCell(new Player(), this.plMgr.getPlayerLocationX(),
                    ...
                    this.plMgr.getPlayerLocationZ(),
                    MazeConstants.LAYER_OBJECT);
            Main.getApplication().showMessage("Can't go outside the maze");
        } catch (final NullPointerException np) {
            this.plMgr.restorePlayerLocation();
            this.vwMgr.restoreViewingWindow();
            m.setCell(new Player(), this.plMgr.getPlayerLocationX(),
                ...
                for (y = this.vwMgr.getViewingWindowLocationY(); y <= this.vwMgr
                    ...
                    try {
                            ...
                            if (this.trueSightFlag) {
                                name1 = app.getMazeManager().getMaze()
                                        .getCell(y, x,
                                                this.plMgr.getPlayerLocationZ(),
                                                MazeConstants.LAYER_GROUND)
                                        .editorRenderHook(y, x, this.plMgr
                                                .getPlayerLocationZ());
                        ...
                        if (this.trueSightFlag) {
                            ...
                            this.drawGrid[xFix][yFix].setIcon(GraphicsManager
                                    .getImage(new EmptyVoid().gameRenderHook(y,
                                            x,
                                            this.plMgr.getPlayerLocationZ())));
                        }
                    } catch (final NullPointerException np) {
    ...
    public void solvedMaze() {
        ...
        if (this.st.checkScore()) {
            Main.getApplication().playHighScoreSound();
        }
        this.st.commitScore();
        this.exitGame();
    }
    ...
    public void checkGameOver() {
        if (!Main.getApplication().getMazeManager().getMaze().isAlive()) {
            this.gameOver();
        }
    }
    ...
    public void decay() {
        if (this.actingRemotely) {
            Main.getApplication().getMazeManager().getMaze().setCell(
    ...
    public void decayTo(final MazeObject decay) {
        if (this.actingRemotely) {
            Main.getApplication().getMazeManager().getMaze().setCell(decay,
    ...
    public void morph(final MazeObject morphInto, final int x, final int y,
        ...
        try {
            m.setCell(morphInto, x, y, z, morphInto.getLayer());
            Main.getApplication().showMessage(msg);
            this.keepNextMessage();
            this.redrawMazeNoRebuild();
            app.getMazeManager().setDirty(true);
        } catch (final ArrayIndexOutOfBoundsException ae) {
    ...
    public void showUseDialog() {
        int x;
        final MazeObjectList list = Main.getApplication().getObjects();
    ...
    public void showSwitchBowDialog() {
        ...
        final String result = CommonDialogs.showInputDialog(
                "Switch to which bow?", "RuleMazer", userChoices,
        ...
        if (this.usingAnItem() && app.getMode() == Application.STATUS_GAME) {
            ...
            try {
                ...
                if ((target.isSolid() || !visible)
                    ...
                    this.setUsingAnItem(false);
                    Main.getApplication().showMessage("Can't teleport there");
                }
                if (target.getName().equals(new Player().getName())) {
                    this.setUsingAnItem(false);
                    Main.getApplication().showMessage("Don't aim at yourself!");
                }
                if (!target.isDestroyable()
                        && name.equals(new AnnihilationWand().getName())) {
                    this.setUsingAnItem(false);
                    Main.getApplication().showMessage("Can't destroy that");
                }
                if (!target.isDestroyable()
                ...
                if ((!target.isDestroyable()
                    ...
                    this.setUsingAnItem(false);
                    Main.getApplication().showMessage("Aim at a wall");
                }
                if ((!target.isDestroyable()
                        || !target.isOfType(TypeConstants.TYPE_TRAP))
                        && name.equals(new DisarmTrapWand().getName())) {
                    this.setUsingAnItem(false);
                    Main.getApplication().showMessage("Aim at a trap");
                }
    ...
    public void controllableTeleport() {
        ...
        Main.getApplication().showMessage("Click to set destination");
    }
    ...
    public void identifyObject(final int x, final int y) {
        ...
        try {
            ...
            Main.getApplication().showMessage(gameName2 + " on " + gameName1);
            SoundManager.playSound(SoundConstants.SOUND_CATEGORY_SOLVING_MAZE,
                    SoundConstants.SOUND_IDENTIFY);
        } catch (final ArrayIndexOutOfBoundsException ae) {
            final EmptyVoid ev = new EmptyVoid();
            ev.determineCurrentAppearance(destX, destY, destZ);
            Main.getApplication().showMessage(ev.getGameName());
            SoundManager.playSound(SoundConstants.SOUND_CATEGORY_SOLVING_MAZE,
                    SoundConstants.SOUND_IDENTIFY);
        }
    }

    public void loadGameHookXML(final XMLDataReader mazeFile,
    ...
    public void saveGameHookXML(final XMLDataWriter mazeFile)
            throws IOException {
        final Application app = Main.getApplication();
    ...
    private void setUpGUI() {
        ...
        this.outputFrame = new JFrame("RuleMazer");
