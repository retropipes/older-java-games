...
import com.puttysoftware.rulemazer.objects.BarrierGenerator;
import com.puttysoftware.rulemazer.objects.CrackedWall;
import com.puttysoftware.rulemazer.objects.Crevasse;
...
import com.puttysoftware.rulemazer.objects.EnragedBarrierGenerator;
import com.puttysoftware.rulemazer.objects.ForceField;
import com.puttysoftware.rulemazer.objects.HotRock;
import com.puttysoftware.rulemazer.objects.Ice;
import com.puttysoftware.rulemazer.objects.IcedBarrierGenerator;
...
import com.puttysoftware.rulemazer.objects.PoisonedBarrierGenerator;
...
class LayeredTower implements Cloneable {
    ...
    public MazeObject getCell(int fR, int fC, int fF, final int extra) {
        int row = fR;
        int col = fC;
        int floor = fF;
    ...
    public void tickTimers(final int floor) {
        ...
        if (this.timerActive) {
            ...
            if (this.timerValue == 0) {
                ...
                Main.getApplication().getGameManager().solvedLevel();
            }
        }
    }
    ...
    public boolean isSquareVisible(final int x1, final int y1, final int x2,
        ...
        boolean result = true;
        final int zLoc = Main.getApplication().getGameManager()
        ...
        if (xDist <= this.visionRadius && yDist <= this.visionRadius) {
            if (this.radialScan(x2, y2, zLoc, MazeConstants.LAYER_OBJECT,
    ...
    public void setCell(final MazeObject mo, int fR, int fC, int fF,
            final int extra) {
        int row = fR;
        int col = fC;
        int floor = fF;
    ...
    public void setVisionRadius(int fVR) {
        int newVR = fVR;
        if (newVR > LayeredTower.MAX_VISION_RADIUS) {
            newVR = LayeredTower.MAX_VISION_RADIUS;
        }
        if (newVR < LayeredTower.MIN_VISION_RADIUS) {
            newVR = LayeredTower.MIN_VISION_RADIUS;
        }
        this.visionRadius = newVR;
        this.initialVisionRadius = newVR;
    }
    ...
    public void fillFloorRandomlyCustom(final Maze maze, final int z,
        ...
        if (withoutRuleSets != null) {
            ...
            int N = 1;
            for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
                MazeObject[] objectsWithPrerequisites = MazeObjectList
                ...
                while (objectsWithPrerequisites != null) {
                    ...
                    N++;
                    objectsWithPrerequisites = MazeObjectList
    ...
    public void writeXMLLayeredTower(final XMLDataWriter writer)
            throws IOException {
        int y, x, z, e;
    ...
    public static LayeredTower readXMLLayeredTowerV1(
            final XMLDataReader reader, final int ver) throws IOException {
        ...
        for (x = 0; x < lt.getColumns(); x++) {
            for (y = 0; y < lt.getRows(); y++) {
                for (z = 0; z < lt.getFloors(); z++) {
                    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
                        lt.setCell(Main.getApplication().getObjects()
    ...
    public static LayeredTower readXMLLayeredTowerV2(
            final XMLDataReader reader, final int ver) throws IOException {
        int y, x, z, e, mazeSizeX, mazeSizeY, mazeSizeZ;
        mazeSizeX = reader.readInt();
        mazeSizeY = reader.readInt();
    ...
    public static LayeredTower readXMLLayeredTowerV3(
            final XMLDataReader reader, final int ver) throws IOException {
        int y, x, z, e, mazeSizeX, mazeSizeY, mazeSizeZ;
        mazeSizeX = reader.readInt();
        mazeSizeY = reader.readInt();
    ...
    public static LayeredTower readXMLLayeredTowerV4(
            final XMLDataReader reader, final int ver) throws IOException {
        int y, x, z, e, mazeSizeX, mazeSizeY, mazeSizeZ;
        mazeSizeX = reader.readInt();
        mazeSizeY = reader.readInt();
    ...
    public void writeSavedTowerStateXML(final XMLDataWriter writer)
            throws IOException {
        this.savedTowerState.writeSavedTowerStateXML(writer);
    }

    public void readSavedTowerStateXML(final XMLDataReader reader,
