...
import com.puttysoftware.rulemazer.maze.xml.XMLPrefixIO;
import com.puttysoftware.rulemazer.maze.xml.XMLSuffixIO;
...
public class Maze implements MazeConstants {
    ...
    public Maze() throws IOException {
        ...
        this.basePath = System.getProperty("java.io.tmpdir") + File.separator
                + "RuleMazer" + File.separator + randomID + ".maze";
    ...
    public boolean removeLevel() {
        if (this.levelCount > 1) {
            if (this.activeLevel >= 1 && this.activeLevel <= this.levelCount) {
                ...
                final boolean deleted = this.getLevelFile(this.activeLevel)
                        .delete();
                if (!deleted) {
                    return false;
                }
    ...
    public Maze readMazeXML() throws IOException {
        ...
        final XMLDataReader metaReader = new XMLDataReader(m.basePath
    ...
    private XMLDataReader getLevelReaderXML() throws IOException {
        return new XMLDataReader(this.basePath + File.separator + "level"
                + this.activeLevel + ".xml", "level");
    }

    private int readMazeMetafileXML(final XMLDataReader reader)
            throws IOException {
    ...
    private void readMazeLevelXML(final XMLDataReader reader)
            throws IOException {
    ...
    public void writeMazeXML() throws IOException {
        ...
        final XMLDataWriter metaWriter = new XMLDataWriter(this.basePath
                + File.separator + "metafile.xml", "maze");
        // Write metafile
    ...
    private XMLDataWriter getLevelWriterXML() throws IOException {
        return new XMLDataWriter(this.basePath + File.separator + "level"
                + this.activeLevel + ".xml", "level");
    }

    private void writeMazeMetafileXML(final XMLDataWriter writer)
            throws IOException {
        if (this.xmlPrefixHandler != null) {
    ...
    private void writeMazeLevelXML(final XMLDataWriter writer)
            throws IOException {
        // Clear XML compatibility flag
        Main.getApplication().getMazeManager().setMazeXML1Compatible(false);
        // Clear XML 2 compatibility flag
