...
public class MazeEffectManager {
    ...
    public void decayEffects() {
        for (int x = 0; x < MazeEffectManager.NUM_EFFECTS; x++) {
            if (this.activeEffects[x].isActive()) {
                ...
                if (!this.activeEffects[x].isActive()) {
                    Main.getApplication().showMessage("You feel normal again.");
                    // Clear effect grid
                    this.clearGridEntry(x);
                    // Pack
                    Main.getApplication().getGameManager().getOutputFrame()
                            .pack();
                }
            }
        }
    ...
    public void deactivateAllEffects() {
        ...
        // Pack
        Main.getApplication().getGameManager().getOutputFrame().pack();
        for (int x = 0; x < MazeEffectManager.NUM_EFFECTS; x++) {
