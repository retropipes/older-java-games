...
import com.puttysoftware.rulemazer.objects.EmptyVoid;
import com.puttysoftware.rulemazer.objects.FinishTo;
import com.puttysoftware.rulemazer.objects.InvisibleOneShotTeleport;
import com.puttysoftware.rulemazer.objects.InvisibleTeleport;
import com.puttysoftware.rulemazer.objects.MetalButton;
import com.puttysoftware.rulemazer.objects.MovingFinish;
import com.puttysoftware.rulemazer.objects.OneShotTeleport;
import com.puttysoftware.rulemazer.objects.Player;
import com.puttysoftware.rulemazer.objects.RandomInvisibleOneShotTeleport;
import com.puttysoftware.rulemazer.objects.RandomInvisibleTeleport;
import com.puttysoftware.rulemazer.objects.RandomOneShotTeleport;
import com.puttysoftware.rulemazer.objects.RandomTeleport;
import com.puttysoftware.rulemazer.objects.StairsDown;
import com.puttysoftware.rulemazer.objects.StairsUp;
import com.puttysoftware.rulemazer.objects.Teleport;
import com.puttysoftware.rulemazer.objects.TreasureChest;
import com.puttysoftware.rulemazer.objects.TwoWayTeleport;
...
public class MazeEditor {
    ...
    public void updateEditorLevelAbsolute(final int w) {
        this.elMgr.setEditorLocationW(w);
        // Level Change
        Main.getApplication().getMazeManager().getMaze().switchLevel(w);
        this.fixLimits();
        this.setUpGUI();
        this.checkMenus();
        this.redrawEditor();
    ...
    private void redrawVirtual(final int x, final int y, final String name3) {
        // Draw the square
        final Application app = Main.getApplication();
        int xFix, yFix;
        ...
        try {
            String name1, name2;
            name1 = app.getMazeManager().getMaze()
                    .getCell(y, x, this.elMgr.getEditorLocationZ(),
                            MazeConstants.LAYER_GROUND)
                    .editorRenderHook(y, x, this.elMgr.getEditorLocationZ());
            name2 = app.getMazeManager().getMaze()
                    .getCell(y, x, this.elMgr.getEditorLocationZ(),
                            MazeConstants.LAYER_OBJECT)
                    .editorRenderHook(y, x, this.elMgr.getEditorLocationZ());
                ...
                if (this.conditionalEditFlag == MazeEditor.CEF_CONDITION) {
                    ...
                    final String resp = CommonDialogs
                            .showTextInputDialogWithDefault(
                                    "Condition Trigger Value (Number of Sun Stones needed)",
                                    "Editor", def);
                    int respVal = -1;
                    try {
    ...
    public MazeObject editMetalButtonTarget() {
        Main.getApplication().showMessage("Click to set metal button target");
        final Application app = Main.getApplication();
        this.horzScroll.removeAdjustmentListener(this.mhandler);
        this.vertScroll.removeAdjustmentListener(this.mhandler);
        this.secondaryPane.removeMouseListener(this.mhandler);
    ...
    public MazeObject editTreasureChestContents() {
        Main.getApplication().showMessage("Pick treasure chest contents");
    ...
    public void setTeleportDestination(final int x, final int y) {
        ...
        if (this.TELEPORT_TYPE == MazeEditor.TELEPORT_TYPE_MOVING_FINISH) {
            Main.getApplication().showMessage("Next moving finish set.");
        } else if (this.TELEPORT_TYPE == MazeEditor.TELEPORT_TYPE_FIRST_MOVING_FINISH) {
            Main.getApplication().showMessage("First moving finish set.");
        } else {
            Main.getApplication().showMessage("Destination set.");
        }
        app.getMazeManager().setDirty(true);
        this.redrawEditor();
    }
    ...
    public void setMetalButtonTarget(final int x, final int y) {
        ...
        this.secondaryPane.addMouseListener(this.mhandler);
        this.checkMenus();
        Main.getApplication().showMessage("Target set.");
        app.getMazeManager().setDirty(true);
        this.redrawEditor();
    }
    ...
    public void setTreasureChestContents() {
        ...
        app.getMazeManager().getMaze().setCell(new TreasureChest(contents),
                ...
                MazeConstants.LAYER_OBJECT);
        this.checkMenus();
        Main.getApplication().showMessage("Contents set.");
        app.getMazeManager().setDirty(true);
        this.redrawEditor();
    }
    ...
    public void editPlayerLocation() {
        ...
        Main.getApplication().showMessage("Click to set start point");
    }
    ...
    public void setPlayerLocation(final int x, final int y, final int z) {
        ...
        // Erase old player
        try {
            app.getMazeManager().getMaze().setCell(new Empty(), oldX, oldY,
                    oldZ, MazeConstants.LAYER_OBJECT);
        } catch (final ArrayIndexOutOfBoundsException aioob) {
            // Ignore
        }
        // Set new player
        ...
        app.getMazeManager().getMaze().setCell(new Player(), x, y, z,
                MazeConstants.LAYER_OBJECT);
    }
    ...
    public void setPlayerLocation() {
        ...
        app.getMazeManager().getMaze().setCell(new Player(),
                this.elMgr.getEditorLocationX(),
                this.elMgr.getEditorLocationY(),
    ...
    void setPlayerLocation(final int x, final int y) {
        ...
        try {
            ...
            app.getMazeManager().getMaze().setCell(new Player(), destX, destY,
                    this.elMgr.getEditorLocationZ(),
                    MazeConstants.LAYER_OBJECT);
            Main.getApplication().showMessage("Start point set.");
        } catch (final ArrayIndexOutOfBoundsException aioob) {
            app.getMazeManager().getMaze().restoreStart();
            try {
                app.getMazeManager().getMaze().setCell(new Player(), oldX, oldY,
                        oldZ, MazeConstants.LAYER_OBJECT);
            } catch (final ArrayIndexOutOfBoundsException aioob2) {
                // Ignore
            }
            Main.getApplication().showMessage("Aim within the maze");
        }
        // Swap event handlers
    ...
    public void fillLevel() {
        if (this.confirmNonUndoable(
            ...
            Main.getApplication().getMazeManager().getMaze().fillLevelDefault();
            Main.getApplication().showMessage("Level filled.");
    ...
    public void fillFloor() {
        if (this.confirmNonUndoable(
            ...
            Main.getApplication().showMessage("Floor filled.");
            Main.getApplication().getMazeManager().setDirty(true);
            this.redrawEditor();
        }
    }
    ...
    public void fillLevelRandomly() {
        if (this.confirmNonUndoable(
            ...
            if (Main.getApplication().getMenuManager().useFillRuleSets()) {
                Main.getApplication().getMazeManager().getMaze()
            ...
            Main.getApplication().showMessage("Level randomly filled.");
            Main.getApplication().getMazeManager().setDirty(true);
            this.redrawEditor();
        }
    }
    ...
    public void fillFloorRandomly() {
        if (this.confirmNonUndoable(
            ...
            Main.getApplication().showMessage("Floor randomly filled.");
            Main.getApplication().getMazeManager().setDirty(true);
            this.redrawEditor();
        }
    }
    ...
    void goToDestination(final int x, final int y) {
        if (this.goToDestMode) {
            ...
            final MazeObject there = Main.getApplication().getMazeManager()
            ...
            if (there instanceof GenericTeleport) {
                ...
                this.redrawVirtual(destX, destY, "destination");
            } else {
                Main.getApplication().showMessage(
    ...
    public void exitEditor() {
        ...
        gm.stateChanged();
        Main.getApplication().getGUIManager().showGUI();
    }
    ...
    public void undo() {
        ...
        if (x != -1 && y != -1 && z != -1 && w != -1) {
            ...
            Main.getApplication().showMessage("Nothing to undo");
        }
    }
    ...
    public void redo() {
        ...
        if (x != -1 && y != -1 && z != -1 && w != -1) {
            ...
            Main.getApplication().showMessage("Nothing to redo");
        }
    }
