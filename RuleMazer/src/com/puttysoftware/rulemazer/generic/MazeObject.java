...
public abstract class MazeObject implements DirectionConstants, TypeConstants,
    ...
    public void pushFailedAction(final ObjectInventory inv, final int x,
        ...
        Main.getApplication().getGameManager().keepNextMessage();
        Main.getApplication().showMessage("Can't push that");
    }
    ...
    public void pullFailedAction(final ObjectInventory inv, final int x,
        ...
        SoundManager.playSound(SoundConstants.SOUND_CATEGORY_SOLVING_MAZE,
                SoundConstants.SOUND_ACTION_FAILED);
        Main.getApplication().getGameManager().keepNextMessage();
        Main.getApplication().showMessage("Can't pull that");
    }
    ...
    public final void writeMazeObjectXML(final XMLDataWriter writer)
            throws IOException {
    ...
    public final MazeObject readMazeObjectXML(final XMLDataReader reader,
    ...
    public final MazeObject readMazeObjectXML2(final XMLDataReader reader,
            final String ident, final int ver) throws IOException {
        if (ident.equals(this.getXMLIdentifier())) {
            final int cc = this.getCustomFormat();
            if (cc == MazeObject.CUSTOM_FORMAT_MANUAL_OVERRIDE) {
    ...
    public final MazeObject readMazeObjectXML3(final XMLDataReader reader,
            final String ident, final int ver) throws IOException {
        if (ident.equals(this.getXMLIdentifier())) {
            final int cc = this.getCustomFormat();
            if (cc == MazeObject.CUSTOM_FORMAT_MANUAL_OVERRIDE) {
    ...
    public final MazeObject readMazeObjectXML4(final XMLDataReader reader,
            final String ident, final int ver) throws IOException {
        if (ident.equals(this.getXMLIdentifier())) {
            final int cc = this.getCustomFormat();
            if (cc == MazeObject.CUSTOM_FORMAT_MANUAL_OVERRIDE) {
