...
import com.puttysoftware.mazerunner2.creatures.characterfiles.CharacterLoader;
...
public class Party {
    ...
    private PartyMember pickPartyMemberInternal(String[] pickNames,
        ...
        String response = CommonDialogs.showInputDialog("Pick " + number
                ...
                "MazeRunnerII", pickNames, pickNames[0]);
        if (response != null) {
            int loc = this.findMember(response, 0, this.members.length);
            if (loc != -1) {
                return this.members[loc];
    ...
    static Party readLegacy(XLegacyDataReader worldFile) throws IOException {
        ...
        for (int z = 0; z < memCount; z++) {
            ...
            if (present) {
                pty.members[z] = PartyMember.readLegacy(worldFile);
            }
        }
        return pty;
    }
