...
class SeekerAI extends AbstractMapAIRoutine {
    // Fields
    ...
    private boolean spellCheck(MapAIContext ac) {
    ...
    private static int getMaxCastIndex(MapAIContext ac) {
        int currMP = ac.getCharacter().getTemplate().getCurrentMP();
        int[] allCosts = ac.getCharacter().getTemplate().getSpellBook()
                .getAllSpellCosts();
        int result = -1;
        if (currMP > 0) {
    ...
    private static boolean stealCheck(MapAIContext ac) {
        RandomRange random = new RandomRange(1, 100);
        int chance = random.generate();
    ...
    private static boolean drainCheck(MapAIContext ac) {
        RandomRange random = new RandomRange(1, 100);
        int chance = random.generate();
