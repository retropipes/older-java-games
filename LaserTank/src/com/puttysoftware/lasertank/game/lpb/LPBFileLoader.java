...
class LPBFileLoader {
    ...
    static boolean loadLPB(final FileInputStream file) {
        try {
            ...
            int bytesread = file.read(LPBFileLoader.lname, 0,
                    LPBFileLoader.LNAME_SIZE);
            if (bytesread != LPBFileLoader.LNAME_SIZE) {
                return false;
            }
            bytesread = file.read(LPBFileLoader.author, 0,
                    LPBFileLoader.AUTHOR_SIZE);
            if (bytesread != LPBFileLoader.AUTHOR_SIZE) {
                return false;
            }
            bytesread = file.read(LPBFileLoader.lnum, 0,
                    LPBFileLoader.LNUM_SIZE);
            if (bytesread != LPBFileLoader.LNUM_SIZE) {
                return false;
            }
            bytesread = file.read(LPBFileLoader.rawsize, 0,
                    LPBFileLoader.RAWSIZE_SIZE);
