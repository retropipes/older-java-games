...
public class ExplodingBarrel extends AbstractReactionWall {
    ...
    public Direction laserEnteredActionHook(final int locX, final int locY,
        ...
        // Did tank die?
        final boolean dead = this.laserEnteredActionInnerP1(locX, locY, locZ,
                false);
        if (dead) {
            // Kill tank
        ...
        ExplodingBarrel.laserEnteredActionInnerP2(locX, locY, locZ,
                this.getLayer());
    ...
    public boolean rangeActionHook(final int locX, final int locY,
        ...
        SoundManager.playSound(SoundConstants.SOUND_BOOM);
        // Did tank die?
        final boolean dead = this.laserEnteredActionInnerP1(locX + dirX,
                locY + dirY, locZ, false);
        if (dead) {
            // Kill tank
            LaserTank.getApplication().getGameManager().gameOver();
            return true;
    ...
    private boolean laserEnteredActionInnerP1(final int locX, final int locY,
            final int locZ, final boolean oldDead) {
        ...
        final boolean boom2 = LaserTank.getApplication().getArenaManager()
                .getArena().getCell(locX, locY - 1, locZ, this.getLayer())
                .getClass().equals(ExplodingBarrel.class);
        if (boom2) {
            return this.laserEnteredActionInnerP1(locX, locY - 1, locZ, dead);
        }
        final boolean boom4 = LaserTank.getApplication().getArenaManager()
                .getArena().getCell(locX - 1, locY, locZ, this.getLayer())
                .getClass().equals(ExplodingBarrel.class);
        if (boom4) {
            return this.laserEnteredActionInnerP1(locX - 1, locY, locZ, dead);
        }
        final boolean boom6 = LaserTank.getApplication().getArenaManager()
                .getArena().getCell(locX + 1, locY, locZ, this.getLayer())
                .getClass().equals(ExplodingBarrel.class);
        if (boom6) {
            return this.laserEnteredActionInnerP1(locX + 1, locY, locZ, dead);
        }
        final boolean boom8 = LaserTank.getApplication().getArenaManager()
                .getArena().getCell(locX, locY + 1, locZ, this.getLayer())
                .getClass().equals(ExplodingBarrel.class);
        if (boom8) {
            return this.laserEnteredActionInnerP1(locX, locY + 1, locZ, dead);
    ...
    private static void laserEnteredActionInnerP2(final int locX,
            final int locY, final int locZ, final int locW) {
        // Destroy barrel
        ...
        try {
            final boolean boom2 = LaserTank.getApplication().getArenaManager()
                    .getArena().getCell(locX, locY - 1, locZ, locW).getClass()
                    .equals(ExplodingBarrel.class);
            if (boom2) {
                ExplodingBarrel.laserEnteredActionInnerP2(locX, locY - 1, locZ,
                        locW);
            }
        ...
        try {
            final boolean boom4 = LaserTank.getApplication().getArenaManager()
                    .getArena().getCell(locX - 1, locY, locZ, locW).getClass()
                    .equals(ExplodingBarrel.class);
            if (boom4) {
                ExplodingBarrel.laserEnteredActionInnerP2(locX - 1, locY, locZ,
                        locW);
            }
        } catch (final ArrayIndexOutOfBoundsException aioobe) {
            // Ignore
    ...
    public void pushCollideAction(final AbstractMovableObject pushed,
        ...
        if (pushed.isOfType(TypeConstants.TYPE_BALL)) {
            this.laserEnteredAction(x, y, z, 0, 0,
                    LaserTypeConstants.LASER_TYPE_GREEN, 1);
        }
    }
