...
import com.puttysoftware.mastermaze.maze.objects.*;
...
public class MazeObjectList {
    ...
    public MazeObjectList() {
        final MazeObject[] allObjects = { new Amethyst(),
                ...
                new OneShotChainTeleport(), new OneShotConditionalTeleport(),
    ...
    public MazeObject[] getAllContainableObjects() {
        final MazeObject[] objects = this.getAllObjects();
        final MazeObject[] tempAllContainableObjects = new MazeObject[objects.length];
        int objectCount = 0;
        for (int x = 0; x < objects.length; x++) {
            if (objects[x].isOfType(TypeConstants.TYPE_CONTAINABLE)) {
    ...
    public MazeObject[] getAllProgrammableKeys() {
        final MazeObject[] objects = this.getAllObjects();
        final MazeObject[] tempAllProgrammableKeys = new MazeObject[objects.length];
        int objectCount = 0;
        for (int x = 0; x < objects.length; x++) {
    ...
    public final MazeObject[] getAllRequiredInBattle(final int layer) {
        final MazeObject[] objs = this.getAllObjects();
    ...
    public final MazeObject[] getAllNotRequiredInBattle(final int layer) {
        final MazeObject[] objs = this.getAllObjects();
    ...
    public MazeObject readMazeObject(final XDataReader reader,
        ...
        for (final MazeObject object : objects) {
            try {
                ...
                } else if (formatVersion == FormatConstants.MAZE_FORMAT_2) {
                    o = instance.readMazeObjectV2(reader, UID);
                    if (o != null) {
                        return o;
                    }
                }
    ...
    public MazeObject readSavedMazeObject(final XDataReader reader,
