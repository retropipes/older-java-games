...
import com.puttysoftware.mastermaze.maze.objects.BarrierGenerator;
import com.puttysoftware.mastermaze.maze.objects.CrackedWall;
import com.puttysoftware.mastermaze.maze.objects.Crevasse;
import com.puttysoftware.mastermaze.maze.objects.Dirt;
import com.puttysoftware.mastermaze.maze.objects.Empty;
import com.puttysoftware.mastermaze.maze.objects.EnragedBarrierGenerator;
import com.puttysoftware.mastermaze.maze.objects.ForceField;
import com.puttysoftware.mastermaze.maze.objects.HotRock;
import com.puttysoftware.mastermaze.maze.objects.Ice;
import com.puttysoftware.mastermaze.maze.objects.IcedBarrierGenerator;
...
import com.puttysoftware.mastermaze.maze.objects.PoisonedBarrierGenerator;
import com.puttysoftware.mastermaze.maze.objects.ShockedBarrierGenerator;
import com.puttysoftware.mastermaze.maze.objects.Slime;
...
class LayeredTower implements Cloneable {
            ...
            for (v = y - r; v <= y + r; v++) {
                try {
                    ...
                    if (testName.equals("Monster")) {
                        ...
                        MasterMaze.getApplication().getBattle()
                                .doBattleByProxy();
                        // Respawn it
                        this.generateOneMonster();
                    }
    ...
    public void fillRandomlyInBattle(final Maze map, final int w,
            final MazeObject pass1FillBottom, final MazeObject pass1FillTop) {
        for (int z = 0; z < this.getFloors(); z++) {
    ...
    private void fillFloorRandomlyInBattle(final Maze map, final int z,
            final int w, final MazeObject pass1FillBottom,
            final MazeObject pass1FillTop) {
        // Pre-Pass
        ...
        // Pass 3
        for (int layer = 0; layer < MazeConstants.LAYER_COUNT; layer++) {
            final MazeObject[] requiredObjects = objects
    ...
    public void tickTimers(final int floor) {
        ...
        if (this.timerActive) {
            ...
            if (this.timerValue == 0) {
                ...
                MasterMaze.getApplication().getGameManager().solvedLevel();
            }
        }
    }
    ...
    private static MazeObject[][][] shuffleObjects(
            final MazeObject[][][] preShuffle, final int r, final int opx,
        ...
        for (x = 0; x < lt.getColumns(); x++) {
            for (y = 0; y < lt.getRows(); y++) {
                for (z = 0; z < lt.getFloors(); z++) {
                    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
                        lt.setCell(
                                MasterMaze.getApplication().getObjects()
    ...
    public static LayeredTower readLayeredTowerV2(final XDataReader reader)
            throws IOException {
        int y, x, z, e, mazeSizeX, mazeSizeY, mazeSizeZ;
        mazeSizeX = reader.readInt();
        mazeSizeY = reader.readInt();
