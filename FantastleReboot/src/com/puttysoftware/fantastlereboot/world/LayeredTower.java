...
final class LayeredTower implements Cloneable {
    ...
    public LayeredTower(final int rows, final int cols, final int floors,
            final long randSeed) {
        this.data = new LowLevelWorldDataStore(cols, rows, floors,
                Layers.COUNT);
    ...
    private boolean checkMoveMonster(final int locX, final int locY,
        ...
        if (this.monsterData.getCell(locY, locX, locZ) && !this.monsterData
                .getCell(locY + moveY, locX + moveX, locZ)) {
            return true;
        }
        return false;
    }
        ...
        for (locX = 0; locX < cols; locX++) {
            for (locY = 0; locY < rows; locY++) {
                ...
                if (this.hasMonster(locX, locY, pz)) {
                    if (locX + moveX >= 0 && locX + moveX < cols
                            && locY + moveY >= 0 && locY + moveY < rows) {
                        final FantastleObjectModel there = world.getCell(
                                locX + moveX, locY + moveY, pz, Layers.OBJECT);
                        final boolean checkMove = this.checkMoveMonster(locX,
                                locY, pz, moveX, moveY);
    ...
    public void postBattle(final World world, final int locX, final int locY) {
        ...
        FantastleObjectModel currObj = world.getCell(genX, genY, pz,
                Layers.OBJECT);
    ...
    public boolean cellRangeCheck(final int row, final int col,
            final int floor) {
        int fR = row;
        int fC = col;
        final int fF = floor;
        ...
        return fR >= 0 && fR < this.getRows() && fC >= 0
                && fC < this.getColumns() && fF >= 0 && fF < this.getFloors();
    }
    ...
    public void updateExploredSquares(final int xp, final int yp,
            final int zp) {
        ...
        if (this.visionMode == VisionModes.NONE) {
            ...
            if (this.areCoordsInBounds(x1, y1, x2, y2)) {
                if (result && (this.visionMode
                        | VisionModes.FIELD_OF_VIEW) == this.visionMode) {
                    result = result
                            && this.isSquareVisibleFieldOfView(x1, y1, x2, y2);
                }
                if ((this.visionMode
                        | VisionModes.EXPLORE) == this.visionMode) {
    ...
    private boolean areCoordsInBounds(final int x1, final int y1, final int x2,
        ...
        return fx1 >= -1 && fx1 <= this.getRows() && fx2 >= -1
                && fx2 <= this.getRows() && fy1 >= -1
                && fy1 <= this.getColumns() && fy2 >= -1
                && fy2 <= this.getColumns();
    }
    ...
    private boolean isSquareVisibleFixedRadius(final int x1, final int y1,
        ...
        if (this.cellRangeCheck(fx1, fy1, zLoc)
            ...
            return LayeredTower.radialScan(fx2, fy2, this.visionRadius, fx1,
                    fy1);
        } else {
            return true;
        }
    ...
    private void fillFloor(final FantastleObjectModel bottom,
            ...
            if (objectsWithoutPrerequisites != null) {
                ...
                for (x = 0; x < columns; x++) {
                    for (y = 0; y < rows; y++) {
                        ...
                        if (okay) {
                            this.setCell(
                                    GameObjects.getNewInstanceByUniqueID(
    ...
    private void rectFill(final int x, final int y, final int w, final int h,
            final String tile) {
    ...
    private void endingRoom(final int w) {
        if (w != World.getMaxLevels() - 1) {
            try {
                this.placeIfEmpty(this.lastRoomX + this.lastRoomW - 2,
    ...
    private boolean rectCheck(final int x, final int y, final int w,
            final int h) {
        for (int ya = y; ya < y + h; ya++) {
            for (int xa = x; xa < x + w; xa++) {
                String tile;
    ...
    private void hallMake(final int x, final int y) {
        if (LayeredTower.isEmpty(this.tiles[x - 1][y])
                && LayeredTower.isEmpty(this.tiles[x + 1][y])) {
            this.tiles[x][y] = "H";
            return;
        }
        if (LayeredTower.isEmpty(this.tiles[x][y - 1])
                && LayeredTower.isEmpty(this.tiles[x][y + 1])) {
            this.tiles[x][y] = "H";
            return;
        }
        if (LayeredTower.isEmpty(this.tiles[x - 1][y])) {
        ...
        if (this.rectCheck(x - length, y - 1, length, 3)) {
            ...
            if (length >= 3) {
                ...
                if (sbranch > 1 && sbranch < length - 1) {
                    ...
                    this.hallMakeSouthbound(x - sbranch, y + 1,
                ...
                if (nbranch > 1 && nbranch < length - 1) {
                    ...
                    this.hallMakeNorthbound(x - nbranch, y - 1,
                            this.rand.nextInt(this.hallsize));
                }
            }
        } else {
    ...
    private void hallMakeSouthbound(final int x, final int y,
            final int length) {
    ...
    private void hallMakeNorthbound(final int x, final int y,
            final int length) {
    ...
    private void roomMakeSouthbound(final int x, final int y, final int w,
            final int h) {
        final int wc = w / 2;
        final int hc = h / 2;
    ...
    private void roomMakeEastbound(final int x, final int y, final int w,
            final int h) {
        final int wc = w / 2;
        final int hc = h / 2;
        final int xorig = x + 1;
    ...
    private void roomMakeWestbound(final int x, final int y, final int w,
            final int h) {
        final int hc = h / 2;
        final int wc = w / 2;
        final int xorig = x - w;
    ...
    private void roomMakeNorthbound(final int x, final int y, final int w,
            final int h) {
        final int wc = w / 2;
        final int hc = h / 2;
        final int xorig = x - wc;
    ...
    private void convertTilestoObjects(final int z) {
        ...
        final FantastleObjectModel[] roomReplacers = new FantastleObjectModel[] {
                new ArmorShop(), new BankShop(), new BonusShop(),
                new ElementalShop(), new HealShop(), new ItemShop(),
                new NecklaceShop(), new RegenerateShop(), new SpellShop(),
                new WeaponShop(), new Pit(), new SuperPit(), new Spring(),
                new SuperSpring(), open, open, open, open, open, open, open,
        ...
        for (int x = 0; x < this.size.width; x++) {
            for (int y = 0; y < this.size.height; y++) {
                ...
                if ("#".equals(tile)) {
                    ...
                    final int rIndex = RandomRange.generate(0,
                            roomReplacers.length - 1);
    ...
    private void addMonstersRandomly(final int z) {
        ...
        for (int y = 0; y < generateHowMany; y++) {
            ...
            FantastleObjectModel there = this.getCell(xLoc, yLoc, z,
                    Layers.OBJECT);
                ...
                for (z = 0; z < sizeZ; z++) {
                    for (e = 0; e < Layers.COUNT; e++) {
                        this.savedTowerState.setCell(
                                GameObjects.readObject(reader, formatVersion),
                                y, x, z, e);
                    }
                }
            }
