...
public final class WorldFileManager {
    ...
    private static void loadWorldFile(final String filename) {
        if (!FilenameChecker
                .isFilenameOK(WorldFileManager.getNameWithoutExtension(
                        WorldFileManager.getFileNameOnly(filename)))) {
    ...
    public static boolean saveWorld() {
        ...
        while (!FilenameChecker.isFilenameOK(returnVal)) {
            returnVal = CommonDialogs.showTextInputDialog("Name?",
                    "Save World");
            if (returnVal != null) {
                extension = FileExtensions.getWorldExtensionWithPeriod();
                final File file = new File(WorldFileManager.getWorldDirectory()
                        + returnVal + extension);
                filename = file.getAbsolutePath();
                if (!FilenameChecker.isFilenameOK(returnVal)) {
                    ...
                    if (!file.getParentFile().exists()) {
                        ...
                        if (!okay) {
                            FantastleReboot.exception(new IOException(
