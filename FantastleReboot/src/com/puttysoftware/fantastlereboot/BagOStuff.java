...
public class BagOStuff {
    ...
    void postConstruct() {
        try {
            BagOStuff.pd = new ProductData(BagOStuff.UPDATE_SITE,
                    BagOStuff.NEW_VERSION_SITE, BagOStuff.VERSION_MAJOR,
    ...
    public static void checkForUpdates(final boolean manual) {
        try {
            final UpdateCheckResults results = Prefs.checkForUpdates(manual,
                    BagOStuff.pd);
            if (results.hasUpdate()) {
                final int resp = CommonDialogs.showConfirmDialog(
            ...
            if (manual) {
                CommonDialogs.showDialog(
                        "The check for updates could not be completed,\n"
