...
public class MapBattleLogic extends Battle {
    ...
    public BattleResults getResult() {
        final MapBattleDefinitions.BattleState state = this.mbd
                .getBattleState();
    ...
    public void arrowDone(final BattleCharacter hit) {
        ...
        if (hit != null && !hit.getCreature().isAlive()) {
            ...
            this.mbd.getBattleWorld().setCell(new OpenSpace(), hit.getX(),
        ...
        if (next != null && nextGround != null && currGround != null) {
            if (!next.isSolid()) {
                if (useAP && this.getActiveActionCounter() >= AIContext
                    ...
                    if (this.mbd.getBattleWorld().cellRangeCheck(px - 1, py - 1,
                            0)) {
                        obj1 = m.getCell(px - 1, py - 1, 0, Layers.OBJECT);
                    }
                    if (this.mbd.getBattleWorld().cellRangeCheck(px, py - 1,
                            0)) {
                    ...
                    if (this.mbd.getBattleWorld().cellRangeCheck(px - 1, py,
                            0)) {
                    ...
                    if (!this.mbd.getActiveCharacter().getCreature()
                            .hasMapAI()) {
    ...
    public boolean castSpell() {
        ...
        if (activeBC.canAct(Battle.AP_CAST_SPELL)) {
            final BattleCharacter anyEnemy = this.mbd
    ...
    public boolean steal() {
        ...
        if (activeBC.canAct(Battle.AP_STEAL)) {
            ...
            final int stealChance = StatConstants.CHANCE_STEAL
                    + active.getJob().getAttribute(
                            JobConstants.ATTRIBUTE_STEAL_SUCCESS_MODIFIER);
            ...
            if (randomChance <= stealChance) {
                ...
                if (stealAmount == 0) {
                    ...
                    this.setStatusMessage(activeBC.getName()
                            + " tries to steal, and successfully stole "
                            + stealAmount + " Gold!");
                    return true;
                }
            } else {
    ...
    public boolean drain() {
        ...
        if (activeBC.canAct(Battle.AP_DRAIN)) {
            ...
            drainChance = StatConstants.CHANCE_DRAIN
                    + active.getJob().getAttribute(
                            JobConstants.ATTRIBUTE_DRAIN_SUCCESS_MODIFIER);
    ...
    public void doResult(final BattleResults result) {
        ...
        if (!this.resultDoneAlready) {
            ...
            final boolean bossFlag = this.mbd
            ...
            if (result == BattleResults.WON) {
                ...
                CommonDialogs.showTitledDialog(
                        ...
                        "Perfect Victory!");
                final long pBattleExp = Prefs
                        .getPerfectBattleExp(this.battleExp);
                final int pBattleGold = Prefs
                        .getPerfectBattleGold(this.battleGold);
                ...
                CommonDialogs.showTitledDialog(
                        "The party has been annihilated!", "Annihilation!");
