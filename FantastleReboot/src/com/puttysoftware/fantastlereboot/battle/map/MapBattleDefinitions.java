        ...
        for (final BattleCharacter battler : this.battlers) {
            if (battler != null) {
                if (battler.getCreature().isAlive()
                        && !battler.isLocationSet()) {
                    rx = randX.generate();
                    ry = randY.generate();
                    FantastleObjectModel obj = this.battleWorld.getCell(rx, ry,
                            0, Layers.OBJECT);
                    while (obj.isSolid()) {
                        rx = randX.generate();
                        ry = randY.generate();
                        obj = this.battleWorld.getCell(rx, ry, 0,
                                Layers.OBJECT);
                    }
                    battler.setX(rx);
                    battler.setY(ry);
    ...
    void maintainEffects() {
        for (final BattleCharacter battler : this.battlers) {
            if (battler != null && battler.isActive()
                ...
                if (creature.isAlive()
                        && creature.getTeamID() == Creature.TEAM_PARTY
                        && creature.getCurrentHP() <= creature.getMaximumHP()
                                * 3 / 10) {
    ...
    boolean setNextActive() {
        ...
        for (final BattleCharacter battler : this.battlers) {
            if (battler != null && battler.isActive()
                ...
                final Creature creature = battler.getCreature();
                final int creatureSpeed = (int) creature
