...
class MapBattleGUI {
        ...
        for (x = xView; x <= xlView; x++) {
            for (y = yView; y <= ylView; y++) {
                ...
                if (mbd.getBattleWorld().cellRangeCheck(y, x, 0)) {
                    final FantastleObjectModel obj1 = mbd.getBattleWorld()
                            .getCell(y, x, 0, Layers.GROUND);
                    final FantastleObjectModel obj2 = mbd.getBattleWorld()
                            .getCell(y, x, 0, Layers.OBJECT);
                    final String cacheName = MapBattleGUI
                            .generateCacheName(obj1, obj2);
                    final BufferedImageIcon icon1 = obj1.getBattleImage();
                    final BufferedImageIcon icon2 = obj2.getBattleImage();
                    this.drawGrid.setImageCell(
                            ImageCompositor.composite(cacheName, icon1, icon2),
                            xFix, yFix);
                } else {
    ...
    void redrawOneBattleSquare(final MapBattleDefinitions mbd, final int x,
        ...
        if (mbd.getBattleWorld().cellRangeCheck(y, x, 0)) {
            ...
            this.drawGrid.setImageCell(
                    ImageCompositor.composite(cacheName, icon1, icon2, icon3),
                    xFix, yFix);
            this.battlePane.repaint();
        }
        this.battleFrame.pack();
