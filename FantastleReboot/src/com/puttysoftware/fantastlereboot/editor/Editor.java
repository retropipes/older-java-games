...
public class Editor {
    ...
    private static void redrawGround() {
        ...
        Editor.outputFrame.pack();
        Editor.outputFrame.setTitle("Editor (Ground Layer) - Floor " + (z + 1)
                + " Level " + (w + 1));
    }
    ...
    private static void redrawGroundAndObjects() {
        ...
        Editor.secondaryPane.repaint();
        Editor.outputFrame.pack();
        Editor.outputFrame.setTitle("Editor (Object Layer) - Floor " + (z + 1)
                + " Level " + (w + 1));
    }
    ...
    public static void editObject(final int x, final int y) {
        ...
        final int gridX = x / ImageConstants.SIZE + EditorView.getLocX()
                - xOffset + yOffset;
        final int gridY = y / ImageConstants.SIZE + EditorView.getLocY()
                + xOffset - yOffset;
    ...
    private static void checkStairPair(final int z, final int w) {
        final FantastleObjectModel obj1 = WorldManager.getWorld().getCell(
                EditorLoc.getLocX(), EditorLoc.getLocY(), z, Layers.OBJECT);
    ...
    private static void pairStairs(final int type, final int z, final int w) {
        ...
        case STAIRS_UP:
            if (world.cellRangeCheck(locX, locY, z + 1)) {
                world.setCell(new StairsDown(), locX, locY, z + 1,
                        Layers.OBJECT);
            }
            break;
        case STAIRS_DOWN:
    ...
    private static void unpairStairs(final int type, final int z, final int w) {
        ...
        case STAIRS_UP:
            if (world.cellRangeCheck(locX, locY, z + 1)) {
                world.setCell(new OpenSpace(), locX, locY, z + 1,
                        Layers.OBJECT);
            }
            break;
        case STAIRS_DOWN:
            ...
            if (input2 != null) {
                ...
                if (input3 != null) {
                    try {
                        ...
                        success = WorldManager.getWorld().addLevel(levelSizeX,
                                levelSizeY, levelSizeZ);
                        if (success) {
                            Editor.fixLimits();
                            if (!flag) {
                                EditorView.setLocX(
                                        0 - (EditorView.getSizeX() - 1) / 2);
                                EditorView.setLocY(
                                        0 - (EditorView.getSizeY() - 1) / 2);
                            }
                            Editor.checkMenus();
                        }
    ...
    public static void undo() {
        ...
        EditorLoc.setLocX(x);
        EditorLoc.setLocY(y);
        if (Editor.engine.isDataValid()
                && world.cellRangeCheck(x, y, z, w, e)) {
