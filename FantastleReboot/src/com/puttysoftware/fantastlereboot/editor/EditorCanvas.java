    ...
    public void paint(final Graphics g) {
        ...
        for (x = viewX; x <= viewLRX; x++) {
            for (y = viewY; y <= viewLRY; y++) {
                ...
                if (inBounds) {
                    final FantastleObjectModel obj1 = m.getCell(y, x, z,
                            Layers.GROUND);
                    ...
                    if (EditorCanvas.MAX_LAYER >= Layers.OBJECT) {
                        final FantastleObjectModel obj2 = m.getCell(y, x, z,
                                Layers.OBJECT);
                        final BufferedImageIcon img2 = obj2.getEditorImage();
                        final String cacheName = EditorCanvas
                                .generateCacheName(obj1, obj2, obj3, obj4);
                        this.drawGrid.setImageCell(ImageCompositor.composite(
                                cacheName, img1, img2, img3, img4), xFix, yFix);
                    } else {
                        final String cacheName = EditorCanvas
                                .generateCacheName(obj1, obj3, obj4);
                        this.drawGrid.setImageCell(ImageCompositor.composite(
                                cacheName, img1, img3, img4), xFix, yFix);
                    }
                } else {
                    this.drawGrid.setImageCell(
                            EditorCanvas.NOTHING.getEditorImage(), xFix, yFix);
                }
            }
        }
