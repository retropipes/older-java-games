...
public class EffectManager {
    ...
    public static void initialize() {
        ...
        EffectManager.activeEffects[EffectConstants.EFFECT_CONFUSED] = new Confused(
                0);
        EffectManager.activeEffects[EffectConstants.EFFECT_DIZZY] = new Dizzy(
                0);
        EffectManager.activeEffects[EffectConstants.EFFECT_DRUNK] = new Drunk(
                0);
        EffectManager.activeEffects[EffectConstants.EFFECT_STICKY] = new Sticky(
                0);
    ...
    private static void clearGridEntry(final int effectID) {
        ...
        if (index != -1) {
            ...
            for (int z = index; z < EffectManager.MAX_ACTIVE_EFFECTS - 1; z++) {
                EffectManager.activeEffectMessages[z].setText(
                        EffectManager.activeEffectMessages[z + 1].getText());
                EffectManager.activeEffectIndices[z] = EffectManager.activeEffectIndices[z
                        + 1];
            }
            // Clear last entry
            EffectManager
                    .clearGridEntryText(EffectManager.MAX_ACTIVE_EFFECTS - 1);
        }
    }
