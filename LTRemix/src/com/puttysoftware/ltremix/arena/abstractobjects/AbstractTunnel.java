...
public abstract class AbstractTunnel extends AbstractArenaObject {
    ...
    private static void checkTunnelsOfColor(final int color) {
        ...
        final String targetName = ColorResolver.resolveColorConstantToName(
                color) + StringConstants.COMMON_STRING_SPACE
                + StringLoader.loadString(StringConstants.GENERIC_STRINGS_FILE,
                        StringConstants.GENERIC_STRING_TUNNEL);
        final int[] pgrmdest = app.getArenaManager().getArena().tunnelScan(0, 0,
                0, targetName);
        if (pgrmdest != null) {
    ...
    public void postMoveAction(final int dirX, final int dirY, final int dirZ) {
        ...
        final int[] pgrmdest = app.getArenaManager().getArena().tunnelScan(dirX,
                dirY, dirZ, targetName);
        if (pgrmdest != null) {
            app.getGameManager().updatePositionAbsoluteNoEvents(pgrmdest[0],
                    pgrmdest[1], pgrmdest[2]);
        }
    }
    ...
    public boolean pushIntoAction(final AbstractMovableObject pushed,
        ...
        final Application app = LTRemix.getApplication();
        final String targetName = ColorResolver
                .resolveColorConstantToName(this.getColor())
                + StringConstants.COMMON_STRING_SPACE + this.getBaseName();
        final int color = this.getColor();
        final int[] pgrmdest = app.getArenaManager().getArena().tunnelScan(x, y,
                z, targetName);
        if (pgrmdest != null) {
            AbstractTunnel.tunnelsFull[color] = false;
            app.getGameManager().updatePushedIntoPositionAbsolute(pgrmdest[0],
                    pgrmdest[1], pgrmdest[2], x, y, z, pushed, this);
        } else {
            AbstractTunnel.tunnelsFull[color] = true;
