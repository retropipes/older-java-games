...
public abstract class AbstractButton extends AbstractArenaObject {
    ...
    // Constructors
    protected AbstractButton(final AbstractButtonDoor bd,
    ...
    public int hashCode() {
        ...
        result = prime * result
                + (this.buttonDoor == null ? 0 : this.buttonDoor.hashCode());
    ...
    public boolean boundButtonDoorEquals(final AbstractButton other) {
        ...
        } else if (!this.buttonDoor.getClass()
                .equals(other.buttonDoor.getClass())) {
            return false;
        }
        return true;
    }
    ...
    public boolean editorPlaceHook(final int x, final int y, final int z) {
        ...
        final int[] loc = app.getArenaManager().getArena().findObject(z,
                this.getButtonDoor().getBaseName());
        if (loc != null) {
            this.setDoorX(loc[0]);
            this.setDoorY(loc[1]);
            this.setTriggered(false);
        }
        if (this instanceof AbstractTriggerButton
                || this instanceof AbstractPressureButton) {
            app.getArenaManager().getArena().fullScanButtonCleanup(x, y, z,
                    this);
        }
    ...
    @Override
    public abstract boolean pushIntoAction(final AbstractMovableObject pushed,
            final int x, final int y, final int z);

    @Override
    public abstract void pushOutAction(final AbstractMovableObject pushed,
            final int x, final int y, final int z);
