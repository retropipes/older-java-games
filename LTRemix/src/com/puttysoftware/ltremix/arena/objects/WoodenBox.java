...
public class WoodenBox extends AbstractMovableObject {
    ...
    public int laserEnteredAction(final int locX, final int locY,
        ...
        if (forceUnits >= this.getMinimumReactionForce()) {
            final AbstractArenaObject mof = app.getArenaManager().getArena()
                    .getCell(locX + dirX, locY + dirY, locZ,
                            this.getPrimaryLayer());
            final AbstractArenaObject mor = app.getArenaManager().getArena()
                    .getCell(locX - dirX, locY - dirY, locZ,
                            this.getPrimaryLayer());
            if (laserType == LaserTypeConstants.LASER_TYPE_MISSILE) {
                ...
                app.getGameManager().morph(new Empty(), locX, locY, locZ,
                    ...
                    && (mor.isOfType(TypeConstants.TYPE_CHARACTER)
                            || !mor.isSolid())) {
                app.getGameManager().updatePushedPosition(locX, locY,
                        locX - dirX, locY - dirY, this);
                this.playSoundHook();
            } else if (mof != null
