...
import com.puttysoftware.brainmaze.editor.rulesets.RuleSet;
...
public abstract class MazeObject implements TypeConstants, RandomGenerationRule {
	...
	public MazeObject(final boolean isSolid, final boolean canBeInventoried, final boolean sightBlock) {
		this.solid = isSolid;
		this.pushable = false;
		this.pushableInto = false;
		this.pushableOut = false;
	...
	public MazeObject clone() {
		try {
			final MazeObject copy = (MazeObject) super.clone();
	...
	public void pushFailedAction(final ObjectInventory inv, final int x, final int y, final int pushX,
		...
		BrainMaze.getApplication().getGameManager().keepNextMessage();
		BrainMaze.getApplication().showMessage("Can't push that");
	}
	...
	public void pullFailedAction(final ObjectInventory inv, final int x, final int y, final int pullX,
		...
		SoundManager.playSound(SoundConstants.SOUND_CATEGORY_SOLVING_MAZE, SoundConstants.SOUND_ACTION_FAILED);
		BrainMaze.getApplication().getGameManager().keepNextMessage();
		BrainMaze.getApplication().showMessage("Can't pull that");
	}
