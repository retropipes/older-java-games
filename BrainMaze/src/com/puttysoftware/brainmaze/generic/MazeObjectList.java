...
import com.puttysoftware.brainmaze.maze.FormatConstants;
import com.puttysoftware.brainmaze.maze.MazeConstants;
import com.puttysoftware.brainmaze.objects.*;
...
public class MazeObjectList {
	...
	public MazeObjectList() {
		final MazeObject[] allObjects = { new Empty(), new Grass(), new Dirt(), new Sand(), new Snow(), new Tundra(),
				new Tile(), new Ice(), new Water(), new HotRock(), new Slime(), new Lava(), new SunkenBlock(),
				new BridgedLavaHorizontal(), new BridgedLavaVertical(), new BridgedSlimeHorizontal(),
				new BridgedSlimeVertical(), new BridgedWaterHorizontal(), new BridgedWaterVertical(), new Player(),
				new SunStone(), new MoonStone(), new Wall(), new InvisibleWall(), new FakeWall(), new DamageableWall(),
				new CrackedWall(), new DamagedWall(), new CrumblingWall(), new BreakableWallHorizontal(),
				new BreakableWallVertical(), new BlueWallOff(), new BlueWallOn(), new GreenWallOff(), new GreenWallOn(),
				...
				new Axe(), new Tree(), new CutTree(), new Key(), new Lock(), new BlueKey(), new BlueLock(),
				new GreenKey(), new GreenLock(), new MagentaKey(), new MagentaLock(), new OrangeKey(), new OrangeLock(),
				new PurpleKey(), new PurpleLock(), new RedKey(), new RedLock(), new RoseKey(), new RoseLock(),
				new SeaweedKey(), new SeaweedLock(), new SkyKey(), new SkyLock(), new WhiteKey(), new WhiteLock(),
				new YellowKey(), new YellowLock(), new CyanKey(), new CyanLock(), new BlueCrystal(),
				new BlueCrystalWall(), new GreenCrystal(), new GreenCrystalWall(), new MagentaCrystal(),
				new MagentaCrystalWall(), new OrangeCrystal(), new OrangeCrystalWall(), new PurpleCrystal(),
				new PurpleCrystalWall(), new RedCrystal(), new RedCrystalWall(), new RoseCrystal(),
				new RoseCrystalWall(), new SeaweedCrystal(), new SeaweedCrystalWall(), new SkyCrystal(),
				new SkyCrystalWall(), new WhiteCrystal(), new WhiteCrystalWall(), new YellowCrystal(),
				new YellowCrystalWall(), new CyanCrystal(), new CyanCrystalWall(), new Door(), new SunDoor(),
				new MoonDoor(), new BlueButton(), new GreenButton(), new MagentaButton(), new OrangeButton(),
				new PurpleButton(), new RedButton(), new RoseButton(), new SeaweedButton(), new SkyButton(),
				new WhiteButton(), new YellowButton(), new CyanButton(), new Teleport(), new InvisibleTeleport(),
				...
				new InvisibleConditionalChainTeleport(), new BlockTeleport(), new InvisibleBlockTeleport(),
				new StairsUp(), new StairsDown(), new Pit(), new InvisiblePit(), new Springboard(),
				new InvisibleSpringboard(), new PushableBlock(), new PullableBlock(), new PushablePullableBlock(),
				new NoBoots(), new GlueBoots(), new AquaBoots(), new BioHazardBoots(), new FireBoots(), new HotBoots(),
				new PasswallBoots(), new SlipperyBoots(), new EmptyVoid(), new Sign(), new NoBlock(), new NoPlayer() };
	...
	public MazeObject[] getAllObjects() {
		return this.allObjectList.toArray(new MazeObject[this.allObjectList.size()]);
	}
	...
	public BufferedImageIcon[] getAllEditorAppearances() {
		...
		for (int x = 0; x < allEditorAppearances.length; x++) {
			allEditorAppearances[x] = GraphicsManager.getTransformedImage(objects[x].getName(),
					objects[x].getBaseName(), objects[x].getTemplateColor(), objects[x].getAttributeName(),
	...
	public BufferedImageIcon[] getAllGroundLayerEditorAppearances() {
		...
		for (int x = 0; x < objects.length; x++) {
			if (objects[x].getLayer() == MazeConstants.LAYER_GROUND) {
				tempAllGroundLayerEditorAppearances[x] = GraphicsManager.getTransformedImage(objects[x].getName(),
						objects[x].getBaseName(), objects[x].getTemplateColor(), objects[x].getAttributeName(),
						objects[x].getAttributeTemplateColor());
			}
		}
	...
	public BufferedImageIcon[] getAllObjectLayerEditorAppearances() {
		...
		for (int x = 0; x < objects.length; x++) {
			if (objects[x].getLayer() == MazeConstants.LAYER_OBJECT) {
				tempAllObjectLayerEditorAppearances[x] = GraphicsManager.getTransformedImage(objects[x].getName(),
						objects[x].getBaseName(), objects[x].getTemplateColor(), objects[x].getAttributeName(),
						objects[x].getAttributeTemplateColor());
			}
		}
	...
	public MazeObject[] getAllInventoryableObjectsMinusSpecial() {
		...
		int objectCount = 0;
		for (int x = 0; x < objects.length; x++) {
			if (objects[x].isInventoryable() && !objects[x].isOfType(TypeConstants.TYPE_BOOTS)) {
	...
	public final MazeObject getNewInstanceByName(final String name) {
		...
		if (instance == null) {
			...
			try {
				...
				BrainMaze.getErrorLogger().logError(e);
				return null;
			}
		}
	}
	...
	public MazeObject readMazeObject(final XDataReader reader, final int formatVersion) throws IOException {
		...
		MazeObject o = null;
		String UID = "";
		if (formatVersion == FormatConstants.MAZE_FORMAT_1 || formatVersion == FormatConstants.MAZE_FORMAT_2
