...
import com.puttysoftware.brainmaze.objects.HotBoots;
import com.puttysoftware.brainmaze.objects.MoonStone;
import com.puttysoftware.brainmaze.objects.PasswallBoots;
import com.puttysoftware.brainmaze.objects.SlipperyBoots;
import com.puttysoftware.brainmaze.objects.SunStone;
import com.puttysoftware.brainmaze.objects.Wall;
...
public class GameLogicManager {
            ...
            if (proceed) {
                ...
                try {
                    if (this.checkSolid(this.savedMazeObject, below, nextBelow,
                            nextAbove)) {
                        redrawsSuspended = this.updatePositionRelativePull(
                                dirX, dirY);
                    } else {
                        redrawsSuspended = this.updatePositionRelativePush(
                                dirX, dirY);
                    }
                } catch (final ArrayIndexOutOfBoundsException ae) {
    ...
    public void updatePositionAbsolute(final int x, final int y, final int z) {
        ...
        try {
            ...
            m.restorePlayerLocation();
            this.vwMgr.restoreViewingWindow();
            BrainMaze.getApplication().showMessage("Can't go outside the maze");
        } catch (final NullPointerException np) {
            m.restorePlayerLocation();
            this.vwMgr.restoreViewingWindow();
            BrainMaze.getApplication().showMessage("Can't go outside the maze");
    ...
    public void controllableTeleport() {
        ...
        BrainMaze.getApplication().showMessage("Click to set destination");
    }
    ...
    public void identifyObject(final int x, final int y) {
        ...
        try {
            ...
            BrainMaze.getApplication().showMessage(ev.getGameName());
            SoundManager.playSound(SoundConstants.SOUND_CATEGORY_SOLVING_MAZE,
                    SoundConstants.SOUND_IDENTIFY);
        }
    }
