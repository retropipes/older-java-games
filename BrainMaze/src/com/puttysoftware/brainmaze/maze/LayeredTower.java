...
import com.puttysoftware.brainmaze.generic.RandomGenerationRule;
import com.puttysoftware.brainmaze.objects.Empty;
import com.puttysoftware.brainmaze.objects.HotRock;
        ...
        for (x = 0; x < lt.getColumns(); x++) {
            for (y = 0; y < lt.getRows(); y++) {
                for (z = 0; z < lt.getFloors(); z++) {
                    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
                        lt.setCell(
                                BrainMaze.getApplication().getObjects()
                                        .readMazeObject(reader, ver),
                                y, x, z, e);
