...
public class World {
	...
	public void addAtEmptyLocation(final Item item, final int newDepth) {
		int x;
		int y;
		do {
			x = (int) (Math.random() * this.width);
			y = (int) (Math.random() * this.height);
		} while (!this.tile(x, y, newDepth).isGround() || this.item(x, y, newDepth) != null);
		this.items[x][y][newDepth] = item;
	}
