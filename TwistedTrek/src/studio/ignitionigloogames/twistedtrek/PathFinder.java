...
public class PathFinder {
	...
	private int totalCost(final Point from, final Point to) {
		...
		final int cost = this.costToGetTo(from) + PathFinder.heuristicCost(from, to);
	...
	private ArrayList<Point> createPath(final Point start, final Point end) {
		Point computedEnd = end;
		final ArrayList<Point> path = new ArrayList<>();
		while (!computedEnd.equals(start)) {
			path.add(computedEnd);
			computedEnd = this.parents.get(computedEnd);
		}
