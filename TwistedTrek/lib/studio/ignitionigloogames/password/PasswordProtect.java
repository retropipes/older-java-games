...
public class PasswordProtect {
	...
	public static void setPassword(final XDataWriter passwordFile) {
		...
		PasswordProtect.passwordLabel.setText("Type the new password below (it will be hidden as you type):");
	...
	public static void promptForPassword(final XDataReader passwordFile) {
		...
		PasswordProtect.passwordLabel.setText("Type the password below (it will be hidden as you type):");
		PasswordProtect.passwordField.setText(null);
		PasswordProtect.passwordFrame.pack();
	...
	private static String hashPassword() {
		try {
			...
			final byte[] bytes = Arrays.toString(pw).getBytes("UTF-8");
	...
	private static void createComponents() {
		if (PasswordProtect.passwordFrame == null) {
			...
			PasswordProtect.passwordFrame.getRootPane().setDefaultButton(PasswordProtect.okButton);
			...
			PasswordProtect.passwordFrame.getContentPane().add(PasswordProtect.passwordLabel, BorderLayout.NORTH);
			PasswordProtect.passwordFrame.getContentPane().add(PasswordProtect.passwordField, BorderLayout.CENTER);
			PasswordProtect.passwordFrame.getContentPane().add(PasswordProtect.buttonPanel, BorderLayout.SOUTH);
			PasswordProtect.passwordFrame.pack();
		}
	}
}