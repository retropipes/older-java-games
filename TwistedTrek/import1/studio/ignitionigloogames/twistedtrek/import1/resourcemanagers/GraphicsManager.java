...
public class GraphicsManager {
	...
	static BufferedImageIcon getUncachedImage(final String name) {
		try {
			...
			final URL url = GraphicsManager.class.getResource(
					"/net/worldwizard/fantastle5/resources/graphics/" + dm + "/objects/" + normalName + ".png");
	...
	public static BufferedImageIcon getStatImage(final String name) {
		try {
			...
			final String normalName = GraphicsManager.normalizeName(name);
			final URL url = GraphicsManager.class.getResource(
					"/net/worldwizard/fantastle5/resources/graphics/" + dm + "/stats/" + normalName + ".png");
			final BufferedImage image = ImageIO.read(url);
			final BufferedImageIcon icon = new BufferedImageIcon(image);
			...
			for (int x = 0; x < GraphicsManager.getGraphicSize(); x++) {
				for (int y = 0; y < GraphicsManager.getGraphicSize(); y++) {
					...
					if (c.equals(GraphicsManager.TRANSPARENT)) {
						result.setRGB(x, y, GraphicsManager.REPLACE_STAT.getRGB());
					}
				}
			}
			return result;
			...
			if (icon != null) {
				for (int x = 0; x < GraphicsManager.getGraphicSize(); x++) {
					for (int y = 0; y < GraphicsManager.getGraphicSize(); y++) {
						...
						final Color c = new Color(pixel);
						if (c.equals(GraphicsManager.TRANSPARENT)) {
							result.setRGB(x, y, GraphicsManager.REPLACE.getRGB());
						}
					}
				}
				return result;
	...
	public static BufferedImageIcon getVirtualCompositeImage(final String name1, final String name2,
			final String name3) {
		try {
			final BufferedImageIcon icon3 = ImageCache.getCachedImage(name3);
			final BufferedImageIcon icon2 = GraphicsManager.getCompositeImage(name1, name2);
	...
	public static BufferedImageIcon getLogo() {
		try {
			...
			final URL url = GraphicsManager.class
					.getResource("/net/worldwizard/fantastle5/resources/graphics/" + dm + "/logo/logo.png");
			final BufferedImage image = ImageIO.read(url);
			final BufferedImageIcon icon = new BufferedImageIcon(image);
			return icon;
		} catch (final IOException ie) {
	...
	public static BufferedImageIcon getMiniatureLogo() {
		try {
			...
			final URL url = GraphicsManager.class
					.getResource("/net/worldwizard/fantastle5/resources/graphics/" + dm + "/logo/minilogo.png");
			final BufferedImage image = ImageIO.read(url);
			final BufferedImageIcon icon = new BufferedImageIcon(image);
			return icon;
		} catch (final IOException ie) {
	...
	public static BufferedImageIcon getMicroLogo() {
		try {
			...
			final URL url = GraphicsManager.class
					.getResource("/net/worldwizard/fantastle5/resources/graphics/" + dm + "/logo/micrologo.png");
			final BufferedImage image = ImageIO.read(url);
			final BufferedImageIcon icon = new BufferedImageIcon(image);
			return icon;
		} catch (final IOException ie) {
	...
	private static BufferedImageIcon getBossTemplate() {
		try {
			...
			final URL url = GraphicsManager.class
					.getResource("/net/worldwizard/fantastle5/resources/graphics/" + dm + "/boss/boss.png");
			final BufferedImage image = ImageIO.read(url);
			final BufferedImageIcon icon = new BufferedImageIcon(image);
			return icon;
		} catch (final IOException ie) {
	...
	public static BufferedImageIcon getMonsterImage(final String name, final Element element) {
		final BufferedImageIcon template = MonsterImageCache.getCachedMonsterImage(name);
		final BufferedImageIcon templateOut = new BufferedImageIcon(MonsterImageCache.getCachedMonsterImage(name));
		if (template != null) {
	...
	private static BufferedImage getMonsterTemplate(final String name) {
		try {
			...
			final String normalName = GraphicsManager.normalizeName(name);
			final URL url = GraphicsManager.class.getResource(
					"/net/worldwizard/fantastle5/resources/graphics/" + dm + "/monsters/" + normalName + ".png");
			final BufferedImage image = ImageIO.read(url);
			if (image != null) {
				return image;
			} else {
