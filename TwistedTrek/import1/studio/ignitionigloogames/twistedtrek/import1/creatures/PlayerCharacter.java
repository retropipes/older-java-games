...
import studio.ignitionigloogames.twistedtrek.import1.spells.SpellBook;
...
public class PlayerCharacter extends Creature {
	...
	// Constructors
	public PlayerCharacter(final Race r, final Caste c, final Faith f, final Gender g, final Personality p) {
		super();
		this.race = r;
		this.caste = c;
		this.gender = g;
		...
		this.toNextLevel = this.getExpToNextLevel(this.getLevel() + 1, this.kills);
		this.monsterLevel = this.getLevel();
		this.setSpellBook(SpellBookManager.getSpellBookByID(this.caste.getCasteID()));
	}

	public PlayerCharacter(final Race r, final Caste c, final Faith f, final Gender g, final Personality p,
			final int pAtk, final int pDef, final int pHP, final int pMP, final int k) {
		super();
		this.race = r;
		this.caste = c;
		this.gender = g;
		...
		this.setIntelligence(StatConstants.GAIN_INTELLIGENCE
				+ this.race.getAttribute(RaceConstants.RACE_ATTRIBUTE_INTELLIGENCE_PER_LEVEL) + this.mINT
				+ Math.min(0, this.rINT.generate()));
		this.setAgility(
				StatConstants.GAIN_AGILITY + this.race.getAttribute(RaceConstants.RACE_ATTRIBUTE_AGILITY_PER_LEVEL)
	...
	public int getAttack() {
		return super.getAttack() + this.getWeaponPower() + this.getPermanentAttack()
				+ this.caste.getAttribute(CasteConstants.CASTE_ATTRIBUTE_BONUS_ATTACK);
	}

	@Override
	public int getDefense() {
		return super.getDefense() + this.getArmorBlock() + this.getPermanentDefense()
	...
	public int getPermanentHP() {
		return this.permanentHP * this.caste.getAttribute(CasteConstants.CASTE_ATTRIBUTE_PERMANENT_HP_PER_POINT);
	}
	...
	public int getPermanentMP() {
		return this.permanentMP * this.caste.getAttribute(CasteConstants.CASTE_ATTRIBUTE_PERMANENT_MP_PER_POINT);
	}
	...
	public void loadPlayer(final int pAttack, final int pDefense, final int pHP, final int pMP, final int newKills,
			final int newLevel, final int chp, final int cmp, final int newGold, final int newBank,
			final long newExperience, final int ml, final int bookID, final boolean[] known) {
