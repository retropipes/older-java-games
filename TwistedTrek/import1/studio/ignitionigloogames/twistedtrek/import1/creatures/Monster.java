...
import studio.ignitionigloogames.twistedtrek.import1.items.Equipment;
import studio.ignitionigloogames.twistedtrek.import1.items.EquipmentFactory;
import studio.ignitionigloogames.twistedtrek.import1.items.EquipmentSlotConstants;
import studio.ignitionigloogames.twistedtrek.import1.items.Shop;
import studio.ignitionigloogames.twistedtrek.import1.resourcemanagers.GraphicsManager;
import studio.ignitionigloogames.twistedtrek.import1.resourcemanagers.MonsterNames;
...
public class Monster extends Creature {
	...
	private int getInitialStrength() {
		final PlayerCharacter playerCharacter = PCManager.getPlayer();
		final RandomRange r1 = new RandomRange(0, playerCharacter.getPermanentAttack());
		final RandomRange r2 = new RandomRange(0, this.getLevel() + playerCharacter.getStrength());
		return (int) (r1.generate() + r2.generate() * this.adjustForLevelDifference());
	}

	private int getInitialBlock() {
		final PlayerCharacter playerCharacter = PCManager.getPlayer();
		final RandomRange r1 = new RandomRange(0, playerCharacter.getPermanentDefense());
		final RandomRange r2 = new RandomRange(0, this.getLevel() + playerCharacter.getBlock());
		return (int) (r1.generate() + r2.generate() * this.adjustForLevelDifference());
	}
	...
	private long getInitialExperience() {
		...
		final long toNext = playerCharacter.getExpToNextLevel(playerCharacter.getLevel() + 1,
				playerCharacter.getKills());
		final long toCurrent = playerCharacter.getExpToNextLevel(playerCharacter.getLevel(),
				playerCharacter.getKills());
	...
	private int getInitialGold() {
		final PlayerCharacter playerCharacter = PCManager.getPlayer();
		final int needed = Shop.getEquipmentCost(playerCharacter.getLevel() + 1) * 24;
		final int factor = Monster.getFightsPerLevel();
		final int averageHealingCost = Shop.getHealingCost(playerCharacter.getLevel(),
				playerCharacter.getMaximumHP() * 2 / 3, playerCharacter.getMaximumHP(), playerCharacter.getKills());
		final int averageRegenerationCost = Shop.getRegenerationCost(playerCharacter.getLevel(),
				playerCharacter.getMaximumMP() / 2, playerCharacter.getMaximumMP(), playerCharacter.getKills());
	...
	private int getInitialAgility() {
		final PlayerCharacter playerCharacter = PCManager.getPlayer();
		final RandomRange r = new RandomRange((int) (0.5 * playerCharacter.getAgility()),
	...
	private int getInitialIntelligence() {
		final PlayerCharacter playerCharacter = PCManager.getPlayer();
		final RandomRange r = new RandomRange(0, playerCharacter.getIntelligence());
		return (int) (r.generate() * this.adjustForLevelDifference());
	}
	...
	private static int getFightsPerLevel() {
		...
		return (int) (Monster.BASE_FIGHTS_PER_LEVEL
				+ (playerCharacter.getLevel() - 1) * Monster.FIGHTS_PER_LEVEL_INCREMENT);
	}
