...
public class LoadTask3 extends Thread {
	...
	public void run() {
		...
		try (final XDataReader mazeFile = new XDataReader(this.filename, DataConstants.DATA_MODE_BINARY)) {
			try {
				final boolean supported = LoadTask3.checkFormatVersion(LoadTask3.readFormatVersion(mazeFile));
				if (!supported) {
				...
				this.gameMaze = this.gameMaze.readMaze(mazeFile, FormatConstants.MAZE_FORMAT_3);
				if (this.gameMaze == null) {
					mazeFile.close();
