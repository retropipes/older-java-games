...
import studio.ignitionigloogames.twistedtrek.import1.objects.BarrierGenerator;
import studio.ignitionigloogames.twistedtrek.import1.objects.Empty;
import studio.ignitionigloogames.twistedtrek.import1.objects.IcedBarrierGenerator;
...
class LayeredTower {
			...
			for (y = 0; y < this.getRows(); y++) {
				for (z = 0; z < this.getFloors(); z++) {
					for (e = 0; e < Maze.LAYER_COUNT; e++) {
						copy.towerData[x][y][z][e] = this.towerData[x][y][z][e].clone();
					}
				}
			}
		}
	...
	public void radialScanTimerAction(final int x, final int y, final int z, final int l, final int r,
	...
	public void radialScanKillMonsters(final int x, final int y, final int z, final int l, final int r) {
		int u, v;
		u = v = 0;
		// Perform the scan, and do the action
		for (u = x - r; u <= x + r; u++) {
	...
	public void radialScanFreezeObjects(final int x, final int y, final int z, final int l, final int r) {
		int u, v;
		u = v = 0;
		// Perform the scan, and do the action
		for (u = x - r; u <= x + r; u++) {
			for (v = y - r; v <= y + r; v++) {
				try {
					final boolean reactsToIce = this.getCell(u, v, z, l).isOfType(TypeConstants.TYPE_REACTS_TO_ICE);
					if (reactsToIce) {
						final MazeObject there = this.getCell(u, v, z, l);
						if (there.getClass() == Monster.class) {
							final Monster m = (Monster) there;
							// Freeze the monster
							this.setCell(new IcedMonster(m.getSavedObject()), u, v, z, l);
	...
	public void save() {
		...
		for (x = 0; x < this.getColumns(); x++) {
			for (y = 0; y < this.getRows(); y++) {
				for (z = 0; z < this.getFloors(); z++) {
					for (e = 0; e < Maze.LAYER_COUNT; e++) {
						this.savedTowerState.setDataCell(this.towerData[x][y][z][e], x, y, z, e);
					}
				}
			}
		}
	...
	public void restore() {
		...
		for (x = 0; x < this.getColumns(); x++) {
			for (y = 0; y < this.getRows(); y++) {
				for (z = 0; z < this.getFloors(); z++) {
					for (e = 0; e < Maze.LAYER_COUNT; e++) {
						this.towerData[x][y][z][e] = this.savedTowerState.getDataCell(x, y, z, e);
					}
				}
			}
		}
		...
		try {
			...
			if (!there.isSolid() && !there.getName().equals("Monster")) {
				if (there.getName().equals("Player")) {
					...
					// Move the monster
					this.setCell(monster.getSavedObject(), xLoc, yLoc, zLoc, Maze.LAYER_OBJECT);
					monster.setSavedObject(there);
					this.setCell(monster, xLoc + dirMove[0], yLoc + dirMove[1], zLoc, Maze.LAYER_OBJECT);
	...
	public void updateMovingBlockPosition(final int move, final int xLoc, final int yLoc, final MovingBlock block) {
		...
		try {
			...
			if (!there.isSolid() && !there.getName().equals("Player")) {
				this.setCell(block.getSavedObject(), xLoc, yLoc, zLoc, Maze.LAYER_OBJECT);
				// Move the block
				block.setSavedObject(there);
				this.setCell(block, xLoc + dirMove[0], yLoc + dirMove[1], zLoc, Maze.LAYER_OBJECT);
				// Does the ground have friction?
				if (!ground.hasFriction()) {
	...
	public void generateOneMonster() {
		...
		MazeObject currObj = this.getCell(randomRow, randomColumn, zLoc, Maze.LAYER_OBJECT);
		if (!currObj.isSolid()) {
			...
			while (currObj.isSolid()) {
				randomRow = row.generate();
				randomColumn = column.generate();
				currObj = this.getCell(randomRow, randomColumn, zLoc, Maze.LAYER_OBJECT);
			}
			final Monster m = new Monster();
			m.setSavedObject(currObj);
			this.setCell(m, randomRow, randomColumn, zLoc, Maze.LAYER_OBJECT);
	...
	public void warpObject(final MazeObject mo, final int x, final int y, final int z, final int l) {
		...
		randomRow = row.generate();
		randomColumn = column.generate();
		MazeObject currObj = this.getCell(randomRow, randomColumn, z, Maze.LAYER_OBJECT);
		if (!currObj.isSolid()) {
			...
			while (currObj.isSolid()) {
				randomRow = row.generate();
				randomColumn = column.generate();
				currObj = this.getCell(randomRow, randomColumn, z, Maze.LAYER_OBJECT);
			}
			this.setCell(new Empty(), x, y, z, l);
			this.setCell(mo, randomRow, randomColumn, z, Maze.LAYER_OBJECT);
		}
	...
	public static LayeredTower readLayeredTower(final XDataReader reader, final int formatVersion) throws IOException {
		...
		// New in V5.00
		if (formatVersion == FormatConstants.MAZE_FORMAT_4 || formatVersion == FormatConstants.MAZE_FORMAT_5) {
