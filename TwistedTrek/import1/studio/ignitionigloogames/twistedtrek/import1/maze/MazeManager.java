...
public class MazeManager {
		...
		if (saved) {
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				if (ff.getDescription().equals(gf.getDescription())) {
					app.getPrefsManager().setLastFilterUsedIndex(PreferencesManager.FILTER_GAME);
				} else if (ff.getDescription().equals(m2f.getDescription())) {
					app.getPrefsManager().setLastFilterUsedIndex(PreferencesManager.FILTER_MAZE_V2);
				} else if (ff.getDescription().equals(m3f.getDescription())) {
					app.getPrefsManager().setLastFilterUsedIndex(PreferencesManager.FILTER_MAZE_V3);
				} else if (ff.getDescription().equals(m4f.getDescription())) {
					app.getPrefsManager().setLastFilterUsedIndex(PreferencesManager.FILTER_MAZE_V4);
				} else {
					app.getPrefsManager().setLastFilterUsedIndex(PreferencesManager.FILTER_MAZE_V5);
				}
				app.getPrefsManager().setLastDirOpen(fc.getCurrentDirectory().getAbsolutePath());
	...
	private void loadFile(final String filename, final boolean isSavedGame, final int formatVersion) {
		...
		if (app.getMode() == Application.STATUS_GAME) {
			...
			if (this.lastUsedMazeFile != null && !this.lastUsedMazeFile.equals("")) {
				...
				if (extension != null) {
					...
					this.lastUsedMazeFile += Extension.getMaze5ExtensionWithPeriod();
				}
	...
	public boolean saveMazeAs() {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			final File file = fc.getSelectedFile();
			app.getPrefsManager().setLastDirSave(fc.getCurrentDirectory().getAbsolutePath());
			extension = MazeManager.getExtension(file);
			filename = file.getAbsolutePath();
			if (app.getMode() == Application.STATUS_GAME) {
				if (extension != null) {
					if (!extension.equals(Extension.getGameExtension())) {
						filename = MazeManager.getNameWithoutExtension(file) + Extension.getGameExtensionWithPeriod();
					}
				} else {
				...
				if (extension != null) {
					if (!extension.equals(Extension.getMaze5Extension())) {
						filename = MazeManager.getNameWithoutExtension(file) + Extension.getMaze5ExtensionWithPeriod();
					}
				} else {
