...
import studio.ignitionigloogames.twistedtrek.import1.creatures.StatGUI;
import studio.ignitionigloogames.twistedtrek.import1.effects.EffectConstants;
import studio.ignitionigloogames.twistedtrek.import1.effects.EffectManager;
...
import studio.ignitionigloogames.twistedtrek.import1.objects.AnnihilationWand;
import studio.ignitionigloogames.twistedtrek.import1.objects.DisarmTrapWand;
...
import studio.ignitionigloogames.twistedtrek.import1.objects.FinishMakingWand;
import studio.ignitionigloogames.twistedtrek.import1.objects.Player;
import studio.ignitionigloogames.twistedtrek.import1.objects.TeleportWand;
...
public class GameManager implements EffectConstants {
	...
	public boolean isFloorBelow() {
		...
		try {
			m.getCell(this.plMgr.getPlayerLocationX(), this.plMgr.getPlayerLocationY(),
					this.plMgr.getPlayerLocationZ() - 1, this.plMgr.getPlayerLocationW(), Maze.LAYER_OBJECT);
			return true;
	...
	public boolean isLevelBelow() {
		...
		final Maze m = app.getMazeManager().getMaze();
		try {
			m.getCell(this.plMgr.getPlayerLocationX(), this.plMgr.getPlayerLocationY(), this.plMgr.getPlayerLocationZ(),
	...
	public void setRemoteAction(final int x, final int y, final int z, final int w) {
	...
	public void doClockwiseRotate(final int r) {
		...
		if (this.actingRemotely) {
			b = m.rotateRadiusClockwise(this.remoteCoords[0], this.remoteCoords[1], this.remoteCoords[2],
					this.remoteCoords[3], r);
		} else {
			b = m.rotateRadiusClockwise(this.plMgr.getPlayerLocationX(), this.plMgr.getPlayerLocationY(),
					this.plMgr.getPlayerLocationZ(), this.plMgr.getPlayerLocationW(), r);
		}
		if (b) {
	...
	public void findPlayerAndAdjust() {
		...
		this.plMgr.setPlayerLocation(m.getFindResultColumn(w), m.getFindResultRow(w), m.getFindResultFloor(w), w);
	...
	public void doCounterclockwiseRotate(final int r) {
		...
		boolean b = false;
		if (this.actingRemotely) {
			b = m.rotateRadiusCounterclockwise(this.remoteCoords[0], this.remoteCoords[1], this.remoteCoords[2],
					this.remoteCoords[3], r);
		} else {
	...
	public void updatePositionRelative(final int ox, final int oy) {
		...
		do {
			try {
				...
				try {
					nextBelow = m.getCell(px + x, py + y, pz, pw, Maze.LAYER_GROUND);
				} catch (final ArrayIndexOutOfBoundsException ae) {
					nextBelow = new Empty();
				}
				try {
					nextAbove = m.getCell(px + x, py + y, pz, pw, Maze.LAYER_OBJECT);
				} catch (final ArrayIndexOutOfBoundsException ae) {
					nextAbove = new Wall();
				}
				try {
					previousBelow = m.getCell(px - x, py - y, pz, pw, Maze.LAYER_GROUND);
				} catch (final ArrayIndexOutOfBoundsException ae) {
					previousBelow = new Empty();
				}
				try {
					nextNextBelow = m.getCell(px + 2 * x, py + 2 * y, pz, pw, Maze.LAYER_GROUND);
				} catch (final ArrayIndexOutOfBoundsException ae) {
					nextNextBelow = new Empty();
				}
				try {
					nextNextAbove = m.getCell(px + 2 * x, py + 2 * y, pz, pw, Maze.LAYER_OBJECT);
				} catch (final ArrayIndexOutOfBoundsException ae) {
					nextNextAbove = new Wall();
				}
				try {
					proceed = o.preMoveAction(true, px + x, py + y, this.objectInv);
				} catch (final ArrayIndexOutOfBoundsException ae) {
					proceed = true;
			...
			if (proceed) {
				...
				try {
					if (this.checkSolid(x + px, y + py, this.savedMazeObject, below, nextBelow, nextAbove)) {
						if (this.delayedDecayActive) {
							this.doDelayedDecay();
						}
						m.setCell(this.savedMazeObject, px, py, pz, pw, Maze.LAYER_OBJECT);
						acted = new Empty();
						try {
							acted = m.getCell(px - x, py - y, pz, pw, Maze.LAYER_OBJECT);
						} catch (final ArrayIndexOutOfBoundsException ae) {
							// Do nothing
						}
						if (acted.isPullable() && this.isPullInProgress()) {
							if (!this.checkPull(x, y, pullX, pullY, acted, previousBelow, below,
									this.savedMazeObject)) {
								// Pull failed - object can't move that way
								acted.pullFailedAction(this.objectInv, x, y, pullX, pullY);
								this.st.deductStep();
								this.decayEffects();
							}
						...
						this.savedMazeObject = m.getCell(px, py, pz, pw, Maze.LAYER_OBJECT);
						groundInto = m.getCell(px, py, pz, pw, Maze.LAYER_GROUND);
						m.setCell(new Player(), px, py, pz, pw, Maze.LAYER_OBJECT);
						this.decayEffects();
						this.redrawMaze();
						...
						acted = m.getCell(px + x, py + y, pz, pw, Maze.LAYER_OBJECT);
						if (acted.isPushable()) {
							if (this.checkPush(x, y, pushX, pushY, acted, nextBelow, nextNextBelow, nextNextAbove)) {
								if (this.delayedDecayActive) {
									this.doDelayedDecay();
								}
								m.setCell(this.savedMazeObject, px, py, pz, pw, Maze.LAYER_OBJECT);
								...
								acted.pushFailedAction(this.objectInv, x, y, pushX, pushY);
								this.decayEffects();
								this.st.deductStep();
							}
							...
							this.fireMoveFailedActions(px + x, py + y, this.savedMazeObject, below, nextBelow,
									nextAbove);
							this.decayEffects();
							this.st.deductStep();
						}
					...
					m.setCell(new Player(), this.plMgr.getPlayerLocationX(), this.plMgr.getPlayerLocationY(),
							this.plMgr.getPlayerLocationZ(), this.plMgr.getPlayerLocationW(), Maze.LAYER_OBJECT);
		...
		} while (proceed && !groundInto.hasFrictionConditionally(this.objectInv, false)
	...
	private boolean checkPush(final int x, final int y, final int pushX, final int pushY, final MazeObject acted,
		...
		if (nextBelowAccept && nextNextBelowAccept && nextNextAboveAccept) {
			nextBelow.pushOutAction(this.objectInv, acted, px + pushX, py + pushY, pz, pw);
			acted.pushAction(this.objectInv, nextNextAbove, x, y, pushX, pushY);
			nextNextAbove.pushIntoAction(this.objectInv, acted, px + pushX, py + pushY, pz, pw);
			nextNextBelow.pushIntoAction(this.objectInv, acted, px + pushX, py + pushY, pz, pw);
			return true;
		} else {
			return false;
		}
	...
	private boolean checkPull(final int x, final int y, final int pullX, final int pullY, final MazeObject acted,
		...
		if (previousBelowAccept && belowAccept && aboveAccept) {
			previousBelow.pullOutAction(this.objectInv, acted, px - pullX, py - pullY, pz, pw);
			acted.pullAction(this.objectInv, above, x, y, pullX, pullY);
			above.pullIntoAction(this.objectInv, acted, px - pullX, py - pullY, pz, pw);
			below.pullIntoAction(this.objectInv, acted, px - pullX, py - pullY, pz, pw);
			return true;
		} else {
			return false;
		}
	...
	public void updatePushedPosition(final int x, final int y, final int pushX, final int pushY, final MazeObject o) {
		...
		MazeObject there = mm.getMazeObject(this.plMgr.getPlayerLocationX() + cumX,
				this.plMgr.getPlayerLocationY() + cumY, this.plMgr.getPlayerLocationZ(),
		...
		if (there != null) {
			do {
				...
				there = mm.getMazeObject(this.plMgr.getPlayerLocationX() + cumX, this.plMgr.getPlayerLocationY() + cumY,
						this.plMgr.getPlayerLocationZ(), this.plMgr.getPlayerLocationW(), Maze.LAYER_GROUND);
				if (there == null) {
					break;
				}
	...
	private void movePushedObjectPosition(final int x, final int y, final int pushX, final int pushY,
			final MazeObject o, final MazeObject g) {
		final Application app = Import1.getApplication();
		final Maze m = app.getMazeManager().getMaze();
		try {
			m.setCell(new Empty(), this.plMgr.getPlayerLocationX() + x, this.plMgr.getPlayerLocationY() + y,
					this.plMgr.getPlayerLocationZ(), this.plMgr.getPlayerLocationW(), Maze.LAYER_OBJECT);
	...
	public void updatePulledPosition(final int x, final int y, final int pullX, final int pullY, final MazeObject o) {
		final Application app = Import1.getApplication();
		final Maze m = app.getMazeManager().getMaze();
		try {
			m.setCell(new Empty(), this.plMgr.getPlayerLocationX() - x, this.plMgr.getPlayerLocationY() - y,
	...
	public void updatePushedIntoPositionAbsolute(final int x, final int y, final int z, final int w, final int x2,
			final int y2, final int z2, final int w2, final GenericMovableObject pushedInto, final MazeObject source) {
		final Application app = Import1.getApplication();
		final Maze m = app.getMazeManager().getMaze();
		try {
			if (!m.getCell(x, y, z, w, Maze.LAYER_OBJECT).isConditionallySolid(this.objectInv)) {
				final MazeObject saved = m.getCell(x, y, z, w, Maze.LAYER_OBJECT);
				m.setCell(pushedInto, x, y, z, w, Maze.LAYER_OBJECT);
				m.setCell(source, x2, y2, z2, w2, Maze.LAYER_OBJECT);
				saved.pushIntoAction(this.objectInv, pushedInto, x2, y2, z2 - 1, w2);
				this.redrawMaze();
				app.getMazeManager().setDirty(true);
			}
		} catch (final ArrayIndexOutOfBoundsException ae) {
	...
	public boolean tryUpdatePositionRelative(final int x, final int y) {
		try {
			...
			final MazeObject nextBelow = m.getCell(this.plMgr.getPlayerLocationX() + x,
					this.plMgr.getPlayerLocationY() + y, this.plMgr.getPlayerLocationZ(),
					this.plMgr.getPlayerLocationW(), Maze.LAYER_GROUND);
	...
	public boolean tryUpdatePositionAbsolute(final int x, final int y, final int z) {
		try {
			...
			final MazeObject below = m.getCell(this.plMgr.getPlayerLocationX(), this.plMgr.getPlayerLocationY(),
					this.plMgr.getPlayerLocationZ(), this.plMgr.getPlayerLocationW(), Maze.LAYER_GROUND);
			final MazeObject nextBelow = m.getCell(x, y, z, this.plMgr.getPlayerLocationW(), Maze.LAYER_GROUND);
			final MazeObject nextAbove = m.getCell(x, y, z, this.plMgr.getPlayerLocationW(), Maze.LAYER_OBJECT);
	...
	public void updatePositionAbsolute(final int x, final int y, final int z, final int w) {
		final Application app = Import1.getApplication();
		final Maze m = app.getMazeManager().getMaze();
		try {
			m.getCell(x, y, z, w, Maze.LAYER_OBJECT).preMoveAction(true, x, y, this.objectInv);
		} catch (final ArrayIndexOutOfBoundsException ae) {
			// Do nothing
		} catch (final NullPointerException np) {
			// Do nothing
					...
					try {
						if (visible) {
							final String name1 = app.getMazeManager().getMaze()
									.getCell(y, x, this.plMgr.getPlayerLocationZ(), this.plMgr.getPlayerLocationW(),
											Maze.LAYER_GROUND)
							...
							this.drawGrid[xFix][yFix].setIcon(GraphicsManager.getImage("Darkness"));
						}
					} catch (final ArrayIndexOutOfBoundsException ae) {
						this.drawGrid[xFix][yFix].setIcon(GraphicsManager.getImage(new EmptyVoid().gameRenderHook(y, x,
								this.plMgr.getPlayerLocationZ(), this.plMgr.getPlayerLocationW())));
					} catch (final NullPointerException np) {
						this.drawGrid[xFix][yFix].setIcon(GraphicsManager.getImage(new EmptyVoid().gameRenderHook(y, x,
								this.plMgr.getPlayerLocationZ(), this.plMgr.getPlayerLocationW())));
					}
	...
	void redrawOneSquare(final int x, final int y, final boolean useDelay, final String name3) {
		...
		if (this.outputFrame.isVisible()) {
			...
			visible = app.getMazeManager().getMaze().isSquareVisible(this.plMgr.getPlayerLocationX(),
					this.plMgr.getPlayerLocationY(), x, y, this.plMgr.getPlayerLocationW());
			try {
				if (visible) {
					final String name1 = app.getMazeManager().getMaze()
							.getCell(x, y, this.plMgr.getPlayerLocationZ(), this.plMgr.getPlayerLocationW(),
									Maze.LAYER_GROUND)
							.gameRenderHook(x, y, this.plMgr.getPlayerLocationZ(), this.plMgr.getPlayerLocationW());
					final String name2 = app.getMazeManager().getMaze()
							.getCell(x, y, this.plMgr.getPlayerLocationZ(), this.plMgr.getPlayerLocationW(),
									Maze.LAYER_OBJECT)
							.gameRenderHook(x, y, this.plMgr.getPlayerLocationZ(), this.plMgr.getPlayerLocationW());
					this.drawGrid[xFix][yFix].setIcon(GraphicsManager.getVirtualCompositeImage(name1, name2, name3));
				} else {
					this.drawGrid[xFix][yFix].setIcon(GraphicsManager.getImage("Darkness"));
				}
	...
	public void resetPlayerLocation(final int level) {
		final Application app = Import1.getApplication();
		final Maze m = app.getMazeManager().getMaze();
		this.plMgr.setPlayerLocation(m.getStartColumn(level), m.getStartRow(level), m.getStartFloor(level), level);
	}
	...
	public void resetGameState() {
		...
		if (playerExists) {
			m.findAllStarts();
			this.plMgr.setPlayerLocation(m.getFindResultColumn(startW), m.getFindResultRow(startW),
					m.getFindResultFloor(startW), startW);
			m.save();
		}
	}
	...
	public void solvedLevel() {
		...
		final Application app = Import1.getApplication();
		final Maze m = app.getMazeManager().getMaze();
		final boolean playerExists = m.findPlayerOnLevel(this.plMgr.getPlayerLocationW() + 1);
		if (playerExists) {
	...
	private void gameOver() {
		if (Import1.getApplication().getPrefsManager().getSoundEnabled(PreferencesManager.SOUNDS_UI)) {
	...
	public void decay() {
		if (this.actingRemotely) {
			Import1.getApplication().getMazeManager().getMaze().setCell(new Empty(), this.remoteCoords[0],
	...
	public void morph(final MazeObject morphInto, final int x, final int y, final int z, final int w, final int e) {
		final Application app = Import1.getApplication();
		final Maze m = app.getMazeManager().getMaze();
		try {
	...
	public void morphOther(final MazeObject morphInto, final int x, final int y, final int e) {
		...
		try {
			m.setCell(morphInto, this.plMgr.getPlayerLocationX() + x, this.plMgr.getPlayerLocationY() + y,
					this.plMgr.getPlayerLocationZ(), this.plMgr.getPlayerLocationW(), e);
			this.redrawMazeNoRebuild();
			app.getMazeManager().setDirty(true);
		} catch (final ArrayIndexOutOfBoundsException ae) {
			// Do nothing
	...
	public void showEquipmentDialog() {
		final String[] equipString = PCManager.getPlayer().getItems().generateEquipmentStringArray();
		Messager.showInputDialog("Equipment", "Equipment", equipString, equipString[0]);
	}

	public void showInventoryDialog() {
		final String[] inv1String = this.objectInv.generateInventoryStringArray();
		final String[] inv2String = PCManager.getPlayer().getItems().generateInventoryStringArray(inv1String.length);
		final String[] invString = new String[inv1String.length + inv2String.length];
		...
		Messager.showInputDialog("Inventory", "Inventory", invString, invString[0]);
	}
	...
	public void showUseDialog() {
		...
		final String result = Messager.showInputDialog("Use which item?", "Import1", userChoices,
			...
			for (x = 0; x < choices.length; x++) {
				if (result.equals(userChoices[x])) {
					...
					if (this.objectInv.getUses(this.objectBeingUsed) == 0) {
						...
						this.setUsingAnItem(false);
					} else if (!this.objectBeingUsed.isOfType(TypeConstants.TYPE_BOW)) {
	...
	public void useItemHandler(final int x, final int y) {
		...
		final int destX = x / GraphicsManager.getGraphicSize() + this.vwMgr.getViewingWindowLocationX() - xOffset
				+ yOffset;
		final int destY = y / GraphicsManager.getGraphicSize() + this.vwMgr.getViewingWindowLocationY() + xOffset
				- yOffset;
		...
		if (this.usingAnItem() && app.getMode() == Application.STATUS_GAME) {
			if (!this.objectBeingUsed.isOfType(TypeConstants.TYPE_BOW)) {
				final boolean visible = app.getMazeManager().getMaze().isSquareVisible(this.plMgr.getPlayerLocationX(),
						this.plMgr.getPlayerLocationY(), destX, destY, this.plMgr.getPlayerLocationW());
				try {
					final MazeObject target = m.getCell(destX, destY, destZ, destW, Maze.LAYER_OBJECT);
			...
			if (this.usingAnItem()) {
				this.objectInv.use(this.objectBeingUsed, destX, destY, destZ, destW);
				this.redrawMaze();
			}
		}
	}
	...
	public void identifyObject(final int x, final int y) {
		...
		final int destW = this.plMgr.getPlayerLocationW();
		try {
			final MazeObject target1 = m.getCell(destX, destY, destZ, destW, Maze.LAYER_GROUND);
			final MazeObject target2 = m.getCell(destX, destY, destZ, destW, Maze.LAYER_OBJECT);
	...
	public void loadGameHook(final XDataReader mazeFile, final int formatVersion) throws IOException {
		...
		app.getMazeManager().getMaze().readSavedMazeState(mazeFile, formatVersion);
	...
	private void setUpGUI() {
		...
		this.outputFrame = new JFrame("Import1");
	...
	private class EventHandler implements KeyListener, WindowListener, MouseListener {
		...
		public void keyPressed(final KeyEvent e) {
			if (!GameManager.this.arrowActive) {
				if (!Import1.getApplication().getPrefsManager().oneMove()) {
					if (e.isAltDown()) {
						this.handleArrows(e);
					} else {
