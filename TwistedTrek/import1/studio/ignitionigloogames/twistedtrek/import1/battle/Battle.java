...
import studio.ignitionigloogames.twistedtrek.import1.creatures.Monster;
...
public class Battle implements BattleResults, MoveTypes {
	...
	protected final void doEnemyActions() {
		final int actionToPerform = this.enemy.getAI().getNextAction(this.enemy);
		if (actionToPerform == AIRoutine.ACTION_ATTACK) {
			...
			SpellBookManager.castSpell(this.enemy.getAI().getSpellToCast(), this.enemy);
		} else if (actionToPerform == AIRoutine.ACTION_FLEE) {
			final RandomRange rf = new RandomRange(0, 100);
			final int runChance = rf.generate();
	...
	protected final void computePlayerDamage() {
		...
		if (this.result != BattleResults.ENEMY_FLED) {
			...
			if (Battle.fumble(playerCharacter)) {
				...
				final RandomRange fumDamage = new RandomRange(1, Math.max(playerCharacter.getWeaponPower(), 1));
				...
				final RandomRange randomAttackGenerator = new RandomRange(0,
						(int) playerCharacter.getEffectedStat(StatConstants.STAT_ATTACK));
				final RandomRange randomEnemyDefenseGenerator = new RandomRange(0,
						(int) this.enemy.getEffectedStat(StatConstants.STAT_DEFENSE));
				int currDamage = 0;
				int currRiposteEnemyDamage = 0;
				this.multiplier = MultiplierValues.getRandomNormalValue();
				final boolean didPierce = Battle.pierce();
				final int randomAttack = (int) (randomAttackGenerator.generate() * this.multiplier);
	...
	protected final void computeEnemyDamage() {
		...
		if (this.result != BattleResults.ENEMY_FLED) {
			...
			if (Battle.fumble(this.enemy)) {
				...
				final RandomRange fumDamage = new RandomRange(1, Math.max(this.enemy.getAttack(), 1));
				...
				final RandomRange randomDefenseGenerator = new RandomRange(0,
						(int) playerCharacter.getEffectedStat(StatConstants.STAT_DEFENSE));
				final RandomRange randomEnemyAttackGenerator = new RandomRange(0,
						(int) this.enemy.getEffectedStat(StatConstants.STAT_ATTACK));
				int currEnemyDamage = 0;
				int currRiposteDamage = 0;
				this.enemyMultiplier = MultiplierValues.getRandomNormalValue();
				final boolean didPierce = Battle.pierce();
				final int randomEnemyAttack = (int) (randomEnemyAttackGenerator.generate() * this.enemyMultiplier);
	...
	protected final int computeRunChance() {
		return Battle.BASE_RUN_CHANCE - this.enemy.getLevelDifference() * Battle.RUN_CHANCE_DIFF_FACTOR;
	}

	public final int computeEnemyRunChance() {
		return Battle.ENEMY_BASE_RUN_CHANCE + this.enemy.getLevelDifference() * Battle.ENEMY_RUN_CHANCE_DIFF_FACTOR;
	}
	...
	protected final void displayPlayerRoundResults() {
		...
		if (this.result != BattleResults.ENEMY_FLED) {
			...
			final String playerFumbleDamageString = Integer.toString(this.fumbleDamage);
			...
			if (this.fumbleDamage != 0) {
				...
				final String whack = MultiplierValues.getTextForValue(this.multiplier);
				if (!whack.equals("")) {
					playerWhackString = "Your attack hits with " + whack + "force!\n";
				}
	...
	protected final void displayEnemyRoundResults() {
		...
		if (this.result != BattleResults.ENEMY_FLED) {
			...
			final String enemyFumbleDamageString = Integer.toString(this.enemyFumbleDamage);
			...
			if (this.enemyFumbleDamage != 0) {
				...
				final String whack = MultiplierValues.getTextForValue(this.multiplier);
				if (!whack.equals("")) {
					enemyWhackString = "The " + enemyName + "'s attack hits with " + whack + "force!\n";
				}
	...
	public void doBattleByProxy() {
		...
		Import1.getApplication().getGameManager().getScoreTracker().updateScore(m.getExperience() + m.getGold());
		// Level Up Check
	...
	protected final void updateMessageAreaPostSteal() {
		this.appendToMessageArea("You try to steal money, and successfully steal " + this.stealAmount + " Gold!");
	}
	...
	protected void doResult() {
		...
		if (this.result == BattleResults.WON) {
			...
			Import1.getApplication().getGameManager().getScoreTracker()
					.updateScore(m.getExperience() + m.getGold() + m.getPerfectBonusGold());
		...
		if (playerCharacter.checkLevelUp()) {
			...
			this.appendToMessageArea("You reached level " + playerCharacter.getLevel() + ".");
		}
		// Final Cleanup
		this.stripExtraNewLine();
		this.battleFrame.pack();
	...
	public final void battleDone() {
		...
		if (this.result == BattleResults.LOST || this.result == BattleResults.ANNIHILATED) {
			Import1.getApplication().getGameManager().gameOverWithMessage("You fell in battle... Game Over!");
		}
		Import1.getApplication().getGameManager().showOutput();
