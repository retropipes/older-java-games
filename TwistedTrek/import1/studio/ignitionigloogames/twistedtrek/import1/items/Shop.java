...
public class Shop implements ShopTypes {
	...
	private boolean shopStage1() {
		...
		if (this.typeChoices != null) {
			this.typeResult = Messager.showInputDialog(this.getGoldTotals() + "Select Type", this.getShopNameFromType(),
	...
	private boolean shopStage2() {
		...
		if (this.type == ShopTypes.SHOP_TYPE_WEAPONS) {
			if (this.typeResult.equals(this.typeChoices[0])) {
				...
				this.handDefault = 0;
				this.handResult = Messager.showInputDialog(this.getGoldTotals() + "Select Hand",
	...
	private boolean shopStage3() {
		...
		this.result = Messager.showInputDialog(this.getGoldTotals() + "Select", this.getShopNameFromType(),
	...
	private boolean shopStage4() {
		...
		if (this.type == ShopTypes.SHOP_TYPE_WEAPONS || this.type == ShopTypes.SHOP_TYPE_ARMOR) {
			...
			while (stage4Result.equals("")) {
				stage4Result = Messager.showTextInputDialog(this.getGoldTotals() + this.result + " How Much?",
				...
				try {
					...
					Messager.showErrorDialog("Amount must be greater than zero.", this.getShopNameFromType());
					stage4Result = "";
				}
			}
		} else if (this.type == ShopTypes.SHOP_TYPE_SPELLS) {
		...
		if (this.type != ShopTypes.SHOP_TYPE_BANK) {
			final int stage4Confirm = Messager.showConfirmDialog("This will cost " + this.cost + " Gold. Are you sure?",
					this.getShopNameFromType());
		...
		if (this.type == ShopTypes.SHOP_TYPE_BANK) {
			if (this.index == 0) {
				if (playerCharacter.getGold() < this.cost) {
					Messager.showErrorDialog("Not Enough Gold!", this.getShopNameFromType());
					return false;
				}
			} else {
