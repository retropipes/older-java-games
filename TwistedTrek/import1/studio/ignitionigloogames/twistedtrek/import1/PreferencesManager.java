...
import studio.ignitionigloogames.twistedtrek.import1.objects.Dirt;
import studio.ignitionigloogames.twistedtrek.import1.objects.Grass;
import studio.ignitionigloogames.twistedtrek.import1.objects.Sand;
import studio.ignitionigloogames.twistedtrek.import1.objects.Snow;
import studio.ignitionigloogames.twistedtrek.import1.objects.Tile;
import studio.ignitionigloogames.twistedtrek.import1.objects.Tundra;
...
public class PreferencesManager {
	...
	public void setPrefs(final boolean initial) {
		...
		this.checkBetaUpdatesStartupEnabled = this.checkBetaUpdatesStartup.isSelected();
	...
	private void setUpGUI() {
		...
		this.updateCheckIntervalValues = new String[] { "Daily", "Every 2nd Day", "Weekly", "Every 2nd Week",
				"Monthly" };
		this.updateCheckInterval = new JComboBox<>(this.updateCheckIntervalValues);
		...
		this.editorPane.setLayout(new GridLayout(PreferencesManager.GRID_LENGTH, 1));
		...
		this.waitFrame.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);
	...
	private class PreferencesFileManager {
		...
		public boolean readPreferencesFile() {
			try (final BufferedReader s = new BufferedReader(new FileReader(this.getPrefsFile()))) {
				...
				pm.editorFillChoices.setSelectedIndex(Integer.parseInt(s.readLine()));
				...
				pm.checkBetaUpdatesStartup.setSelected(Boolean.parseBoolean(s.readLine()));
		...
		public void writePreferencesFile() {
			...
			try (final BufferedWriter s = new BufferedWriter(new FileWriter(prefsFile))) {
				...
				s.write(Integer.toString(pm.updateCheckInterval.getSelectedIndex()) + "\n");
	...
	private class EventHandler implements ActionListener, ItemListener, WindowListener {
		...
		public void itemStateChanged(final ItemEvent e) {
			try {
				...
				} else if (o.getClass().equals(pm.music[PreferencesManager.MUSIC_ALL].getClass())) {
					final JCheckBox check = (JCheckBox) o;
