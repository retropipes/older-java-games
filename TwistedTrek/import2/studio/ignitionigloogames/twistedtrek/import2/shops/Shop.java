/* Import2: An RPG */
package studio.ignitionigloogames.twistedtrek.import2.shops;
...
import studio.ignitionigloogames.twistedtrek.import2.items.EquipmentFactory;
...
import studio.ignitionigloogames.twistedtrek.import2.items.Socks;
import studio.ignitionigloogames.twistedtrek.import2.items.WeaponConstants;
	...
	private class ShopDialogGUI {
		...
		private boolean shopStage1() {
			...
			if (Shop.this.type == ShopTypes.SHOP_TYPE_WEAPONS) {
				...
				Shop.this.typeIndex = PartyManager.getParty().getLeader().getFaith().getFaithID();
			}
			if (Shop.this.typeChoices != null) {
				Shop.this.typeResult = CommonDialogs.showInputDialog(Shop.this.getGoldTotals() + "Select Type",
				...
				if (Shop.this.type == ShopTypes.SHOP_TYPE_ARMOR && Shop.this.typeIndex > 1) {
			...
			if (Shop.this.type == ShopTypes.SHOP_TYPE_WEAPONS) {
				if (Shop.this.typeResult.equals(Shop.this.typeChoices[0])) {
					Shop.this.choices = EquipmentFactory
							.createOneHandedWeaponNames(playerCharacter.getCaste().getCasteID());
					// Choose Hand
					final String[] handChoices = WeaponConstants.getHandChoices();
					final int handDefault = 0;
					final String handResult = CommonDialogs.showInputDialog(Shop.this.getGoldTotals() + "Select Hand",
							Shop.this.getShopNameFromType(), handChoices, handChoices[handDefault]);
					if (handResult == null) {
						return false;
					}
					...
					Shop.this.choices = EquipmentFactory
							.createTwoHandedWeaponNames(playerCharacter.getCaste().getCasteID());
				}
			} else if (Shop.this.type == ShopTypes.SHOP_TYPE_ARMOR) {
				...
				Shop.this.choices = PartyManager.getParty().getLeader().getItems()
		...
		private boolean shopStage4() {
			...
			if (Shop.this.type == ShopTypes.SHOP_TYPE_WEAPONS || Shop.this.type == ShopTypes.SHOP_TYPE_ARMOR) {
				...
				Shop.this.item = EquipmentFactory.createSocks(Shop.this.index + 1,
						(Shop.this.index + 1) * 500 * playerCharacter.getLevel());
				Shop.this.cost = Shop.this.item.getBuyPrice();
			} else if (Shop.this.type == ShopTypes.SHOP_TYPE_ENHANCEMENTS) {
				final Equipment old = playerCharacter.getItems().getEquipmentInSlot(Shop.this.index);
				if (old != null) {
					if (old.isTwoHanded()) {
					...
					Shop.this.item = EquipmentFactory.createEnhancedEquipment(old, bonus);
					...
					CommonDialogs.showErrorDialog("Nothing is equipped there!", Shop.this.getShopNameFromType());
					return false;
				}
			} else if (Shop.this.type == ShopTypes.SHOP_TYPE_FAITH_POWERS) {
				final Equipment old = playerCharacter.getItems().getEquipmentInSlot(Shop.this.index);
				if (old != null) {
					...
					final int power = old.getPotency();
					final int bonus = old.getFaithPowerLevel(Shop.this.typeIndex);
					Shop.this.item = EquipmentFactory.createFaithPoweredEquipment(old, Shop.this.typeIndex, bonus);
					Shop.this.cost = Shop.getEnhancementCost(bonus, power);
					if (Shop.this.cost == 0) {
		...
		private void shopStage6() {
			...
			if (Shop.this.type == ShopTypes.SHOP_TYPE_WEAPONS) {
				...
				if (Shop.this.typeResult.equals(Shop.this.typeChoices[0])) {
					final Equipment bought = EquipmentFactory.createOneHandedWeapon(Shop.this.index,
							playerCharacter.getCaste().getCasteID(), 0);
					playerCharacter.getItems().equipOneHandedWeapon(playerCharacter, bought, Shop.this.handIndex, true);
				} else {
					final Equipment bought = EquipmentFactory.createTwoHandedWeapon(Shop.this.index,
							playerCharacter.getCaste().getCasteID(), 0);
