...
final class MovementTask extends Thread {
	...
	// Constructors
	public MovementTask(final GameViewingWindowManager view, final MazeEffectManager effect,
	...
	public boolean tryAbsolute(final int x, final int y, final int z) {
		try {
			...
			final AbstractMazeObject nextBelow = m.getCell(x, y, z, MazeConstants.LAYER_GROUND);
			final AbstractMazeObject nextAbove = m.getCell(x, y, z, MazeConstants.LAYER_OBJECT);
	...
	private static boolean checkSolidAbsolute(final AbstractMazeObject inside, final AbstractMazeObject below,
	...
	private void updatePositionRelative(final int dirX, final int dirY, final int dirZ) {
		...
		do {
			try {
				...
				try {
					nextBelow = m.getCell(px + fX, py + fY, pz + fZ, MazeConstants.LAYER_GROUND);
				} catch (final ArrayIndexOutOfBoundsException ae) {
					nextBelow = new Empty();
				}
				try {
					nextAbove = m.getCell(px + fX, py + fY, pz + fZ, MazeConstants.LAYER_OBJECT);
				} catch (final ArrayIndexOutOfBoundsException ae) {
					nextAbove = new Wall();
				}
				try {
			...
			if (this.proceed) {
				...
				try {
					if (MovementTask.checkSolid(this.saved, below, nextBelow, nextAbove)) {
						...
						if (this.proceed) {
							this.saved = m.getCell(px, py, pz, MazeConstants.LAYER_OBJECT);
	...
	private boolean checkLoopCondition(final AbstractMazeObject below, final AbstractMazeObject nextBelow,
			final AbstractMazeObject nextAbove) {
		return this.proceed && !this.em.isEffectActive(MazeEffectConstants.EFFECT_STICKY) && !nextBelow.hasFriction()
				&& MovementTask.checkSolid(this.saved, below, nextBelow, nextAbove);
	}

	private static boolean checkSolid(final AbstractMazeObject inside, final AbstractMazeObject below,
			final AbstractMazeObject nextBelow, final AbstractMazeObject nextAbove) {
		final boolean insideSolid = inside.isSolid();
		final boolean belowSolid = below.isSolid();
		final boolean nextBelowSolid = nextBelow.isSolid();
	...
	private static void fireMoveFailedActions(final int x, final int y, final AbstractMazeObject inside,
			final AbstractMazeObject below, final AbstractMazeObject nextBelow, final AbstractMazeObject nextAbove) {
		final boolean insideSolid = inside.isSolid();
		final boolean belowSolid = below.isSolid();
		final boolean nextBelowSolid = nextBelow.isSolid();
		final boolean nextAboveSolid = nextAbove.isSolid();
