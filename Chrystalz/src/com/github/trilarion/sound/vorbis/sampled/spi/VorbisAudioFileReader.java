...
public class VorbisAudioFileReader extends TAudioFileReader {
    ...
    private static final int MARK_LIMIT = VorbisAudioFileReader.INITAL_READ_LENGTH
            + 1;
    ...
    public AudioFileFormat getAudioFileFormat(final File file)
        ...
        try (InputStream inputStream = new BufferedInputStream(
            ...
            return this.getAudioFileFormat(inputStream, (int) file.length(),
     ...
     */
    @Override
    public AudioFileFormat getAudioFileFormat(final InputStream inputStream1)
            throws UnsupportedAudioFileException, IOException {
    ...
    public AudioFileFormat getAudioFileFormat(final InputStream inputStream,
            final long medialength)
            throws UnsupportedAudioFileException, IOException {
        return this.getAudioFileFormat(inputStream, (int) medialength,
    ...
    protected AudioFileFormat getAudioFileFormat(final InputStream bitStream,
        ...
        final AudioFormat format = new AudioFormat(
                VorbisAudioFileReader.VORBISENC, this.vorbisInfo.rate,
                AudioSystem.NOT_SPECIFIED, this.vorbisInfo.channels, 1,
                frameRate, false, af_properties);
        // Patch end
        return new VorbisAudioFileFormat(
                VorbisAudioFileReader.OGG_AUDIOFILEFORMAT_TYPE, format,
    ...
    public AudioInputStream getAudioInputStream(final InputStream inputStream)
        ...
        return this.getAudioInputStream(inputStream, AudioSystem.NOT_SPECIFIED,
                AudioSystem.NOT_SPECIFIED);
    }
     ...
     */
    public AudioInputStream getAudioInputStream(final InputStream inputStream,
            final int medialength, final int totalms)
            throws UnsupportedAudioFileException, IOException {
        VorbisAudioFileReader.LOG.log(Level.FINE,
                "getAudioInputStream(InputStream inputStreamint medialength, int totalms)");
        InputStream substituteStream = inputStream;
        try {
            if (!inputStream.markSupported()) {
                substituteStream = new BufferedInputStream(inputStream);
            }
            substituteStream.mark(VorbisAudioFileReader.MARK_LIMIT);
            final AudioFileFormat audioFileFormat = this
                    .getAudioFileFormat(substituteStream, medialength, totalms);
            substituteStream.reset();
            return new AudioInputStream(substituteStream,
     ...
     */
    private void readHeaders(final Map<String, Object> aff_properties)
            throws IOException {
        VorbisAudioFileReader.LOG.log(Level.FINE, "readHeaders(");
        final int bufferSize_ = this.bufferMultiple_ * 256 * 2;
        ...
        if (this.vorbisInfo.synthesis_headerin(this.vorbisComment,
                this.oggPacket_) < 0) {
        ...
        while (i < 2) {
            while (i < 2) {
                ...
                if (result == 1) {
                    ...
                    while (i < 2) {
                        result = this.oggStreamState_
                                .packetout(this.oggPacket_);
                        if (result == 0) {
                            break;
                        }
                        if (result == -1) {
                        ...
                        this.vorbisInfo.synthesis_headerin(this.vorbisComment,
                                this.oggPacket_);
                        i++;
                    }
                }
            ...
            this.index = this.oggSyncState_.buffer(bufferSize_);
            this.buffer = this.oggSyncState_.data;
            this.bytes = this.readFromStream(this.buffer, this.index,
                    bufferSize_);
            if (this.bytes == -1) {
                break;
            }
        ...
        for (final byte[] ptr1 : ptr) {
            ...
            final String currComment = new String(ptr1, 0, ptr1.length - 1,
                    "UTF-8").trim();
            ...
            aff_properties.put("ogg.comment.encodedby",
                    new String(this.vorbisComment.vendor, 0,
                            this.vorbisComment.vendor.length - 1,
     ...
     */
    private int readFromStream(final byte[] buffer1, final int index1,
            final int bufferSize_) {
        int bytes1;
        try {
            bytes1 = this.oggBitStream_.read(buffer1, index1, bufferSize_);
        } catch (final Exception e) {
            VorbisAudioFileReader.LOG.log(Level.FINE,
                    "Cannot Read Selected Song");
            bytes1 = -1;
        }
        return bytes1;
    }
