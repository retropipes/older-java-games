...
public class Comment {
     ...
     * @param contents
     */
    public void add_tag(final String tag, final String contents1) {
    ...
    private int query(final byte[] tag, final int count) {
        ...
        for (i = 0; i < this.comments; i++) {
            if (Comment.tagcompare(this.user_comments[i], fulltag,
                    fulltaglen)) {
                if (count == found) {
    ...
    public String getComment(final int i) {
        ...
        return new String(this.user_comments[i], 0,
                this.user_comments[i].length - 1);
    }

    @Override
    public String toString() {
        String foo = "Vendor: "
                + new String(this.vendor, 0, this.vendor.length - 1);
        for (int i = 0; i < this.comments; i++) {
            foo = foo + "\nComment: " + new String(this.user_comments[i], 0,
                    this.user_comments[i].length - 1);
        }
        foo = foo + "\n";
        return foo;
