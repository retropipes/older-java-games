...
class Mapping0 extends FuncMapping {
    ...
    Object look(final DspState vd, final InfoMode vm, final Object m) {
        ...
        final InfoMapping0 info = look.map = (InfoMapping0) m;
        look.mode = vm;
    ...
    Object unpack(final Info vi, final Buffer opb) {
        ...
        if (opb.read(1) != 0) {
            ...
            for (int i = 0; i < info.coupling_steps; i++) {
                final int testM = info.coupling_mag[i] = opb
                        .read(Util.ilog2(vi.channels));
                final int testA = info.coupling_ang[i] = opb
                        .read(Util.ilog2(vi.channels));
    ...
    synchronized int inverse(final Block vb, final Object l) {
        ...
        final int n = vb.pcmend = vi.blocksizes[vb.W];
        ...
        for (int i = 0; i < vi.channels; i++) {
            ...
            this.floormemo[i] = look.floor_func[submap].inverse1(vb,
                    look.floor_look[submap], this.floormemo[i]);
        ...
        for (int i = 0; i < info.coupling_steps; i++) {
            if (this.nonzero[info.coupling_mag[i]] != 0
                    || this.nonzero[info.coupling_ang[i]] != 0) {
        ...
        for (int i = 0; i < info.submaps; i++) {
            ...
            look.residue_func[i].inverse(vb, look.residue_look[i],
                    this.pcmbundle, this.zerobundle, ch_in_bundle);
        }
