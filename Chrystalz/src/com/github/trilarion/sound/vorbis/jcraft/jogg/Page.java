...
public class Page {
        ...
        for (int i = 0; i < 8; i++) {
            if ((r & 0x80000000) != 0) {
                r = r << 1 ^ 0x04c11db7; /*
                                          ...
                                          * unreflected alg and an init/final of
                                          * 0, not 0xffffffff
                                          */
            } else {
                r <<= 1;
    ...
    void checksum() {
        ...
        for (int i = 0; i < this.header_len; i++) {
            crc_reg = crc_reg << 8 ^ Page.crc_lookup[crc_reg >>> 24 & 0xff
                    ^ this.header_base[this.header + i] & 0xff];
        }
        for (int i = 0; i < this.body_len; i++) {
            crc_reg = crc_reg << 8 ^ Page.crc_lookup[crc_reg >>> 24 & 0xff
                    ^ this.body_base[this.body + i] & 0xff];
        }
    ...
    public Page copy(final Page p) {
        ...
        System.arraycopy(this.header_base, this.header, tmp, 0,
                this.header_len);
