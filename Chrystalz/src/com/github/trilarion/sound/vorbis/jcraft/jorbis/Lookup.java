...
class Lookup {
    ...
    static float coslook(final float a) {
        final double d = a * (.31830989 * Lookup.COS_LOOKUP_SZ);
        final int i = (int) d;
        return Lookup.COS_LOOKUP[i] + (float) (d - i)
                * (Lookup.COS_LOOKUP[i + 1] - Lookup.COS_LOOKUP[i]);
    }
    ...
    static float invsqlook(final float a) {
        final double d = a * (2.f * Lookup.INVSQ_LOOKUP_SZ)
                - Lookup.INVSQ_LOOKUP_SZ;
        final int i = (int) d;
        return Lookup.INVSQ_LOOKUP[i] + (float) (d - i)
                * (Lookup.INVSQ_LOOKUP[i + 1] - Lookup.INVSQ_LOOKUP[i]);
    }
    ...
    /* interpolated 1./sqrt(p) where .5 <= p < 1. */
    static float invsq2explook(final int a) {
    ...
    static float fromdBlook(final float a) {
        final int i = (int) (a * -(1 << Lookup.FROMdB2_SHIFT));
        return i < 0 ? 1.f
                : i >= Lookup.FROMdB_LOOKUP_SZ << Lookup.FROMdB_SHIFT ? 0.f
                        : Lookup.FROMdB_LOOKUP[i >>> Lookup.FROMdB_SHIFT]
                                * Lookup.FROMdB2_LOOKUP[i
                                        & Lookup.FROMdB2_MASK];
    }
}
