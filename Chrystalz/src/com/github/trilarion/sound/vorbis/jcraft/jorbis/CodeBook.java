...
class CodeBook {
    ...
    int encodevs(final float[] a, final Buffer b, final int step,
            final int addmul) {
    ...
    synchronized int decodevs_add(final float[] a, final int offset,
            final Buffer b, final int n) {
    ...
    int decodev_add(final float[] a, final int offset, final Buffer b,
            final int n) {
        int i, j, entry;
        int t1;
        if (this.dim > 8) {
            for (i = 0; i < n;) {
                ...
                t1 = entry * this.dim;
                for (j = 0; j < this.dim;) {
                    a[offset + i++] += this.valuelist[t1 + j++];
                }
            }
        } else {
            for (i = 0; i < n;) {
    ...
    int decodev_set(final float[] a, final int offset, final Buffer b,
            final int n) {
        int i, j, entry;
        int t1;
        for (i = 0; i < n;) {
    ...
    int decodevv_add(final float[][] a, final int offset, final int ch,
            final Buffer b, final int n) {
        int i, j, entry;
        int chptr = 0;
        for (i = offset / ch; i < (offset + n) / ch;) {
            ...
            final int t1 = entry * this.dim;
            for (j = 0; j < this.dim; j++) {
                a[chptr++][i] += this.valuelist[t1 + j];
                if (chptr == ch) {
                    chptr = 0;
                    i++;
                }
    ...
    int decode(final Buffer b) {
        int ptr = 0;
        final DecodeAux t1 = this.decode_tree;
        final int lok = b.look(t1.tabn);
        if (lok >= 0) {
            ptr = t1.tab[lok];
            b.adv(t1.tabl[lok]);
            if (ptr <= 0) {
    ...
    // returns the entry number or -1 on eof
    int decodevs(final float[] a, final int index, final Buffer b,
            final int step, final int addmul) {
        ...
        // brute force it!
        {
            ...
            for (int i = 0; i < this.entries; i++) {
                if (this.c.lengthlist[i] > 0) {
                    final float _this = CodeBook.dist(this.dim, this.valuelist,
                            e, a, step);
    ...
    private static float dist(final int el, final float[] ref, final int index,
            final float[] b, final int step) {
        float acc = (float) 0.;
        for (int i = 0; i < el; i++) {
    ...
    DecodeAux make_decode_tree() {
        int top = 0;
        final DecodeAux t1 = new DecodeAux();
        final int[] ptr0 = t1.ptr0 = new int[this.entries * 2];
        final int[] ptr1 = t1.ptr1 = new int[this.entries * 2];
        final int[] codelist1 = CodeBook.make_words(this.c.lengthlist,
                this.c.entries);
        if (codelist1 == null) {
            return null;
        }
        t1.aux = this.entries * 2;
        for (int i = 0; i < this.entries; i++) {
            if (this.c.lengthlist[i] > 0) {
                int ptr = 0;
                int j;
                for (j = 0; j < this.c.lengthlist[i] - 1; j++) {
                    final int bit = codelist1[i] >>> j & 1;
                    if (bit == 0) {
                        if (ptr0[ptr] == 0) {
                            ptr0[ptr] = ++top;
                        }
        ...
        t1.tabn = Util.ilog(this.entries) - 4;
        if (t1.tabn < 5) {
            t1.tabn = 5;
        }
        final int n = 1 << t1.tabn;
        t1.tab = new int[n];
        t1.tabl = new int[n];
        for (int i = 0; i < n; i++) {
            int p = 0;
            int j = 0;
            for (j = 0; j < t1.tabn && (p > 0 || j == 0); j++) {
                if ((i & 1 << j) != 0) {
                    p = ptr1[p];
                } else {
                    p = ptr0[p];
