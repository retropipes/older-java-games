...
class Residue0 extends FuncResidue {
    ...
    // re-using partword
    synchronized static int _01inverse(final Block vb, final Object vl,
            final float[][] in, final int ch, final int decodepart) {
        int i, j, k, l, s;
        ...
        final int partwords = (partvals + partitions_per_word - 1)
                / partitions_per_word;
        if (Residue0._01inverse_partword.length < ch) {
            Residue0._01inverse_partword = new int[ch][][];
        }
        for (j = 0; j < ch; j++) {
            if (Residue0._01inverse_partword[j] == null
                    || Residue0._01inverse_partword[j].length < partwords) {
    ...
    synchronized static int _2inverse(final Block vb, final Object vl,
        ...
        final int partwords = (partvals + partitions_per_word - 1)
                / partitions_per_word;
        if (Residue0._2inverse_partword == null
                || Residue0._2inverse_partword.length < partwords) {
    ...
    @Override
    int inverse(final Block vb, final Object vl, final float[][] in,
            final int[] nonzero, final int ch) {
        int used = 0;
        for (int i = 0; i < ch; i++) {
            if (nonzero[i] != 0) {
                in[used++] = in[i];
