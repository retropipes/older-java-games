...
class StaticCodeBook {
    ...
    int unpack(final Buffer opb) {
        ...
        case 1: // ordered
        {
            ...
            for (i = 0; i < this.entries;) {
                final int num = opb.read(Util.ilog(this.entries - i));
                if (num == -1) {
                    // goto _eofout;
                    this.clear();
                    return -1;
    ...
    private int maptype1_quantvals() {
        int vals = (int) Math.floor(Math.pow(this.entries, 1. / this.dim));
    ...
    float[] unquantize() {
        if (this.maptype == 1 || this.maptype == 2) {
            int quantvals;
            final float mindel = StaticCodeBook.float32_unpack(this.q_min);
            final float delta = StaticCodeBook.float32_unpack(this.q_delta);
    ...
    static float float32_unpack(final int val) {
        ...
        return StaticCodeBook.ldexp(mant, (int) exp
                - (StaticCodeBook.VQ_FMAN - 1) - StaticCodeBook.VQ_FEXP_BIAS);
    }
