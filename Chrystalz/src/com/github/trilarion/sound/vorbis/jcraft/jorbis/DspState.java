...
public class DspState {
    ...
    static float[] window(final int type, final int window, final int left,
            final int right) {
        ...
        case 0: // The 'vorbis window' (window 0) is sin(sin(x)*sin(x)*2pi)
        {
            ...
            for (int i = 0; i < left; i++) {
                float x = (float) ((i + .5) / left * DspState.M_PI / 2.);
                x = (float) Math.sin(x);
                x *= x;
                x *= DspState.M_PI / 2.;
                x = (float) Math.sin(x);
            ...
            for (int i = 0; i < right; i++) {
                float x = (float) ((right - i - .5) / right * DspState.M_PI
                        / 2.);
                x = (float) Math.sin(x);
                x *= x;
                x *= DspState.M_PI / 2.;
    ...
    final int init(final Info vi1) {
        this.vi = vi1;
        this.modebits = Util.ilog2(vi1.modes);
        ...
        ((Mdct) this.transform[0][0]).init(vi1.blocksizes[0]);
        ((Mdct) this.transform[1][0]).init(vi1.blocksizes[1]);
        ...
        for (int i = 0; i < DspState.VI_WINDOWB; i++) {
            this.window[0][0][0][i] = DspState.window(i, vi1.blocksizes[0],
                    vi1.blocksizes[0] / 2, vi1.blocksizes[0] / 2);
            this.window[1][0][0][i] = DspState.window(i, vi1.blocksizes[1],
                    vi1.blocksizes[0] / 2, vi1.blocksizes[0] / 2);
            this.window[1][0][1][i] = DspState.window(i, vi1.blocksizes[1],
                    vi1.blocksizes[0] / 2, vi1.blocksizes[1] / 2);
            this.window[1][1][0][i] = DspState.window(i, vi1.blocksizes[1],
                    vi1.blocksizes[1] / 2, vi1.blocksizes[0] / 2);
            this.window[1][1][1][i] = DspState.window(i, vi1.blocksizes[1],
                    vi1.blocksizes[1] / 2, vi1.blocksizes[1] / 2);
        }
        this.fullbooks = new CodeBook[vi1.books];
        for (int i = 0; i < vi1.books; i++) {
            this.fullbooks[i] = new CodeBook();
            this.fullbooks[i].init_decode(vi1.book_param[i]);
        }
        ...
        this.pcm = new float[vi1.channels][];
        {
            for (int i = 0; i < vi1.channels; i++) {
        ...
        this.centerW = vi1.blocksizes[1] / 2;
        this.pcm_current = this.centerW;
        // initialize all the mapping/backend lookups
        this.mode = new Object[vi1.modes];
        for (int i = 0; i < vi1.modes; i++) {
            final int mapnum = vi1.mode_param[i].mapping;
            final int maptype = vi1.map_type[mapnum];
            this.mode[i] = FuncMapping.mapping_P[maptype].look(this,
                    vi1.mode_param[i], vi1.map_param[mapnum]);
        }
        return 0;
    }
     ...
     * @return
     */
    public int synthesis_init(final Info vi1) {
        this.init(vi1);
        // Adjust centerW to allow an easier mechanism for determining output
        this.pcm_returned = this.centerW;
        this.centerW -= vi1.blocksizes[this.W] / 4
                + vi1.blocksizes[this.lW] / 4;
        this.granulepos = -1;
        this.sequence = -1;
        return 0;
    }

    DspState(final Info vi1) {
        this();
        this.init(vi1);
        // Adjust centerW to allow an easier mechanism for determining output
        this.pcm_returned = this.centerW;
    ...
    public int synthesis_blockin(final Block vb) {
        ...
        if (this.centerW > this.vi.blocksizes[1] / 2
                && this.pcm_returned > 8192) {
            // don't shift too much; we need to have a minimum PCM buffer of
            // 1/2 long block
            int shiftPCM = this.centerW - this.vi.blocksizes[1] / 2;
            shiftPCM = this.pcm_returned < shiftPCM ? this.pcm_returned
                    : shiftPCM;
            ...
            if (shiftPCM != 0) {
                for (int i = 0; i < this.vi.channels; i++) {
                    System.arraycopy(this.pcm[i], shiftPCM, this.pcm[i], 0,
                            this.pcm_current);
                }
            }
        }
        ...
        this.sequence = vb.sequence;
        {
            ...
            int _centerW = this.centerW + this.vi.blocksizes[this.lW] / 4
                    + sizeW / 4;
            ...
            if (endW > this.pcm_storage) {
                ...
                for (int i = 0; i < this.vi.channels; i++) {
                    ...
                    System.arraycopy(this.pcm[i], 0, foo, 0,
                            this.pcm[i].length);
                    this.pcm[i] = foo;
                }
            }
            ...
            case 0:
                ...
                break;
            case 1:
                beginSl = this.vi.blocksizes[1] / 4
                        - this.vi.blocksizes[this.lW] / 4;
