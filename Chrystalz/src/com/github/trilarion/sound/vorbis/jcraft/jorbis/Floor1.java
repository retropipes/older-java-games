...
class Floor1 extends FuncFloor {
    ...
    Object look(final DspState vd, final InfoMode mi, final Object i) {
        int _n = 0;
        final int[] sortpointer = new int[Floor1.VIF_POSIT + 2];
        // Info vi=vd.vi;
        final InfoFloor1 info = (InfoFloor1) i;
    ...
    Object inverse1(final Block vb, final Object ii, final Object memo) {
        ...
        if (vb.opb.read(1) == 1) {
            ...
            for (int i = 2; i < look.posts; i++) {
                final int predicted = Floor1.render_point(
    ...
    private static int render_point(final int x0, final int x1, final int y0n,
            final int y1n, final int x) {
        int y0 = y0n;
        int y1 = y1n;
    ...
    private static void render_line(final int x0, final int x1, final int y0,
            final int y1, final float[] d) {
        final int dy = y1 - y0;
        final int adx = x1 - x0;
        int ady = Math.abs(dy);
    ...
    class InfoFloor1 {
        static final int INFO_VIF_POSIT = 63;
        static final int VIF_CLASS = 16;
        static final int VIF_PARTS = 31;
        int partitions; /* 0 to 31 */
        int[] partitionclass = new int[InfoFloor1.VIF_PARTS]; /* 0 to 15 */
        int[] class_dim = new int[InfoFloor1.VIF_CLASS]; /* 1 to 8 */
        int[] class_subs = new int[InfoFloor1.VIF_CLASS]; /*
                                                           * 0,1,2,3 (bits: 1<<n
                                                           * poss)
                                                           */
        int[] class_book = new int[InfoFloor1.VIF_CLASS]; /*
                                                           * subs ^ dim entries
                                                           */
        int[][] class_subbook = new int[InfoFloor1.VIF_CLASS][]; /*
                                                                  * [VIF_CLASS][
                                                                  * subs]
                                                                  */
        int mult; /* 1 2 3 or 4 */
        int[] postlist = new int[InfoFloor1.INFO_VIF_POSIT
        ...
        Object copy_info() {
            ...
            System.arraycopy(info.class_dim, 0, ret.class_dim, 0,
                    InfoFloor1.VIF_CLASS);
            System.arraycopy(info.class_subs, 0, ret.class_subs, 0,
                    InfoFloor1.VIF_CLASS);
            System.arraycopy(info.class_book, 0, ret.class_book, 0,
                    InfoFloor1.VIF_CLASS);
            for (int j = 0; j < InfoFloor1.VIF_CLASS; j++) {
                System.arraycopy(info.class_subbook[j], 0, ret.class_subbook[j],
                        0, 8);
            }
            ret.mult = info.mult;
            System.arraycopy(info.postlist, 0, ret.postlist, 0,
                    InfoFloor1.INFO_VIF_POSIT + 2);
    ...
    static class LookFloor1 {
        static final int LOOK_VIF_POSIT = 63;
        int[] sorted_index = new int[LookFloor1.LOOK_VIF_POSIT + 2];
        int[] forward_index = new int[LookFloor1.LOOK_VIF_POSIT + 2];
        int[] reverse_index = new int[LookFloor1.LOOK_VIF_POSIT + 2];
        int[] hineighbor = new int[LookFloor1.LOOK_VIF_POSIT];
        int[] loneighbor = new int[LookFloor1.LOOK_VIF_POSIT];
        int posts;
        int n;
        int quant_q;
        InfoFloor1 vi;
