...
public class VorbisFile {
     ...
     * @throws SoundException
     */
    public VorbisFile(final InputStream is, final byte[] initial,
            final int ibytes) throws SoundException {
        super();
    ...
    private int get_prev_page(final Page page) throws SoundException {
        ...
        while (offst == -1) {
            ...
            while (this.offset < begin + VorbisFile.CHUNKSIZE) {
                ret = this.get_next_page(page,
                        begin + VorbisFile.CHUNKSIZE - this.offset);
    ...
    int bisect_forward_serialno(final long begin, long searched1, final long end1,
        ...
        if (searched >= end1 || ret == -1) {
            this.links = m + 1;
            this.offsets = new long[m + 2];
            this.offsets[m + 1] = searched;
        } else {
            ret = this.bisect_forward_serialno(next, this.offset, end1,
                    page.serialno(), m + 1);
            if (ret == VorbisFile.OV_EREAD) {
                return VorbisFile.OV_EREAD;
            }
    ...
    int fetch_headers(final Info vi1, final Comment vc1, final int[] serialno,
        ...
        while (i < 3) {
            ...
            while (i < 3) {
                ...
                if (vi1.synthesis_headerin(vc1, op) != 0) {
                    vi1.clear();
                    vc1.clear();
                    this.os.clear();
                    return -1;
    ...
    void prefetch_all_headers(final Info first_i, final Comment first_c,
            final int dataoffset) throws SoundException {
        final Page og = new Page();
        int ret;
        ...
        for (int i = 0; i < this.links; i++) {
            if (first_i != null && first_c != null && i == 0) {
                ...
                if (this.fetch_headers(this.vi[i], this.vc[i], null,
                        null) == -1) {
            ...
            {
                final long end1 = this.offsets[i + 1]; // !!!
                this.seek_helper(end1);
                while (true) {
    ...
    int open_seekable() throws SoundException {
        ...
        end1 = this.get_prev_page(og);
        // moer than one logical bitstream?
        if (og.serialno() != serialno) {
            // Chained bitstream. Bisect-search each logical bitstream
            // section. Do so based on serial number only
            if (this.bisect_forward_serialno(0, 0, end1 + 1, serialno, 0) < 0) {
                this.clear();
                return VorbisFile.OV_EREAD;
            }
        } else {
            // Only one logical bitstream
            if (this.bisect_forward_serialno(0, end1, end1 + 1, serialno,
                    0) < 0) {
                this.clear();
                return VorbisFile.OV_EREAD;
            }
        ...
        while (true) {
            ...
            if (this.decode_ready) {
                ...
                if (result > 0) {
                    ...
                    if (this.vb.synthesis(op) == 0) { // lazy check for lazy
                        ...
                        {
                            final int oldsamples = this.vd
                                    .synthesis_pcmout(null, null);
                            this.vd.synthesis_blockin(this.vb);
                            this.samptrack += this.vd.synthesis_pcmout(null,
                                    null) - oldsamples;
                            this.bittrack += op.bytes * 8;
                        }
                        // update the pcm offset.
                        if (granulepos != -1 && op.e_o_s == 0) {
                            final int link = this.seekable ? this.current_link
                                    : 0;
                            int samples;
            ...
            if (!this.decode_ready) {
                ...
                if (this.seekable) {
                    ...
                    final int foo[] = new int[1];
                    final int ret = this.fetch_headers(this.vi[0], this.vc[0],
                            foo, og);
                    this.current_serialno = foo[0];
                    if (ret != 0) {
                        return ret;
    ...
    // return: -1) error
    // 0) OK
    final int open(final InputStream is, final byte[] initial, final int ibytes)
            throws SoundException {
        return this.open_callbacks(is, initial, ibytes);
    }

    int open_callbacks(final InputStream is, final byte[] initial,
            final int ibytes// , callbacks callbacks
    ) throws SoundException {
        int ret;
        this.datasource = is;
        this.oy.init();
    ...
    public int bitrate(final int i) {
        ...
        if (i < 0) {
            ...
            if (this.seekable) {
                ...
                return (int) Math
                        .rint((this.offsets[i + 1] - this.dataoffsets[i]) * 8
                                / this.time_total(i));
            } else {
    ...
    public int bitrate_instant() {
        ...
        final int ret = (int) (this.bittrack / this.samptrack
                * this.vi[_link].rate + .5);
        this.bittrack = 0.f;
        this.samptrack = 0.f;
        return ret;
    }
    ...
    public int pcm_seek(final long pos) {
        ...
        // would be an error condition)
        {
            ...
            while (begin < end1) {
                long bisect;
                int ret;
                if (end1 - begin < VorbisFile.CHUNKSIZE) {
                    bisect = begin;
                } else {
                    bisect = (end1 + begin) / 2;
                }
                this.seek_helper(bisect);
                ret = this.get_next_page(og, end1 - bisect);
                if (ret == -1) {
                    end1 = bisect;
                } else {
    ...
    int time_seek(final float seconds) {
        ...
        {
            final long target = (long) (pcm_total
                    + (seconds - time_total) * this.vi[link].rate);
    ...
    public float time_tell() {
        ...
        return time_total
                + (float) (this.pcm_offset - pcm_total) / this.vi[link].rate;
    }
    ...
    int read(final byte[] buffer, final int length, final int bigendianp,
            final int word, final int sgned, final int[] bitstream) {
    ...
    static class SeekableInputStream extends InputStream {
        ...
        @Override
        public int read(final byte[] buf, final int s, final int len)
