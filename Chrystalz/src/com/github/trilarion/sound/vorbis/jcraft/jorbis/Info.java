...
public class Info {
    ...
    public void clear() {
        ...
        for (int i = 0; i < this.maps; i++) { // unpack does the range checking
            FuncMapping.mapping_P[this.map_type[i]]
                    .free_info(this.map_param[i]);
        }
        this.map_param = null;
        ...
        for (int i = 0; i < this.floors; i++) { // unpack does the range
                                                // checking
            FuncFloor.floor_P[this.floor_type[i]]
                    .free_info(this.floor_param[i]);
        }
        this.floor_param = null;
        for (int i = 0; i < this.residues; i++) { // unpack does the range
                                                  // checking
            FuncResidue.residue_P[this.residue_type[i]]
                    .free_info(this.residue_param[i]);
        }
    ...
    int unpack_info(final Buffer opb) {
        ...
        if (this.rate < 1 || this.channels < 1 || this.blocksizes[0] < 8
                || this.blocksizes[1] < this.blocksizes[0]
                || opb.read(1) != 1) {
            this.clear();
            return -1;
        }
    ...
    int unpack_books(final Buffer opb) {
        ...
        for (int i = 0; i < this.times; i++) {
            ...
            if (this.time_type[i] < 0 || this.time_type[i] >= Info.VI_TIMEB) {
                this.clear();
                return -1;
            }
            this.time_param[i] = FuncTime.time_P[this.time_type[i]].unpack(this,
                    opb);
        ...
        if (this.floor_param == null
                || this.floor_param.length != this.floors) {
            this.floor_param = new Object[this.floors];
        }
        for (int i = 0; i < this.floors; i++) {
            this.floor_type[i] = opb.read(16);
            if (this.floor_type[i] < 0
                    || this.floor_type[i] >= Info.VI_FLOORB) {
                this.clear();
                return -1;
            }
            this.floor_param[i] = FuncFloor.floor_P[this.floor_type[i]]
                    .unpack(this, opb);
        ...
        for (int i = 0; i < this.residues; i++) {
            ...
            if (this.residue_type[i] < 0
                    || this.residue_type[i] >= Info.VI_RESB) {
                this.clear();
                return -1;
            }
            this.residue_param[i] = FuncResidue.residue_P[this.residue_type[i]]
                    .unpack(this, opb);
        ...
        for (int i = 0; i < this.maps; i++) {
            ...
            if (this.map_type[i] < 0 || this.map_type[i] >= Info.VI_MAPB) {
                this.clear();
                return -1;
            }
            this.map_param[i] = FuncMapping.mapping_P[this.map_type[i]]
                    .unpack(this, opb);
        ...
        for (int i = 0; i < this.modes; i++) {
            ...
            if (this.mode_param[i].windowtype >= Info.VI_WINDOWB
                    || this.mode_param[i].transformtype >= Info.VI_WINDOWB
    ...
    int pack_books(final Buffer opb) {
        ...
        for (int i = 0; i < this.floors; i++) {
            ...
            FuncFloor.floor_P[this.floor_type[i]].pack(this.floor_param[i],
                    opb);
        }
        // residues
        ...
        for (int i = 0; i < this.residues; i++) {
            ...
            FuncResidue.residue_P[this.residue_type[i]]
                    .pack(this.residue_param[i], opb);
        }
        // maps
        opb.write(this.maps - 1, 6);
        for (int i = 0; i < this.maps; i++) {
            opb.write(this.map_type[i], 16);
            FuncMapping.mapping_P[this.map_type[i]].pack(this,
                    this.map_param[i], opb);
        }
        // modes
    ...
    public String toString() {
        return "version:" + this.version + ", channels:" + this.channels
                + ", rate:" + this.rate + ", bitrate:" + this.bitrate_upper
                + "," + this.bitrate_nominal + "," + this.bitrate_lower;
    }
}
