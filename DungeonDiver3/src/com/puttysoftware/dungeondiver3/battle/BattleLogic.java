...
import com.puttysoftware.dungeondiver3.battle.damageengines.DamageEngine;
...
import com.puttysoftware.dungeondiver3.support.ai.AutoAI;
...
import com.puttysoftware.dungeondiver3.support.items.combat.CombatItemChucker;
...
import com.puttysoftware.dungeondiver3.support.spells.SpellCaster;
...
public class BattleLogic {
    ...
    private void doBattleInternal(final Map bMap, final Battle b) {
        // Initialize Battle
        DungeonDiver3.getApplication().setInBattle();
    ...
    private void battleDone() {
        ...
        if (!this.resultDoneAlready) {
            ...
            // Leave Battle
            this.hideBattle();
            DungeonDiver3.getApplication().setInGame();
            // Return to whence we came
            DungeonDiver3.getApplication().getGameManager().showOutput();
            DungeonDiver3.getApplication().getGameManager().redrawMap();
            DungeonDiver3.getApplication().getGameManager().updateStats();
            // Check for Game Over
            DungeonDiver3.getApplication().getGameManager().checkGameOver();
        }
    }
    ...
    private void setCharacterLocations() {
        ...
        RandomRange randY = new RandomRange(0, this.bd.getBattleMap()
                .getColumns() - 1);
        int rx, ry;
        // Set Character Locations
    ...
    private boolean setNextActive(boolean isNewRound) {
        ...
        if (res != -1) {
            ...
            if (this.bd.getActiveCharacter().getTemplate().hasAI()
                ...
                // No AI
                SoundManager.playSound(GameSoundConstants.SOUND_PLAYER_UP);
            }
            return false;
        } else {
            // Reset Speed Array
    ...
    void arrowDone(BattleCharacter hit) {
        ...
        if (hit != null && !hit.getTemplate().isAlive()) {
            ...
            // Update Prestige
            BattleDefinitions.killedEnemy(this.bd.getActiveCharacter()
                    .getTemplate());
        ...
        if (next != null && nextGround != null && currGround != null) {
            if (!next.isSolidInBattle()) {
                if ((useAP && this.getActiveActionCounter() >= MapObject
                    ...
                    this.decrementActiveActionCounterBy(MapObject
                            .getBattleAPCost());
                    ...
                    if (LocalPreferencesManager.getRandomBattleEnvironment()) {
                        ...
                        InternalScriptRunner.runScript(nextGround
                                .getBattlePostMoveScript(active));
                    }
                } else {
                ...
                if (next.isOfType(TypeConstants.TYPE_BATTLE_CHARACTER)) {
                    if ((useAP && this.getActiveAttackCounter() > 0) || !useAP) {
                        ...
                        if (LocalPreferencesManager
                            ...
                            // Run any script attached to ground
                            InternalScriptRunner.runScript(currGround
                                    .getBattlePostMoveScript(active));
                        }
                        // Do damage
                        ...
                        if (enemy.isAlive()
                            ...
                            SoundManager
                                    .playSound(GameSoundConstants.SOUND_LOW_HEALTH);
                        }
                        // Handle enemy death
                        if (!enemy.isAlive()) {
                        ...
                        if (!active.getTemplate().isAlive()) {
                            // Update Prestige
                            BattleDefinitions.killedInBattle(active
                                    .getTemplate());
                            // Play death sound
                            SoundManager
                                    .playSound(GameSoundConstants.SOUND_DEATH);
            ...
            if (!this.bd.getActiveCharacter().getTemplate().hasAI()
                ...
                if (success) {
                    this.bd.getActiveCharacter().modifyAP(
                            BattleLogic.ITEM_ACTION_POINTS);
                }
                int currResult = this.getResult();
                if (currResult != BattleResults.IN_PROGRESS) {
                    // Battle Done
    ...
    public boolean steal() {
        ...
        if (this.getActiveActionCounter() > 0) {
            ...
            int stealAmount = 0;
            this.bd.getActiveCharacter().modifyAP(
                    BattleLogic.STEAL_ACTION_POINTS);
    ...
    public boolean drain() {
        ...
        if (this.getActiveActionCounter() > 0) {
            ...
            int drainAmount = 0;
            this.bd.getActiveCharacter().modifyAP(
                    BattleLogic.DRAIN_ACTION_POINTS);
    ...
    private void waitForAI() {
        this.battleGUI.turnEventHandlersOff();
        DungeonDiver3.getApplication().getMenuManager().disableBattleMenus();
    }
    ...
    private void stopWaitingForAI() {
        ...
        this.battleGUI.turnEventHandlersOn();
        DungeonDiver3.getApplication().getMenuManager().enableBattleMenus();
    }
}
