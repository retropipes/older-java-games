...
public class Game extends JFrame {
    ...
    private static final GameImage DIALOG_ICON = ImageLoader
            .loadUIImage("micrologo");
    // Fields
    EventHandler eh;
    GameMap map;
    GameDraw draw;
    ...
    boolean fixBounds(final int opx, final int opy) {
        ...
        if (go0.killsPlayer() || go1.killsPlayer()) {
            ...
            JOptionPane.showMessageDialog(this, "You are dead!", "LaserTank",
                    JOptionPane.INFORMATION_MESSAGE, Game.DIALOG_ICON);
            return false;
        }
        if (go0.isGoal() || go1.isGoal()) {
            ...
            this.map.set(Game.EMPTY, opx, opy, 1);
            this.removeKeyListener(this.eh);
            JOptionPane.showMessageDialog(this, "You win!", "LaserTank",
                    JOptionPane.INFORMATION_MESSAGE, Game.DIALOG_ICON);
            return false;
        }
        return true;
    ...
    boolean fixLaserBounds(final int olx, final int oly) {
        ...
        if (go0.isSolid() || go1.isSolid()) {
            ...
            try {
                go0next = this.map.get(this.laserX + dirX, this.laserY + dirY,
                        0);
            ...
            try {
                go1next = this.map.get(this.laserX + dirX, this.laserY + dirY,
                        1);
            } catch (final ArrayIndexOutOfBoundsException e) {
                // Ignore
            }
            if (go1.laserMoves() && !go1next.isSolid()) {
                ...
                if (go0next.killsPlayer()) {
                    this.sink.play();
                    this.map.set(Game.BRIDGE, this.laserX + dirX,
                            this.laserY + dirY, 0);
                } else {
                    this.map.set(Game.BOX, this.laserX + dirX,
                            this.laserY + dirY, 1);
                }
            } else {
                this.laserDead.play();
    ...
    private class GameDraw extends JPanel {
        ...
        public GameDraw(final int mapSize, final int tileSize) {
            ...
            this.setPreferredSize(
                    new Dimension(mapSize * tileSize, mapSize * tileSize));
            this.tSize = tileSize;
        }
        ...
        public void draw() {
            ...
            if (gr != null) {
                for (int x = 0; x < Game.MAP_SIZE; x++) {
                    for (int y = 0; y < Game.MAP_SIZE; y++) {
                        final GameImage gi0 = g.map.get(x, y, 0)
                                .getAppearance();
                        final GameImage gi1 = g.map.get(x, y, 1)
                                .getAppearance();
                        final GameImage gi2 = g.map.get(x, y, 2)
                                .getAppearance();
                        final GameImage gi3 = g.map.get(x, y, 3)
                                .getAppearance();
                        final GameImage gi = GameImageCache.getComposite(gi0,
                                gi1, gi2, gi3);
